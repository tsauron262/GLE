...
class majCodeConfigurationnProd{
    ...
    function getSN($fin, $id_prod = null){
        $this->db->query();
    }
    ...
    function updateEquipmentOrfellin(){
        ...
        while($ln4 = $this->db->fetch_object($sql4)){
            $this->db->query("UPDATE llx_be_equipment SET id_product = ".$ln4->fk_object." WHERE serial LIKE '%".$ln4->code_config."' and ( LENGTH(serial) = 13 || LENGTH(serial) = 12)");
        }
    }
    
    function testSerialDouble(){
        $sql3 = $this->db->query("SELECT COUNT(*) as nbIdentique, serial FROM `llx_be_equipment` WHERE `id_product` > 0 AND ( LENGTH(serial) = 13 || LENGTH(serial) = 12) GROUP BY `serial`, id_product HAVING nbIdentique > 1  
        ORDER BY COUNT(*)  DESC");
