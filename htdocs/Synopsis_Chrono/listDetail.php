...
$sql = $db->query($requete);
$js .= 'var statutRess = "';
$js .= "-1:" . preg_replace("/'/", "\\'", utf8_encode(html_entity_decode("S&eacute;lection ->"))) . ";";

while ($res = $db->fetch_object($sql)) {
    $fakeChrono = new Chrono($db);
    $fakeChrono->statut = $res->fk_statut;

    $js .= $res->fk_statut . ":" . preg_replace("/'/", "\\'", utf8_encode(html_entity_decode($fakeChrono->getLibStatut(0)))) . ";";
}
