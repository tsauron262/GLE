...
if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty($usercancreate))
	|| (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty($user->rights->multicompany->order_advance->validate))) {
	$usercanvalidate = false;
}
if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty($usercancreate))
	|| (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty($user->rights->multicompany->order_advance->cancel))) {
	$usercancancel = false;
}
if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty($usercanread))
	|| (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty($user->rights->multicompany->order_advance->send))) {
	$usercansend = false;
}
