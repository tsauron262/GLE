...
$requete = "SELECT ".MAIN_DB_PREFIX."commande_fournisseur.ref," .
        "          ".MAIN_DB_PREFIX."commande_fournisseur.fk_soc, " .
        "          ".MAIN_DB_PREFIX."commande_fournisseur.total_ht," .
        "          ".MAIN_DB_PREFIX."commande_fournisseur.rowid," .
...
{
    $requete .= " WHERE ".MAIN_DB_PREFIX."commande_fournisseur.fk_soc = ".$_GET['societe_id'];
}
...
if ($resql)
{
    while ($res=$db->fetch_object($resql))
    {
        ...
        print "    <TD align='left'><a href=\"commandeFourn_detail.php?commande_id=".$res->rowid."\">".img_object($langs->trans("ShowOrder"),"order").$res->ref."</A>";
        $soc = new Societe($db);
        $soc->fetch($res->fk_soc);
        $socname = $soc->nom;
        $com = new CommandeFournisseur($db,$res->fk_soc,$res->rowid);
        $com->fetch($res->rowid);
