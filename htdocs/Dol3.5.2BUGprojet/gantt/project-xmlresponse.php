...
switch ($action) {
    ...
    case "descTask": {
            $requete = "SELECT " . MAIN_DB_PREFIX . "projet_task.title,
                           ...
                           ifnull(" . MAIN_DB_PREFIX . "projet_task.color, '0000FF') as color,
    ...
    case 'insert': {
            ...
            $requete = "INSERT INTO " . MAIN_DB_PREFIX . "projet_task
                                (fk_projet, fk_task_parent, title, duration_effective, fk_user_creat,statut,note,progress,description,color,url,priority , shortDesc, level,dateDeb)
...
function do_select($db, $project_id) {
    ...
    $requete = "SELECT " . MAIN_DB_PREFIX . "projet_task.title,
                      ...
                      FROM " . MAIN_DB_PREFIX . "projet_task
                 LEFT JOIN " . MAIN_DB_PREFIX . "projet_task_time ON " . MAIN_DB_PREFIX . "projet_task_time.fk_task = " . MAIN_DB_PREFIX . "projet_task.rowid
                     WHERE fk_projet = " . $project_id . " AND " . MAIN_DB_PREFIX . "projet_task.fk_task_parent is null";
    $sql = $db->query($requete);
