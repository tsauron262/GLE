...
\nNB : si ces interlocuteurs sont différents, utiliser les champs prévus à cet effet';
			...
			if ($this->hasRemiseCRT()) {
				$clientF = $this->getClientFinal();
				if (!$clientF->getData('type_educ')) {
				...
				} elseif ($clientF->getData('type_educ') == 'E4') {
					$date_fin_validite = $clientF->getData('type_educ_fin_validite');
...
//        return $conf->commande->dir_output;
		...
		if ($this->isLoaded() && $this->dol_object->entity > 0) //            die($conf->fournisseur->facture->multidir_output[$this->dol_object->entity] . '/' . $subdir);
		{
