...
$requete = "SELECT count(*) as cnt, lat,lng, ".MAIN_DB_PREFIX."societe.nom,".MAIN_DB_PREFIX."societe.rowid FROM ".MAIN_DB_PREFIX."facture, ".MAIN_DB_PREFIX."societe, Babel_GeoBI
WHERE ".MAIN_DB_PREFIX."facture.fk_soc=".MAIN_DB_PREFIX."societe.rowid AND ".MAIN_DB_PREFIX."societe.rowid = Babel_GeoBI.`socid` AND lat is not null AND year(datef)  > ". $dateStart ."
...
{
        $dateStart = date('Y') - 3 ;
    $requete = "SELECT SUM(total) as t, quarter(datef) as d, year(datef) as y,count(total) as c
                  FROM ".MAIN_DB_PREFIX."facture
                 WHERE fk_soc = ". $res->rowid." AND year(datef)  > ". $dateStart ."
              GROUP BY d, y";
              //print $requete."\n";
    $arrPropal = array();
...
$table = '<table class"canvasStats" style="display: none;" id="factureTable'.$res->rowid.'">
