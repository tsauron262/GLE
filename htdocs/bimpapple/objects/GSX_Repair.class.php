							...
							foreach ($reason['messages'] as $message) {
								...
								if (is_array($message) && isset($message['id'])) {
									if ($action == 'ACKNOWLEDGE') {
										$NbACKNOWLEDGE++;
										$msg .= BimpInput::renderInput('hidden', 'actions_id_' . $NbACKNOWLEDGE, $message['id']);
										$msg .= BimpInput::renderInput('select', 'actions_value_' . $NbACKNOWLEDGE, '', array('options' => array('N' => 'Non', 'Y' => 'Oui')));
									}
								}
							}
						}
...
//            }
	...
	public function fetchPartReturnLabel($partNumber, $returnOrderNumber, $returnType, $sequenceNumber, &$fileUrl = '', $shipTo = '', &$filePath = '')
	{
		$errors = array();
					...
					if (!count($errors)) {
						...
						foreach ($parts as $part) {
							...
							if (BimpObject::objectLoaded($savPart)) {
								...
								$equipment = $sav->getChildObject('equipment');
								$pu_ht = (float) BS_ApplePart::convertPrixStatic($part_type, (float) $part['new_price'], $part['number'], $equipment->isIphone());
