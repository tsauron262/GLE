...
//            $pdfkey = array_search($res->pdfCatId, $colarray2);
    ...
    set_include_path(get_include_path() . PATH_SEPARATOR . DOL_DOCUMENT_ROOT.'/Babel_TechPeople/deplacements/PHPExcel-1.6.6');
...
$docxl->getProperties()->setSubject("Note de frais de ".$fuser->fullname ." pour la période ".$deplacement->tsperiode);
$docxl->getProperties()->setDescription("Note de frais de ".$fuser->fullname ." pour la période ".$deplacement->tsperiode);
$docxl->getProperties()->setKeywords("note de frais ".$fuser->fullname." ");
$docxl->getProperties()->setCategory("Note de fFrais");
...
$propref = sanitize_string($deplacement->ref);
$dir = $conf->deplacement->dir_output . "/".$deplacement->fk_user . $propref;
$file = $dir ."/" . $propref . ".xlsx";
$file2 = $dir ."/" . $propref . ".csv";
$file3 = $dir ."/" . $propref . ".xls";


$records = '/tmp/ResultXLS.xlsx';
//$writer->save($file);
$writer->setOffice2003Compatibility(true);
$writer->save($file);
//require_once("PHPExcel/Writer/CSV.php");

//$objWriter = new PHPExcel_Writer_CSV($docxl);
...
$objWriter1 = new PHPExcel_Writer_Excel5($docxl);
$objWriter1->save($file3);
