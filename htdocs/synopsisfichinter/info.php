...
/**
  \file       htdocs/synopsisfichinter/info.php
  \ingroup    fichinter
...
if ($_REQUEST['action'] == "changeCli" && $user->rights->synopsisficheinter->rattacher) {
    $requete = false;
    $requete1 = false;
    $requete2 = false;
    if ($_REQUEST['socid'] > 0) {
        $requete = "UPDATE " . MAIN_DB_PREFIX . "fichinter SET fk_soc = " . $_REQUEST['socid'] . " WHERE rowid = " . $fichinterid;

        $tabDi = getElementElement("DI", "FI", null, $fichinterid);
        if (isset($tabDi[0])) {
            $idDi = $tabDi[0]['s'];
            $requete1 = "UPDATE " . MAIN_DB_PREFIX . "synopsisdemandeinterv SET fk_soc = " . $_REQUEST['socid'] . " WHERE rowid = " . $idDi;
        }
    }
...
if ($user->rights->synopsisficheinter->rattacher) {
    ...
    print '<table class="border" width="400" cellpadding=15>';
    print"<form action='info.php?id=" . $fichinter->id . "&action=changeCli' METHOD=POST>";
    print "<tr><th class=\"ui-widget-header ui-state-default\">" . $langs->trans("Company") . "</td><td class=\"ui-widget-content\">";
    print $form->select_company($fichinter->socid, 'socid', '', 1);
    print "</td>";
    print '<td colspan="2" align="center"  class="ui-widget-content">';
