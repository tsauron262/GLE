...
{
    $requete = "UPDATE ".MAIN_DB_PREFIX."Synopsis_Affaire SET statut = 1 WHERE id =".$affaireid;
    $sql=$db->query($requete);
    if (!$sql)
    {
...
{
    $requete = "UPDATE ".MAIN_DB_PREFIX."Synopsis_Affaire SET statut = 4 WHERE id =".$affaireid;
    $sql=$db->query($requete);
    if (!$sql)
    {
        $msg = "Mise &agrave; jour impossible";
...
if ($_REQUEST['action']=='modify')
{
    ...
    $requete = "UPDATE ".MAIN_DB_PREFIX."Synopsis_Affaire SET nom = '".$nom."', description='".$description."' WHERE id = ".$_REQUEST['id'];
    $sql = $db->query($requete);
    if ($sql)
    {
