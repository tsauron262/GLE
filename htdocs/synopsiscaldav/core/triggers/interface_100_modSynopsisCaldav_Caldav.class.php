...
//                $db->query("UPDATE " . MAIN_DB_PREFIX . "user_extrafields SET ctag = ctag+1 WHERE fk_object = " . $object->userownerid);
        ...
        if ($action == "ACTION_MODIFY"){
            $db->query("UPDATE ".MAIN_DB_PREFIX."synopsiscaldav_event SET participentExt = '".$participentExt."', sequence = ".((isset($infoEvent["sequence"]) && ($infoEvent["sequence"] > 0 OR $infoEvent["sequence"] === 0))? $infoEvent["sequence"]+1 : "sequence +1").", etag = '".$objectEtag2."', uri = IF(uri is not null, uri, CONCAT(CONCAT('-', fk_object), '.ics')) WHERE fk_object = ".$object->id);
        }
