...
class Societe extends CommonObject
{
    ...
    function fetch($rowid, $ref='', $ref_ext='', $ref_int='', $idprof1='',$idprof2='',$idprof3='',$idprof4='',$idprof5='',$idprof6='', $email='')
    {
        ...
        else if ($ref)   $sql .= " WHERE (s.nom = '".$this->db->escape($ref)."' || s.code_fournisseur = '".$this->db->escape($ref)."') AND s.entity IN (".getEntity($this->element, 1).")";
    ...
    function getNomUrl($withpicto=0, $option='', $maxlen=0, $notooltip=0, $save_lastsearch_value=-1)
    {
        ...
        while($ln = $this->db->fetch_object($sql)){
            ...
            $label .="<br/><b>Commercial : </b>".$userT->getNomUrl(1, '', 0, 1, 20);
        }
        /*fmoddrsi*/
    ...
    function id_prof_url($idprof,$thirdparty)
    {
        ...
        if (empty($reshook))
        {
            ...
            if ($idprof == 1 && $thirdparty->country_code == 'FR') $url='http://www.societe.com/cgi-bin/search?champs='.str_replace(" ", "", $thirdparty->idprof1);    // See also http://avis-situation-sirene.insee.fr/
