...
//        $this->path = (defined('DIR_SYNCH') ? DIR_SYNCH : DOL_DATA_ROOT . "/synopsischrono/export/" ) . "/extractPaiGle/";
    ...
    public function exportPaiementNormal() {
        ...
        while ($ligne = $this->db->fetch_object($result)) {
            ...
            $tabPaiement[] = array("EcrPiece" => $ligne->ref, "JorCode" => $ligne->labB, "EcrRef" => $ligne->code_client, "EcrDate" => dol_print_date($ligne->datec, "%d/%m/%Y"), "EcrGCptCode" => "411" . $ligne->codeCli8Sens, /* "EcrCpt" => "411071719-GD", */ "EcrLib" => $libOp, "EcrDebit" => $debit, "EcrCredit" => $credit, /* "EcrSolde" => $solde, */ "EcrLettrage" => $ligne->ref, "PcvGpriID" => $ligne->Collab8sens);
            $tabPfId[] = $ligne->pfid;
        }
