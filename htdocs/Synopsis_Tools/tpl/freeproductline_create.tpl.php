...
<form name="addproduct" id="addproduct"	action="<?php echo $_SERVER["PHP_SELF"] . '?id=' . $this->id; ?>#add" method="POST">
    ...
    <tr <?php echo $bcnd[$var]; ?>>
        <td<?php echo (!empty($conf->global->MAIN_VIEW_LINE_NUMBER) ? ' colspan="2"' : ''); ?>>
            <?php
            echo '<span>';
            echo select_type_of_lines2(isset($_POST["type"]) ? $_POST["type"] : -1, 'type', 1);
            echo '</span>';
            ...
            if ((!empty($conf->product->enabled) && !empty($conf->service->enabled)) || (empty($conf->product->enabled) && empty($conf->service->enabled)))
                echo '<br>';
        ...
        <td align="right"><?php
            if ($seller->tva_assuj == "0")
                ...
                echo $form->load_tva('tva_tx', (isset($_POST["tva_tx"]) ? $_POST["tva_tx"] : -1), $seller, $buyer);
            ?>
        </td>
