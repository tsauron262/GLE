...
class InvoiceRge3PDF extends InvoicePDF
{
    ...
    protected function initHeader()
    {
        ...
        if (file_exists($soc_logo_file)) {
            ...
            if (isset($sizes['width']) && (int) $sizes['width'] && isset($sizes['height']) && $sizes['height']) {
                $tabTaille = $this->calculeWidthHeightLogo($sizes['width'], $sizes['height'], 250, 90);
...
Les travaux concernent des opérations standardisées, selon la fiche BAR-TH-113. Le montant de cette contribution financière, hors champ d’application de la TVA, est susceptible de varier en fonction des travaux effectivement réalisés et du volume des CEE attribués à l’opération et est estimé à ".price($this->object->array_options['options_prime'])." €";
            $html .= '</div>';
    //        $html .= '</p>';
            $html .= '</td></tr></table>';
        }
