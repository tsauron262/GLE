	...
	public function isActionAllowed($action, &$errors = [])
	{
		...
		switch ($action) {
			...
			case 'forceDevisSigned':
				if (!in_array((int) $this->getData('status'), array(self::STATUS_ATTENTE, self::STATUS_ACCEPTED))) {
					$errors[] = 'Le statut actuel de la demande de location ne permet pas cette opétation';
				}
...
//                }
			...
			case 'submitContrat':
				$errors[] = 'Fonctionnalité désactivée';
				return 0;
...
//				return 1;
	...
	public function isDemandeValid(&$errors = array(), $id_demande_refin = 0)
	{
		...
		if ($id_demande_refin) {
			$df = BimpCache::getBimpObjectInstance('bimpfinancement', 'BF_DemandeRefinanceur', $id_demande_refin);
			if (!BimpObject::objectLoaded($df)) {
				$errors[] = 'La demande refinanceur #' . $id_demande_refin . ' n\'existe plus';
			} else {
				$values = $df->getCalcValues(true, $errors);

				if (!count($errors)) {
					if (!(int) $values['periodicity']) {
						$errors[] = 'Périodicité non définie';
					}

					if (!(int) $values['nb_mois']) {
						$errors[] = 'Durée totale non définie';
					}

					if (!(float) $values['loyer_evo_mensuel']) {
						$errors[] = 'Loyer mensuel (form. évolutive) non défini';
					}

					if (!(float) $values['loyer_dyn_mensuel']) {
						$errors[] = 'Loyer mensuel (form. dynamique) non défini';
					}

					if (!(float) $values['loyer_dyn_suppl_mensuel']) {
	...
	public function getDemandesRefinanceursArray($active_only = true)
	{
		$demandes = array();
		...
		foreach ($this->getChildrenObjects('demandes_refinanceurs', $filters, 'status', 'DESC') as $df) {
			$label = $df->displayRefinanceur('nom');
			$label .= ' (' . $df->getData('qty') . ' ';
			if ((float) $df->getData('qty') > 1) {
				$label .= BF_DemandeRefinanceur::$period_label_plur[$df->getData('periodicity')];
			} else {
				$label .= BF_DemandeRefinanceur::$period_label[$df->getData('periodicity')];
			}
			$label .= ' - ' . $df->displayData('status', 'default', false, true) . ')';

			$demandes[$df->id] = $label;
		}
	...
	public function getDevisFormulesArray($allowed_only = true)
	{
		$formules = array(
			'evo' => 'Formule évolutive',
			'dyn' => 'Formule dynamique'
		...
		if ($allowed_only && (int) $this->getData('duration') > 36) {
			unset($formules['dyn']);
		}
	...
	public function getSelectFormulesArray($allowed_only = true)
	{
		$formules = static::$formules;
	...
	public function addNotificationNote($content, $type_author = BimpNote::BN_AUTHOR_USER, $email = '', $auto = 0, $visibility = BimpNote::BN_MEMBERS, $delete_on_view = 0)
	{
		...
		if ($id_group) {
		...
		} elseif ((int) $this->getData('id_user_resp')) {
			...
			$errors[] = 'Aucun destinataire pour les notifications défini';
		}
				...
				if ((int) $this->getData('id_main_source')) {
					...
					if (BimpObject::objectLoaded($source)) {
						...
						switch ($field_name) {
							...
							case 'client_siren':
								return BimpTools::getArrayValueFromPath($client_data, 'siren', '');

							case 'client_rcs':
								return BimpTools::getArrayValueFromPath($client_data, 'rcs', '');
	...
	public function getSelectedDemandeRefinanceurData($returned_field = 'id', $with_default = false)
	{
		BimpObject::loadClass('bimpfinancement', 'BF_DemandeRefinanceur');
		...
		if ($with_default) {
			$where = 'id_demande = ' . $this->id . ' AND status > 0 AND status < 20';
			return $this->db->getValue('bf_demande_refinanceur', $returned_field, $where, 'status', 'DESC');
		}
		return null;
	}
	...
	public function getCommandesFournisseurData()
	{
		...
		foreach ($lines as $line) {
			...
			if (is_array($line_comm)) {
				foreach ($line_comm as $id_cf => $qty) {
					$cf = BimpCache::getBimpObjectInstance('bimpcommercial', 'Bimp_CommandeFourn', (int) $id_cf);
					if (BimpObject::objectLoaded($cf)) {
						$id_fourn = (int) $cf->getData('fk_soc');

						if (!isset($commFourns[$id_fourn])) {
							$commFourns[$id_fourn] = array();
						}
						if (!isset($commFourns[$id_fourn][(int) $id_cf])) {
							$commFourns[$id_fourn][(int) $id_cf] = array(
								'comm'  => $cf,
								'lines' => array()
							);
						}
						$commFourns[$id_fourn][(int) $id_cf]['lines'][(int) $line->id] = array(
							'qty'  => (float) $qty,
							'line' => $line
						);
					}
	...
	public function getSourceClientData($data_name, $id_source = 'main')
	{
		$return = '';
		if ($id_source === 'main') {
			$id_source = (int) $this->getData('id_main_source');
		...
		if (BimpObject::objectLoaded($source)) {
			...
			switch ($data_name) {
				case 'email':
					if (isset($client_data['contact']['email'])) {
						$return = $client_data['contact']['email'];
					}
					if (!$return && isset($client_data['email'])) {
						$return = $client_data['email'];
					}
					break;

				case 'nom':
					if (isset($client_data['signataire'])) {
						...
						if ($nom) {
							$return .= ($return ? ' ' : '') . strtoupper($nom);
						}
					}

					if (!$return && isset($client_data['contact'])) {
						$prenom = BimpTools::getArrayValueFromPath($client_data, 'contact/prenom', '');
						$nom = BimpTools::getArrayValueFromPath($client_data, 'contact/nom', '');
					...
					if (!$return && isset($client_data['nom'])) {
						$return .= ucfirst($client_data['nom']);
					}
					break;
				...
				case 'phone':
					$return = BimpTools::getArrayValueFromPath($client_data, 'signataire/phone', '');
					break;
			}
		}
	...
	{
		$doc_status_list_save = self::$doc_status_list;
...
//				self::$doc_status_list[self::DOC_GENERATED]['label'] = 'A envoyer à ' . $this->displaySourceName();
				...
				if (($field == 'devis_status' && !(int) $this->getData('id_signature_devis')) ||
					($field == 'contrat_status' && !(int) $this->getData('id_signature_contrat'))) {
		...
		self::$doc_status_list = $doc_status_list_save;
		return $return;
	}
	...
	public function renderDemandesList($list_type, $list_title)
	{
		$list = new BC_ListTable($this, $list_type, 1, null, $list_title);
		...
		switch ($list_type) {
			...
			case 'complets':
				$list->addFieldFilterValue('status', BF_Demande::STATUS_ACCEPTED);
				$list->addFieldFilterValue('devis_status', BF_Demande::DOC_ACCEPTED);
				$list->addFieldFilterValue('contrat_status', BF_Demande::DOC_ACCEPTED);
				$list->addFieldFilterValue('pvr_status', BF_Demande::DOC_ACCEPTED);
				$list->addFieldFilterValue('or_fac', array(
					'or' => array(
						'id_facture_fin'   => 0,
						'id_facture_fourn' => 0
					)
			...
			case 'en_place':
				...
				$list->addFieldFilterValue('contrat_status', BF_Demande::DOC_ACCEPTED);
				$list->addFieldFilterValue('pvr_status', BF_Demande::DOC_ACCEPTED);
				$list->addFieldFilterValue('id_facture_fin', array(
					'operator' => '>',
					'value'    => 0
				));
				$list->addFieldFilterValue('id_facture_fourn', array(
					'operator' => '>',
					'value'    => 0
				));
				$list->addFieldFilterValue('or_fac_rev', array(
					'or' => array(
						'id_facture_cli_rev'   => 0,
						'id_facture_fourn_rev' => 0
					)
				));
				break;
			...
			case 'cession_finale':
				...
				$list->addFieldFilterValue('id_facture_cli_rev', array(
					'operator' => '>',
					'value'    => 0
				));
				$list->addFieldFilterValue('id_facture_fourn_rev', array(
					'operator' => '>',
					'value'    => 0
				));
				break;
...
//        if (!(int) $this->getData('id_main_source')) {
		...
		if (BimpObject::objectLoaded($signature_contrat)) {
			if (!$signature_contrat->isSigned()) {
				$html .= '<div style="margin-top: 10px">';
				$msg = BimpRender::renderIcon('fas_exclamation-triangle', 'iconLeft');
				$msg .= '<a href="' . $signature_contrat->getUrl() . '" target="_blank">Signature du contrat de location en attente' . BimpRender::renderIcon('fas_external-link-alt', 'iconRight') . '</a>';
...
//        }
			...
			foreach (array(1, 0) as $signed) {
				...
				if (file_exists($dir . $file_name)) {
					...
					if ($url) {
						...
						if ($doc_type === 'contrat' && $signed) {
							$signature = $this->getChildObject('signature_contrat');
							if (BimpObject::objectLoaded($signature)) {
								$ds_certif_url = $signature->getCertifDocusignUrl();
								if ($ds_certif_url) {
									$docs_buttons[] = array(
										'label'   => 'Certificat DocuSign',
										'icon'    => 'fas_file-pdf',
										'onclick' => 'window.open(\'' . $ds_certif_url . '\');'
									);
								}
							}
						}
	...
	public function renderHeaderStatusExtra()
	{
		...
		if ($this->isClosed()) {
			$html .= '<span class="danger">' . BimpRender::renderIcon('fas_times', 'iconLeft');
			if ((int) $this->getData('status') > 0) {
				$html .= 'Cédé au financeur';
			} else {
				$html .= 'Demande fermée';
			}
			$html .= '</span>';
...
//                            }
	...
	public function generateDocument($doc_type, $data = array(), &$warnings = array(), &$success = '', &$file_name = '')
	{
		$errors = array();
		...
		$id_demande_refin = BimpTools::getArrayValueFromPath($data, 'id_df', 0);

		if ($this->isDemandeValid($errors, $id_demande_refin)) {
			$options = array();
			...
			$source = $this->getSource();
			if (BimpObject::objectLoaded($source)) {
				$data['devis_source_label'] = 'Selon ' . lcfirst($source->displayOrigine(true, false, true));
			} else {
				$data['devis_source_label'] = 'Selon le devis ' . $this->getData('ref_devis_source');
			}
			...
			if (isset($data['no_footer']) && (int) $data['no_footer']) {
				$pdf->add_footer = false;
			}
...
//							}
					...
					$api->sendDocFinancement($this->id, $type_origine, $id_origine, $doc_type, $docs_content, $signature_params, $signataires_data, 0, $req_errors);
	...
	public function createFactureFournisseur($id_fourn = 0, $id_mode_reglement = 0, $id_cond_reglement = 0, $ref_supplier = '', $libelle = '', &$warnings = array())
	{
		...
		if ((int) $this->getData('id_main_source')) {
			...
			if (!$id_fourn) {
				...
				if (!$id_fourn) {
					$errors[] = 'Paramètre de configuration "ID Fournisseur ' . $source->displayTypeSource() . '" non défini';
				}
			}
		}
	...
	public function actionGenerateDevisFinancement($data, &$success)
	{
		...
		$file_name = '';
		$errors = $this->generateDocument('devis', $data, $warnings, $success, $file_name);
...
//            $file_name = $this->getSignatureDocFileName('devis');
	...
	public function actionGenerateContratFinancement($data, &$success)
	{
		...
		} elseif ($type_pdf != $this->getData('type_contrat')) {
			$this->updateField('type_contrat', $type_pdf);
		}
			...
			if ($this->isDemandeValid($errors)) {
				...
				switch ($type_pdf) {
					case 'papier':
						...
						// PVR :
						$pvr_result = $this->actionGeneratePVReception(array(
							'no_footer'           => true,
							'client_is_company'   => $is_company,
							'client_representant' => $client_data['representant'],
							'client_repr_qualite' => $client_data['repr_qualite'],
							'loueur_nom'          => $loueur_data['nom'],
							'loueur_qualite'      => $loueur_data['qualite']
						));

						if (count($pvr_result['errors'])) {
							$errors[] = BimpTools::getMsgFromArray($pvr_result['errors'], 'Echec de la création du PV de réception');
						} else {
							...
							$pdf = new ContratFinancementPDF($db, $this, $client_data, $loueur_data, $cessionnaire_data, 'papier', $files_dir . $pvr_result['file_name']);
...
//									$files_dir . $pvr_result['file_name'],
									$annexe_file_path
								), 'F');
							}
	...
	public function actionGeneratePropositionLocation($data, &$success)
	{
		...
		$pdf_errors = array();
		$file_name = self::createPropositionPDF($data, $pdf_errors);
...
//        $use_db_transactions = (int) BimpCore::getConf('use_db_transactions', null);
		...
		$date_loyer = BimpTools::getArrayValueFromPath($data, 'date_loyer', $this->getData('date_loyer'));
		if (!$date_loyer) {
			$errors[] = 'Date de mise en loyer non spécifiée';
		} elseif ($date_loyer != $this->getData('date_loyer')) {
			$this->updateField('date_loyer', $date_loyer);
		}
...
//        $use_db_transactions = (int) BimpCore::getConf('use_db_transactions', null);
	...
	public function validate()
	{
		...
		if ($this->getData('formule') == 'dyn' && (int) $this->getData('duration') > 36) {
			$errors[] = 'il n\'est pas possible de sélectionner la formule dynamique pour une durée supérieure à 36 mois. Veuillez corriger';
		}

		$date_fin = $this->getData('date_fin');

		// Si date fin non éditée manuellement et date de mise en loyer ou durée modifiée
		if ($date_fin == $this->getInitData('date_fin')) {
			...
			if ($date_loyer && $duration &&
				...
				$this->set('date_fin', $dt->format('Y-m-d'));
			}
		}
	...
	{
		if ($doc_type == 'contrat' && $this->getData('type_contrat') != 'elec') {
			$errors[] = 'Contrat généré au format papier';
			return 0;
		}
	...
	public function createSignature($doc_type, $data, &$warnings = array(), &$success = '')
	{
		...
		if ($this->isLoaded($errors)) {
			...
			if ($has_source) {
				$contact_email = BimpTools::getArrayValueFromPath($data, 'contact_email', (int) $this->getSourceClientData('email'));
				$contact_nom = BimpTools::getArrayValueFromPath($data, 'contact_nom', (int) $this->getSourceClientData('nom'));
				$contact_fonction = BimpTools::getArrayValueFromPath($data, 'contact_fonction', (int) $this->getSourceClientData('fonction'));
				$contact_phone = BimpTools::getArrayValueFromPath($data, 'contact_phone', (int) $this->getSourceClientData('phone'));
			} else {
			...
			switch ($signature_type) {
				case 'docusign':
					if (!$allow_docusign) {
						$errors[] = 'Signature via DocuSign non autorisée pour les ' . $doc_type . 's de location';
					}
					break;

				case 'elec':
					if (!$allow_dist) {
						$errors[] = 'Signature électronique à distance non autoriée pour les ' . $doc_type . 's de location';
					}
					break;
			}
			...
			if (!count($errors)) {
				...
				if (!$has_source && !$id_client) {
					...
					if (!count($errors) && BimpObject::objectLoaded($signature)) {
						...
						if ($has_source) {
							$signataire_data['type'] = BimpSignataire::TYPE_CUSTOM;
							$signataire_data['email'] = $contact_email;
							$signataire_data['nom'] = $contact_nom;
							$signataire_data['fonction'] = $contact_fonction;
							$signataire_data['phone'] = $contact_phone;
						} else {
							$signataire_data['type'] = BimpSignataire::TYPE_CLIENT;
							$signataire_data['id_client'] = $id_client;
							$signataire_data['id_contact'] = $id_contact;
						}

						$signataire_locataire = BimpObject::createBimpObject('bimpcore', 'BimpSignataire', $signataire_data, true, $signataire_errors, $warnings);
...
//									break;
				...
				if ($this->hasSource()) {
					...
					foreach ($sources as $source) {
						...
						if (!count($req_errors)) {
							...
							$api->sendDocFinancement($this->id, $type_origine, $id_origine, $doc_type, $docs_content, array(), array(), 1, $req_errors);
						}
	...
	public function onDocuSignRecipientSigned(BimpSignature $signature, BimpSignataire $signataire)
	{
		$errors = array();

		if ($this->isLoaded($errors)) {
			$doc_type = $signature->getData('doc_type');

			if ($doc_type == 'contrat') {
				$signataire_label = $signataire->getData('label');

				$msg = 'Signature DocuSign effectué par le ' . strtolower($signataire_label);
				$this->addNotificationNote($msg);
			}
		}
