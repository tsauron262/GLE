...
$requete = "SELECT ".MAIN_DB_PREFIX."fichinter.rowid," .
        ...
        "          concat_ws('/',day(".MAIN_DB_PREFIX."fichinter.datei),month(".MAIN_DB_PREFIX."fichinter.datei),year(".MAIN_DB_PREFIX."fichinter.datei)) as dateFi" .
        "     FROM ".MAIN_DB_PREFIX."fichinter ";
if ($_GET["societe_id"] && $_GET["societe_id"] > 0)
{
    $requete .= " WHERE ".MAIN_DB_PREFIX."fichinter.fk_soc = ".$_GET['societe_id'];
}
...
if ($resql)
{
    while ($res=$db->fetch_object($resql))
    {
        ...
        $fi = new Fichinter($db,$res->fk_soc);
        $fi->fetch($res->rowid);
        print "    <TD align='left'><A href='intervention_detail.php?intervention_id=".$res->rowid."'>".img_object($langs->trans("showIntervention"),"intervention")."&nbsp;" .$fi->ref."</A>";
        $soc = new Societe($db);
