				...
				if ($with_pa_infos) {
					if ((int) $this->getData('fk_product_fournisseur_price')) {
						...
						if (!$custom_pa_id_fourn) {
							$html .= '<span class="danger">Aucun fournisseur spécifié</span>';
						} else {
			...
			if (!empty($sub_lines)) {
				...
				foreach ($sub_lines as $line) {
					...
					if (!empty($data_correct)) {
						if ($this->db->update('contratdet', $data_correct, 'rowid = ' . $line->id) > 0) {
							$infos[] = BimpTools::getMsgFromArray($line_infos, 'Ligne n° ' . $line->getData('rang'));
						}
					}
				}
			}
				...
				if (!$qty) {
					...
					if (!$date_fin) {
						...
						$new_line = BimpObject::createBimpObject('bimpcontrat', 'BCT_ContratLine', array(
							...
							'buy_price_ht'                 => $pa_ht,
							'custom_pa_id_fourn'           => $custom_pa_id_fourn,
