...
class modequipement extends DolibarrModules
{
	...
	*/
	function __construct($db)
	{
		...
		$equipementArray = array('thirdparty:+equipement:Equipements:@equipement:/equipement/tabs/societe.php?id=__ID__',
		...
		// dictionnarys
		$this->dictionnaries = array('langs'=>array('equipement@equipement'),'tabname'=>array(MAIN_DB_PREFIX."c_equipement_etat",MAIN_DB_PREFIX."c_equipementevt_type"),
		...
		$this->export_fields_array[$r]=array('e.rowid'=>"EquipId",'e.ref'=>"EquipRef",'e.fk_product'=>"RefProduit",'e.numversion'=>"VersionNumber",'e.datec'=>"EquipDateCreation",'e.fk_statut'=>'EquipStatus',
		'e.description'=>"EquipNote",'e.dateo'=>"Dateo",'e.datee'=>"Datee",'e.fk_etatequipement'=>"EtatEquip",'e.numimmocompta'=>"NumImmoCompta",
		'e.fk_soc_fourn'=>"CompanyFournish",'e.fk_soc_client'=>"CompanyClient",'e.fk_facture_fourn'=>"RefFactFourn",'e.fk_facture'=>"RefFactClient",'e.fk_entrepot'=>"EntrepotStock",
		...
		$this->export_TypeFields_array[$r]=array('e.ref'=>"Text",'e.fk_product'=>"List:product:label",'e.numversion'=>"Text",'e.datec'=>"Date",'e.fk_statut'=>'Statut',
		'e.description'=>"Text",'e.dateo'=>"Date",'e.datee'=>"Date",'e.fk_etatequipement'=>"List:c_equipement_etat:libelle",'e.numimmocompta'=>"Text",
		'e.fk_soc_fourn'=>"List:societe:nom",'e.fk_soc_client'=>"List:societe:nom",'e.fk_facture_fourn'=>"List:facture_fourn:ref_ext",'e.fk_facture'=>"List:facture:facnumber",'e.fk_entrepot'=>"List:entrepot:label",
		'ee.datec'=>"Date",'ee.duree'=>"Duree",'ee.description'=>"Text",'ee.fk_equipementevt_type'=>"List:c_equipementevt_type:libelle",
		'ee.dateo'=>"Date",'ee.datee'=>"Date",'ee.fulldayevent'=>"Boolean",'ee.total_ht'=>"Number",
		'ee.fk_fichinter'=>'List:fichinter:ref','ee.fk_contrat'=>"List:contrat:ref",'ee.fk_expedition'=>"List:expedition:ref");

		$this->export_entities_array[$r]=array(
		'e.rowid'=>"equipement@equipement:Equipement",'e.ref'=>"equipement@equipement:Equipement",'e.fk_product'=>"product",'e.numversion'=>"equipement@equipement:Equipement",
		...
		'e.datee'=>"equipement@equipement:Equipement",'e.fk_etatequipement'=>"equipement@equipement:Equipement",'e.numimmocompta'=>"equipement@equipement:Equipement",
		'e.fk_soc_fourn'=>"company",'e.fk_soc_client'=>"company",'e.fk_facture_fourn'=>"invoice",'e.fk_facture'=>"invoice",'e.fk_entrepot'=>"stock",
		'ee.rowid'=>'equipement@equipement:Equipement','ee.datec'=>"equipement@equipement:Equipement",'ee.duree'=>"equipement@equipement:Equipement",
		'ee.description'=>"equipement@equipement:Equipement",'ee.fk_equipementevt_type'=>"equipement@equipement:Equipement",
		'ee.dateo'=>"equipement@equipement:Equipement",'ee.datee'=>"equipement@equipement:Equipement",
		'ee.fulldayevent'=>"equipement@equipement:Equipement",'ee.total_ht'=>"equipement@equipement:Equipement",
...
//		$this->export_sql_end[$r] .=' LEFT JOIN '.MAIN_DB_PREFIX.'societe as sfourn ON (e.fk_soc_fourn = sfourn.rowid)';
		...
		$sql="SELECT name, label, fieldrequired FROM ".MAIN_DB_PREFIX."extrafields WHERE elementtype = 'equipement' AND entity IN (0, ".$conf->entity.')';
...
//
		...
		$this->import_tables_array[$r]=array('ee'=>MAIN_DB_PREFIX.'equipementevt', 'extra'=>MAIN_DB_PREFIX.'equipementevt_extrafields');
		$this->import_fields_array[$r]=array(	'ee.fk_equipement'=>"RefEquipement",
		...
		// Add extra fields
		$import_extrafield_sample=array();
		$sql="SELECT name, label, fieldrequired FROM ".MAIN_DB_PREFIX."extrafields as extra WHERE elementtype = 'equipementevt' AND entity IN (0, ".$conf->entity.')';
		$resql=$this->db->query($sql);
		if ($resql)    // This can fail when class is used on old database (during migration for example)
		{
			while ($obj=$this->db->fetch_object($resql))
