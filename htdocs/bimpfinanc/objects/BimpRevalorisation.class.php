...
//				}
			...
			if ($data['status'] == 1 || $data['status'] == 2) {
				foreach ($data['id_objects'] as $nb => $idT) {
					...
					if (!$instance->isActionAllowed('process')) {
						$errors[] = ($nb + 1) . ' éme ligne séléctionnée : opération impossible';
					}
				}
				if (!count($errors)) {
					foreach ($data['id_objects'] as $nb => $idT) {
	...
	public function actionCheckAppleCareSerials($data, &$success = '')
	{
		...
		$nb_ok = 0;
		$id_facture = BimpTools::getArrayValueFromPath($data, 'id_facture');

		$errors = self::checkAppleCareSerials($nb_ok, $id_facture);

		if ($nb_ok) {
			$success = $nb_ok . ' équipement(s) attribué(s) avec succès';
		} else {
			$warnings[] = 'Aucun équipement attribué';
		}
	...
	public static function checkAppleCareSerials(&$nb_ok = 0, $id_facture = null)
	{
		$errors = array();
