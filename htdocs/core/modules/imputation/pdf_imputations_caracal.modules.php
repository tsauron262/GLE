 ...
 */
class pdf_imputations_caracal extends ModeleImputations {
    ...
    function pdf_imputations_caracal($db) {
        ...
        require_once(DOL_DOCUMENT_ROOT . "/includes/phpexcel/PHPExcel/Writer/Excel2007.php");
    }
...
//        //$outputlangs->setPhpLang();
        if ($conf->imputations->dir_output) {
            ...
            while ($res = $this->db->fetch_object($sql)) {
                ...
                $title = dol_trunc(utf8_decode($res->ref . " " . $res->title), 25, 'right');
                $docxl->getActiveSheet($iter)->setTitle($title);
