    ...
    protected function initfooter()
    {
        ...
        if ($this->footerCompany->capital) {
            ...
            if (is_numeric($captital) && $captital > 0) {
                $line2 .= ($line1 ? " au " : "") . $this->langs->transnoentities("CapitalOf", price($captital, 0, $this->langs, 0, 0, 0, $conf->currency));
            } else {
                $line2 .= ($line1 ? " au " : "") . $this->langs->transnoentities("CapitalOf", $this->footerCompany->capital, $this->langs);
            }
        }
...
//            }
    ...
    public function getTargetInfosHtml()
    {
        ...
        if (is_null($this->contact) || $this->contact->getFullName($langs) != $nomsoc) {
            $html .= $nomsoc . "<br/>";
            if (!is_null($this->contact) && is_object($this->object) && is_object($this->object->thirdparty) && $this->object->thirdparty->name_alias != "" && (!isset($htis->thirdparty->name_alias) || $this->thirdparty->name_alias != $this->object->thirdparty->name_alias)) {
