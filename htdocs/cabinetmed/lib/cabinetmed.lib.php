...
function addAlert($db, $type, $id, $value) {
    ...
    $result = $db->query("SELECT rowid from " . MAIN_DB_PREFIX . "cabinetmed_patient WHERE rowid =" . $id);
...
function dol_cm_strptime($text, $format) {
    ...
    if ($posday >= 0)
        $result['tm_mday'] = (int) substr($text, $posday, 2);
    if ($posmonth >= 0)
        $result['tm_month'] = ((int) substr($text, $posmonth, 2)) - 1;
    if ($posyear >= 0)
        $result['tm_year'] = ((int) substr($text, $posyear, 4)) - 1900;
    if ($result['tm_mday'] < 0)
        $result['tm_mday'] = '';
    if ($result['tm_month'] < 0)
        $result['tm_month'] = '';
    if ($result['tm_year'] < 0)
        $result['tm_year'] = '';
    //var_dump($result);
