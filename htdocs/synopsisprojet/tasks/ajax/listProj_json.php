...
if($searchOn='true')
{
    ...
    if ($searchField == "nom" || $_REQUEST['label']){
        $searchField = "p.title";
        $searchString = $_REQUEST['label'];
    }
...
while($row = $db->fetch_array($result,MYSQL_ASSOC))
{
    ...
    $responce->rows[$i]['cell']=array($row['id'],
                                      "<a href='".DOL_URL_ROOT."/synopsisprojet/card.php?id=".$row['id']."'>".$row['nom']."</a>",
                                      "<a href='".DOL_URL_ROOT."/synopsisprojet/card.php?id=".$row['id']."'>".$row['ref']."</a>",
                                      $row['dateo'],
                                      $proj->getLibStatut(4),
                                      $overallprogress,
                                      $soc->getNomUrl(1),
