...
//                $nbS = $this->getStockProd($idPr, $idEn);
    ...
    private function getEquipmentNonSerialisable(){
        ...
        $sql = $this->db->query("SELECT serial FROM `llx_be_equipment` be WHERE be.id_product > 0 AND be.id_product NOT IN (SELECT pe.fk_object FROM llx_product_extrafields pe WHERE pe.serialisable = 1)");
