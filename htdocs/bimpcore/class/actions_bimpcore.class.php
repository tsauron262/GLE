...
//                    $tabObj = array("bimpcore", "Bimp_Fournisseur");
			...
			if (stripos($url, '/synopsisdemandeinterv/') !== false || stripos($url, '/synopsisfichinter/') !== false || stripos($url, '/fichinter/') !== false) {
		...
		if (isset($tabObj) && stripos($url, 'ajax') === false) {
			$bObj = BimpCache::getBimpObjectInstance($tabObj[0], $tabObj[1], $_REQUEST[(isset($tabObj[2]) ? $tabObj[2] : 'id')]);
