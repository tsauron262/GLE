 ...
 */
class box_graph_imput extends ModeleBoxes
{
	...
	function loadBox($max=5)
	{
		...
		$text = $langs->trans($this->titre,$max);
		...
		if (! $user->rights->societe->client->voir || $socid) $prefix.='private-'.$user->id.'-';	// If user has no permission to see all, output dir is specific to user
                ...
                $prefix = "imputations-".$this->boxcode;
...
$sql = $this->db->query("SELECT * FROM `".MAIN_DB_PREFIX."synopsisprojet_stat` WHERE `type` LIKE '".$this->typeStat."' AND dateC >= '".$startyear."-01-01' ORDER BY `llx_synopsisprojet_stat`.`dateC` ASC");
for($i=0;$i<12;$i++){
    $ligne = $this->db->fetch_object($sql);
    $data2[$i] = array($data2[$i][0], $ligne->valeur/$this->coefValeur,$data2[$i][2]);
}
for($i=0;$i<12;$i++){
    $ligne = $this->db->fetch_object($sql);
    $data2[$i] = array($data2[$i][0], $data2[$i][1], $ligne->valeur/$this->coefValeur);
}
