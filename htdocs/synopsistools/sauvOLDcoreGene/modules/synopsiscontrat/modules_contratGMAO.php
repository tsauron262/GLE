 ...
 */
class ModeleContratGMAO extends CommonDocGenerator {
...
function contratGMAO_pdf_create($db, $id, $modele = '', $outputlangs = '') {
    ...
    if (!$modelisok) {
        if ($conf->global->CONTRAT_GMAO_ADDON_PDF)
            $modele = $conf->global->CONTRAT_GMAO_ADDON_PDF;
        $file = "pdf_contrat_" . $modele . ".modules.php";
...
//        $db->query($requete);
        ...
        if (!$conf->global->CONTRAT_GMAO_ADDON_PDF) {
            print $langs->trans("Error") . " " . $langs->trans("Error_CONTRAT_GMAO_ADDON_PDF_NotDefined");
        } else {
