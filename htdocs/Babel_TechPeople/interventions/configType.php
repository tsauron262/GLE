...
/*
  ...
  */
  ...
  {
      $requete = "SELECT * FROM ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv WHERE id = ".$_REQUEST['id'];
      $sql = $db->query($requete);
      $res = $db->fetch_object($sql);
      $oldRang = $res->rang;
      $newRang = $oldRang - 1;
      $requete = "UPDATE ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv SET rang = ".$oldRang . " WHERE rang = ".$newRang;
      $sql = $db->query($requete);
      $requete = "UPDATE ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv SET rang = ".$newRang . " WHERE id = ".$_REQUEST['id'];
      $sql = $db->query($requete);
  ...
  if ($_REQUEST['action']=='add')
  {
      ...
      $requete = "SELECT max(rang) + 1 as rg FROM ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv";
      $sql = $db->query($requete);
      $res = $db->fetch_object($sql);
      $max = $res->rg;
      $requete = "INSERT INTO ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv
      ...
      if ($default."x" != "x"){
            $requete = "UPDATE ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv SET `default` = NULL WHERE id !=".$newId;
            $sql=$db->query($requete);
      }
...
//                $isDefault = $_REQUEST['default-'.$id];
                ...
                $rang = 1;
                $requete = " SELECT max(rang) + 1 as mx FROM ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv ";
                $sql = $db->query($requete);
                $res = $db->fetch_object($sql);
                $newRang = ($res->mx > 0?$res->mx:$rang);
                //`default` =
                $requete = "UPDATE ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv
...
//                print $requete.";<br/>";
        ...
        if ($default > 0){
            $requete = "UPDATE ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv SET `default` = 1 WHERE id =".$default;
            $sql=$db->query($requete);
  //          print $requete.";<br/>";
        }
  }
...
EOF;
  ...
  $requete = "SELECT * FROM ".MAIN_DB_PREFIX."synopsisfichinter_c_typeInterv ORDER by rang ";
  $sql = $db->query($requete);
