...
//    $demandeInterv->fk_commande = $_POST["comLigneId"];
        ...
        if ($this->fk_contrat > 0) $sql .= ", fk_contrat";
        if ($this->date > 0) $sql.=  ", datei";
        $sql.= ") ";
        ...
        if ($this->fk_user_prisencharge > 0) $sql .= ", ".$this->fk_user_prisencharge;
        if ($this->fk_commande >0) $sql .= ", ".$this->fk_commande;
        if ($this->fk_contrat > 0) $sql .= ", ".$this->fk_contrat;
        if ($this->date > 0) $sql.= ", '".date('Y-m-d',$this->date)."'";
        $sql.= ")";
        $sqlok = 0;
    ...
    */
    function LibStatut($statut,$mode=0)
    {
        ...
        if ($mode == 2)
        {
            ...
            if ($statut==2) return img_picto($this->statuts_short[$statut],'statut3').' '.$this->statuts_short[$statut];
            if ($statut==3) return img_picto($this->statuts_short[$statut],'statut6').' '.$this->statuts_short[$statut];
        }
        if ($mode == 3)
        {
            ...
            if ($statut==2) return img_picto($this->statuts_short[$statut],'statut3');
            if ($statut==3) return img_picto($this->statuts_short[$statut],'statut6');
        }
        if ($mode == 4)
        {
            ...
            if ($statut==2) return img_picto($this->statuts_short[$statut],'statut3').' '.$this->statuts[$statut];
            if ($statut==3) return img_picto($this->statuts_short[$statut],'statut6').' '.$this->statuts[$statut];
        }
        if ($mode == 5)
        {
            ...
            if ($statut==2) return $this->statuts_short[$statut].' '.img_picto($this->statuts_short[$statut],'statut3');
            if ($statut==3) return $this->statuts_short[$statut].' '.img_picto($this->statuts_short[$statut],'statut6');
        }
    }
    ...
    */
    function addline($demandeIntervid,
                     $desc,
                     $date_intervention,
                     $duration,
                     $typeinterv,
 ...
 //               var_dump($demandeIntervligne);
    ...
    function getNomUrl($withpicto=0,$option=0)
    {
        ...
        if ($option == 6)
            $lien = '<a href="'.GLE_FULL_ROOT.$urlOption.'/Synopsis_DemandeInterv/fiche.php?id='.$this->id.'">';
        $lienfin='</a>';
        ...
        if ($withpicto && $option==6) $result.=($lien.img_object($label,$picto,false,false,false,true).$lienfin);
    ...
    */
    function insert()
    {
        ...
        if (!$rangToUse >0)
        {
            ...
            $sql = 'SELECT * FROM '.MAIN_DB_PREFIX.'Synopsis_demandeIntervdet';
...
//Compute total tva / total_TTC
        ...
        if ($this->fk_commandedet > 0) $sql .= ',fk_commandedet ';
        if ($this->fk_contratdet > 0) $sql .= ',fk_contratdet ';
        $sql.= ')';
        ...
        if ($this->qte > 0) $sql .= ' ,'.preg_replace('/,/','.',$this->qte);
        if ($this->pu_ht > 0) $sql .= ",".preg_replace('/,/','.',$this->pu_ht).",".preg_replace('/,/','.',$this->total_ht).",".preg_replace('/,/','.',$total_tva).",".preg_replace('/,/','.',$total_ttc);
        if ($this->fk_commandedet > 0) $sql .= ', '.$this->fk_commandedet;
        if ($this->fk_contratdet > 0) $sql .= ', '.$this->fk_contratdet;
...
//print "toto".$this->total_ht;
        ...
        if ($this->qte."x"!="x") $sql .= ",qte = ".$this->qte;
        if ($this->pu_ht."x"!="x") $sql .= ",pu_ht = ".preg_replace('/,/','.',$this->pu_ht);
        if ($this->pu_ht."x"!="x") $sql .= ",total_ht = ".preg_replace('/,/','.',$this->total_ht);
        if ($this->pu_ht."x"!="x") $sql .= ",total_tva = ".preg_replace('/,/','.',$total_tva);
        if ($this->pu_ht."x"!="x") $sql .= ",total_ttc = ".preg_replace('/,/','.',$total_ttc);
        if ($this->isForfait."x"!="x") $sql .= ",isForfait = ".$this->isForfait;
        if ($this->comLigneId > 0) $sql .= ",fk_commandedet =  ".$this->comLigneId;
        else $sql .= ",fk_commandedet =  NULL";

        if ($this->fk_contratdet > 0) $sql .= ",fk_contratdet =  ".$this->fk_contratdet;
...
//        $sql.= " WHERE fk_demandeInterv=".$this->fk_demandeInterv;
        $result = 1;
                ...
                if ($result < 0) { $error++; $this->errors=$interface->errors; } else { $result = 1;}
                // Fin appel triggers
            return($result);
