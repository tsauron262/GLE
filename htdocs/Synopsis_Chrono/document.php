...
//        if ($result < 0) { $error++; /*$this->errors=$interface->errors;*/ }
            ...
            if ($result < 0) {
                ...
                // Echec transfert (fichier depassant la limite ?)
                $mesg = '<div class="error ui-state-error">' . $langs->trans($result) . '</div>';
                // print_r($_FILES);
            } else {
...
if ($id > 0) {
    ...
    if ($chrono->fetch($id)) {
        ...
        if ($_GET['action'] == 'delete') {
            $html->form_confirm($_SERVER["PHP_SELF"] . '?id=' . $_GET["id"] . '&amp;urlfile=' . urlencode($_GET["urlfile"]), $langs->trans('DeleteFile'), $langs->trans('ConfirmDeleteFile'), 'confirm_deletefile');
            print '<br>';
        }
