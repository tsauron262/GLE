...
//        }
	...
	public function setSignedDocuSign($date_signed = '', $check_signature_status = true)
	{
		$errors = array();
		...
		if ((int) $this->getData('status') === BimpSignataire::STATUS_SIGNED) {
			return array();
		}
		...
		$errors = $this->validateArray(array(
			'status'         => self::STATUS_SIGNED,
			'date_signed'    => $date_signed,
			'type_signature' => self::TYPE_DOCUSIGN
		));
		...
		if (!count($errors)) {
			...
			if (!count($errors)) {
				...
				if (!BimpObject::objectLoaded($signature)) {
					$errors[] = 'Signature parente absente';
				} else {
					if ($check_signature_status) {
						$signature->checkStatus();
					}
					...
					if ($signature->isObjectValid($errors, $obj)) {
						$msg = 'Signature électronique complétée pour le signataire "' . $this->getName() . '" pour le document "' . $signature->displayDocTitle() . '"';
						$obj->addObjectLog($msg, 'SIGNATURE_' . strtoupper($signature->getData('doc_type')) . '_DOCUSIGN_COMPLETED_FOR_' . strtoupper($this->getData('code')));

						if (method_exists($obj, 'onDocuSignRecipientSigned')) {
							$obj->onDocuSignRecipientSigned($signature, $this);
						}
					}
				}
			}
