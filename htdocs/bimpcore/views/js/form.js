...
//        }
	...
	$container.find('select.object_child_select').each(function () {
		if (!parseInt($(this).data('object_field_select_events_init'))) {
			$(this).data('object_field_select_events_init', 1);

			$(this).change(function () {
				reloadObjectFieldSelectInput($(this).findParentByClass('object_field_select_container'), $(this));
			});
		}
	});
}
...
function reloadObjectFieldSelectInput($container, $modified_input, new_value) {
	if (!$.isOk($container)) {
		return;
	}

	var $input = $container.find('.object_field_input');
	if (!$.isOk($input)) {
		return;
	}
	...
	if (typeof (new_value) !== 'undefined') {
		value = new_value;
	} else if ($.isOk($modified_input)) {
		value = $modified_input.val();
	} else {
		value = $container.find('.object_field_select').val();
	}

	$input.attr('name');

	BimpAjax('loadObjectFieldSelectInput', {
		'module': $container.data('module'),
		'object_name': $container.data('object_name'),
...
function addInputObjectFieldReference($button) {
	if (!$.isOk($button)) {
		return;
	}
	...
	var $container = $button.findParentByClass('add_object_field_reference_form');
	if ($.isOk($container)) {
		var val = '';
		var $select = $container.find('select.object_field_input');
		if ($select.length) {
			val = $select.val();
		}

		if (!val) {
			bimp_msg('Aucun champ sélectionné', 'danger', null, true);
		} else {
			var $inputContainer = $container.findParentByClass('inputContainer');
			if ($.isOk($inputContainer)) {
				var field_name = $inputContainer.data('field_name');
				if (field_name) {
					var $input = $inputContainer.find('[name="' + field_name + '"]');
					if ($input.length) {
						insertInputText($input, '__' + val + '__ ');
						$select.val('').change();
					}
				}
			}
...
function insertInputText($input, text) {
	var $cke = $input.parent().find('.cke');
	if ($cke.length) {
		var $btn = $cke.find('.cke_button__source');
		if ($btn.length && $btn.hasClass('cke_button_off')) {
			$input = $cke.find('iframe').contents().find('body');
		} else {
			$input = $cke.find('textarea.cke_source');
		}
	}

	var inputTag = $input.tagName();

	if (inputTag === 'input' || inputTag === 'textarea') {
		var element = $input.get(0);
		var position = element.selectionStart;
		var value = $input.val();
		var debut = value.substring(0, position);
		var fin = value.substring(position);
		var new_val = debut + text + fin;

		$input.val(new_val);
		element.selectionStart = element.selectionEnd = position + text.length;
	} else {
		value = $input.html();
		$input.html(value + text);
	}
}
