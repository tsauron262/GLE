<?php
    ...
    class export {
        ...
        public function exportFacture($ref = ""):void {
            global $db;
            $errors = [];
            $list = $this->bdb->getRows('facture', 'exported = 0 AND fk_statut IN(1,2) AND type != 3 AND datef > "2023-01-01" AND datef < "2023-12-08" AND (datef >= "'.$this->lastDateExported->format('Y-m-d').'")'.$this->getEntityFilter());
        ...
        public function exportPaiement($ref = '', $want = Array()):void  {
            ...
            $list = $this->bdb->getRows('paiement', 'exported = 0 AND datef < "2023-12-08" AND datep >= "'.$this->lastDateExported->format('Y-m-d').' 00:00:00"'.$this->getEntityFilter());
