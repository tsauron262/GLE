...
//        $sql = $dbT->query($req);
    ...
    foreach($result as $serv => $datasServ){
        echo '<h1>Consul : '.$serv.'</h1>';
        foreach($datasServ as $servSql => $dataSql){
            echo '<h2>Mysql : '.$servSql.'</h2>';
