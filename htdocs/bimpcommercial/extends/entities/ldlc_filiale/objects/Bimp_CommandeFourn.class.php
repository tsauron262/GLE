								...
								if ($commFourn->find(['ref' => $ref])) {
									...
									if (isset($data->Stream->Order->Parcels)) {
										...
										foreach ($parcellesBrut['Parcel'] as $parcel) {
											...
											if (!$noteOK) {
												$id_groupe = BimpCore::getUserGroupId('achat');
												if ($id_groupe) {
													$commFourn->addNote($text, null, 0, 1, '', 1, BimpNote::BN_DEST_GROUP,  $id_groupe);
												} else {
...
//        $errors = BimpTools::merge_array($errors, $this->verifMajLdlc($data, $success));
		...
		if (!count($errors)) {
			...
			global $mysoc;
			$adresseFact = array(
				"tag"      => "Address", "attrs" => array("type" => "billing"),
				"children" => array(
...
//                $name = "BIMP";
			$adresseLiv = array(
				"tag"      => "Address", "attrs" => array("type" => "shipping"),
				"children" => array(
...
//                $shipping_mode = "PNS6";
			$tab = array(
				array(
					"tag"      => "Stream", "attrs" => array("type" => "order", 'version' => "1.0"),
					"children" => array(
						array(
							"tag"      => "Order", "attrs" => array("date" => date("Y-m-d H:i:s"), 'reference' => $this->getData('ref'), "external_identifier" => $this->getData('ref'), "currency" => "EUR", "source" => $this->code_representant, "shipping_vat_on" => $portHt, "shipping_vat_off" => $portTtc, "shipping_mode" => $shipping_mode),
							"children" => array(
								array(
									"tag"      => "Customer", "attrs" => array("identifiedby" => "code", 'linked_entity_code' => "PRO"),
									"children" => array(
										"Owner"          => "FILI",
