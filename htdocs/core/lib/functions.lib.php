...
function get_localtax($tva, $local, $thirdparty_buyer = "", $thirdparty_seller = "") {
    ...
    if ($resql) {
        ...
        if ($local == 1 && $obj->localtax1_type != '7')
            return $obj->localtax1;
        elseif ($local == 2 && $obj->localtax2_type != '7')
...
function getTypeOfLocalTaxFromRate($vatrate, $number, $thirdparty) {
    ...
    if ($resql) {
        $obj = $db->fetch_object($resql);
        if ($number == 1)
            return array($obj->localtax1_type, $obj->localtax1);
        elseif ($number == 2)
            return array($obj->localtax2_type, $obj->localtax2);
    }
