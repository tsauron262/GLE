...
//                        }
    ...
    public function getDurreeVendu(&$ratio = 1)
    {
        $tot = 0;
		...
		$ratio = $this->getRatioWithAvProlongation();
        $tot = $tot * $ratio;
        return $tot;
    }
    ...
    public function getTotalPa($line_type = -1)//-1 tous 3 que les lignes avec duree 4 que les lignes sans duree
    {
        $total_PA = 0;
...
//        $html .= $objects->renderList('contrat');
    ...
    public function renderThisStatsFi($display = true, $in_contrat = true)
    {
		...
		$ratio_avenants = 1;
        $duree_vendu = $this->getDurreeVendu($ratio_avenants);
        if ($duree_vendu) {
            //        $fis = $this->getListFi();
            ...
            $htmlT .= "Nombre d'heures vendue : " . $ficheInter->timestamp_to_time($duree_vendu) . ($ratio_avenants != 1 ? ' (ratio avenants : ' . $ratio_avenants . ')' : '') . '<br/>';
//            $html .= "Nombre d'heures de délégation vendue : ".print_r($this->getTotalHeureDelegation(true),true).'<br/>';
        ...
        $marge = $total_contrat - $cout;
        $marge_previsionelle = $total_contrat - $coutPrevi;
        if ($marge > 0) {
            $class = 'success';
            $icone = 'arrow-up';
        } elseif ($marge < 0) {
