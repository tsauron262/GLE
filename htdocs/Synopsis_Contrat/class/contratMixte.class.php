...
class contratMixte extends Synopsis_Contrat {
    ...
    public function display1LineTKT($val) {
        ...
        if ($this->lineTkt[$val->id]['fk_contrat_prod']) {
            ...
            if ($tmpcontratProd->qte == -1)
                $qteInit = 'Illimit&eacute;';
...
//            $this->sumTkt[$res->rowid]=array('total'=>$res->total_ht , 'tva'=> $res->total_tva,'ttc' => $res->total_ttc, 'qty' => $res->qty, 'pu_ht' => $res->price_ht);
        if ($qteInit == 0) {
            ...
            if ($tickets . 'x' != "x") {
                ...
                if ($tickets == -1)
                    $html .= 'Tickets&nbsp;:&nbsp;Illimit&eacute; <br/>(' . $consomme . ' consomm&eacute;es)';
                if ($tickets > 0) {
                    global $conf;
                    if ($restant < 0) {
        ...
        if ($tickets . 'x' != "x") {
            ...
            if ($tickets == -1)
                $tt[] = '<b>Tickets&nbsp;:&nbsp;</b>Illimit&eacute; <br/>(' . $consomme . ' consomm&eacute;es)';
            if ($tickets > 0)
                $tt[] = '<b>Tickets&nbsp;:&nbsp;</b>' . $tickets . " (" . $restant . " restants)";
        }
...
EOF;
        ...
        $requete = "SELECT count(rowid) as sDI FROM " . MAIN_DB_PREFIX . "synopsisdemandeinterv WHERE fk_contrat = " . $this->id;
        $sql = $this->db->query($requete);
        $res = $this->db->fetch_object($sql);
    ...
    public function sumMnt() {
        ...
        $requete = "SELECT min(datei) as mini, max(datei) as maxi FROM " . MAIN_DB_PREFIX . "synopsisdemandeinterv WHERE fk_contrat = " . $this->id . " GROUP BY fk_contrat";
        $sql = $this->db->query($requete);
        $res = $this->db->fetch_object($sql);
