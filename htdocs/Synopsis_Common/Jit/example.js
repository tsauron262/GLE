    ...
    $.getJSON(jsonUrl,function(data){
        ...
        var getTree = (function() {
            ...
            return function(nodeId, level) {
                var myJSONText = JSON.stringify(json, function (key, value) {
                                    return value;
                                });
            var subtree = myJSONText.replace(/id:"([a-zA-Z0-9]+)"/g,
                function(all, match) {
                    return "id:" + match + "_" + i;
                }
            );
...
// ST.Geom.implement({
//       'treeFitsInCanvas': function() {
//               return true;
//       }
//   });

        var infovis = document.getElementById('infovis');
        var w = infovis.offsetWidth + 1100 , h = infovis.offsetHeight + 1100;
        //init canvas
        ...
        //Create a new ST instance
         st = new ST(canvas, {
            ...
            onBeforeCompute: function(node){
                Log.write("Chargement de " + node.name);
            },
            ...
            onCreateLabel: function(label, node){
                label.id = node.id;
                var suffix="";
                if (node.name.charAt(20) +"x" != "x")
                {

                    if (node.name.charAt(16) +"x" != "x")
                    {
                        suffix="...";
                    }
                    var tmpCnt = node.name.length - 5;
                    label.innerHTML = node.name.substr(0,10)+suffix;
                    if (node.name.charAt(16) + "x" != "x") {
                        label.innerHTML += node.name.substr(tmpCnt, 5);
                    }
                } else {
                ...
                label.ondblclick = function(){
                    var tmp = node.id;
                    //nodeid au format : p[prodid]-c[catid]
                    var prodid = tmp.replace(/[\w](\d*)[\W][\w]\d*/gi,"$1");
                        prodid = prodid.replace(/[A-Za-z]/g, "");
                ...
                label.onmouseover = function(){
                    ...
                    {
                        $("<div id ='someid'; class='node' style='position: absolute; width: 220px; background: #C0C0F0; z-index: 2000000; height: 200%;  margin-top: 10px; color: #FFFFFF; overflow-y: hidden;'>Cat&eacute;gorie: "+node.name+"</div>").appendTo($(label));
                    } else {
                        $("<div id ='someid'; class='node' style='position: absolute; width: 220px; background: #C0C0F0; z-index: 2000000; height: 200%;  margin-top: 10px; color: #FFFFFF; overflow-y: hidden;'>"+node.name+"</div>").appendTo($(label));
                    }
                };
                ...
                if (node.id.match(new RegExp('^[c]','g')))
                    label.className = "node1";
            },
        ...
        st.select(st.root);
        //end
        document.getElementById("mycanvas").scrollTop=575;
        document.getElementById("mycanvas").scrollLeft=850;
...
//        left.onchange = changeHandler;
//        bottom.onchange = changeHandler;
//        right.onchange = changeHandler;
    //end
    });
