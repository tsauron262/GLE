...
class BimpApiRequest
{
    ...
    public function __construct($api, $request_name)
    {
        ...
        if (!file_exists($dir . '/' . $file_name)) {
            $this->addError('Erreur: le fichier "' . $file_name . '" n\'existe pas.', true, Bimp_Log::BIMP_LOG_URGENT);
        } else {
            ...
            while (true) {
                ...
                if (!(int) $params['edit']) {
                    ...
                    switch ($params['type']) {
                        ...
                        case 'money':
                            $html .= BimpInput::renderInput('money', $inputName, $inputValue, array());
                            break;
    ...
    public function generateRequestFormHtml($values, $extra_data = array())
    {
        ...
        $html .= '<form id="api' . $this->api->name . '_form_' . $this->requestName . '" class="request_form" enctype="multipart/form-data">';
...
//        }
    ...
    public function processRequestDatas($fields, $fieldIndex = null, $fieldPrefix = '')
    {
        $datas = array();
        foreach ($fields as $field_name => $field_params) {
                ...
                if (!empty($subFields)) {
                    if ((int) $params['multiple']) {
                        ...
                        if ($nextIdx) {
                            $array = array();
                            for ($i = 1; $i < $nextIdx; $i++) {
                                $result = $this->processRequestDatas($subFields, $i, $inputName);
                                if (!empty($result)) {
                                    $array[] = $result;
                                }
                            }
                        ...
                        $result = $this->processRequestDatas($subFields, null, $inputName);
                        if (!empty($result)) {
    ...
    public function processRequestForm()
    {
        ...
        $fields = $this->config->getCompiledParams('fields');
        $requestDatas = $this->processRequestDatas($fields);
