...
//log: keysearchp_idprodmod=&p_idprodmod=697&debut_loc=20%2F12%2F2010&fin_loc=30%2F12%2F2010&pu_ht=1+000%2C00&duration_value=&duration_unit=m&qte=1
        ...
        if ($fk_product > 0 && $idLigne > 0)
        {
            ...
            if ($res)
            {
                ...
                //Update serial
                $requete = "SELECT * FROM Babel_product_serial_cont WHERE element_id = ".$idLigne." AND element_type='contratLoc'";
                $sql = $db->query($requete);
                if ($db->num_rows($sql) > 0)
                {
                    $res = $db->fetch_object($sql);
                    $requete = "UPDATE Babel_product_serial_cont SET serial_number='".addslashes($serial)."' WHERE id = ".$res->id;
                    $sql = $db->query($requete);
                } else {
                    $requete = "INSERT INTO Babel_product_serial_cont
...
// qte, produit, debut, fin, subprice subprice duration subprice duration unit
    ...
    case 'delLocline':
    {
        ...
        {
            $xml .= "<OK>OK</OK>";
            $requete = "DELETE FROM Babel_product_serial_cont WHERE element_id = ".$_REQUEST['lineId'];
            $sql = $db->query($requete);
        } else {
            $xml .= "<KO><![CDATA[".$db->lastqueryerror . "\n ". $db->lasterror ."]]></KO>";
        }
