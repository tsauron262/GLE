    ...
    //TODO droit ?
        if ($user->rights->contrat->creer)
        {
            if ( $_REQUEST["p_idprod"] > 0)
            {
                ...
                $date_start='';
                $date_end='';
//Date start et Date End du contrat
                $requete = " SELECT unix_timstamp(mise_en_service) as dates, unix_timestamp(fin_validite) as datef FROM ".MAIN_DB_PREFIX."contrat WHERE rowid = ".$_REQUEST['id'];
                $sql = $db->query($requete);
                $res = $db->fetch_object($sql);


                $date_start = $res->dates;
                $date_end = $res->datef;
                $price_base_type = 'HT';
                $pu_ht=0;
                $pu_ttc=0;
...
//                    $mesg='<div class="error ui-state-error">'.$contrat->error.'</div>';
                    $xml .= '<KO><![CDATA['.$result." ".$db->lastqueryerror() ."\n".$db->lasterror().'\n'.$db->lasterrno.'\n'.$db->errno.'\n'.$db->error.']]></KO>';
                }
            }
                ...
                if ($result > 0)
                {
                    ...
                    $requete="UPDATE ".MAIN_DB_PREFIX."Synopsis_contratdet_GMAO
                                 SET durValid = ".$durVal.",
                                     ...
                                     isSAV = 1,
                                     SLA = '".$sla."',
                                     fk_contrat_prod = ".($_REQUEST['fk_prod_contrat']>0?$_REQUEST['fk_prod_contrat']:"NULL")."
...
//print $requete;
            ...
            $xml .= "<serial_number>".$res2->serial_number."</serial_number>";
