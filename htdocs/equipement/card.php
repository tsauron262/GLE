...
if ($action == 'create') {
	...
	print $langs->trans("Product").'</td><td align=right>';
	print "<a href=# onclick=\"$('#descprod').toggle();\" >".img_picto("", "edit_add")."</a>";
	print '</td></tr></table></td><td colspan="3">';
	print $prod->getNomUrl(1)." : ".$prod->label.'</td></tr>';
	print "<tr style='display:none' id='descprod'>";
	...
	if ($action != 'editstock' && $object->statut == 0) {
		...
		img_edit($langs->trans('Modify'), 1).'</a></td>';
	}
	print '</tr></table></td><td colspan="3">';
	if ($action == 'editstock') {
	...
	if ($user->rights->facture->lire) {
		...
		if ($action != 'editfactclient' && $object->statut == 0) {
			print '<td align="right">';
			if ( $object->fk_soc_client > 0 ) 
				print '<a href="'.$_SERVER["PHP_SELF"].'?action=editfactclient&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'), 1).'</a></td>';
			else
	...
	if ($conf->global->EQUIPEMENT_USEDLUODATE == 1) {
		...
		if ($action != 'editdatee' && $object->statut == 0) 
			print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editdated&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'), 1).'</a></td>';
		print '</tr></table></td><td colspan="3">';
		if ($action == 'editdated') 	{
	...
	print $form->editfieldval("Description", 'description', $object->description, $object, $user->rights->equipement->creer, 'textarea:4:100');
	print '</td>';
	print '</tr>';
