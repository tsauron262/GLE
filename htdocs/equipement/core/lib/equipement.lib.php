	...
	if ($resql)
	{
		...
		if ($num)
		{
			...
			while ($i < $num)
			{
				...
				if ($nbexpedition==0) // si il y en assez à expédier, on affiche si on veux ventiler l'envoie selon les lots
		...
		if ($numlot)
		{
			...
			while ($i < $numlot)
			{
				...
				if ($objlot->nb > 0)
				{
					...
					$titlelot = " [".dol_print_date($db->jdate($objlot->dateo),'day')." - ".dol_print_date($db->jdate($objlot->datee),"day")."] ";
