...
$type = isset($_REQUEST['type'])? $_REQUEST['type'] : 43;
...
function execute_op_get_widgets_by_column() {
    ...
    $newTab = array();
    if (is_array($widgets))
        foreach ($widgets[0] as $val => $id)
            $newTab[$id] = $val;

    $lignes = array();
    foreach ($newTab as $id => $nom) {
        $lignes[] = '{
      "title" : "' . $nom . '",
      "id" : "' . $id . '",
      "column" : "first",
      "open" : true,
      "url" : "/Synopsis_Tools/dashboard2/ajaxData.php?op=get_widget&id=' . $nom . '"
    }';
    }
...
//  $return = $return['fullscreen'];
    $return .= ob_get_contents();
    $return = call_user_func("widget_".$id);
    ob_clean();
    $return = str_replace(array("box_titre", "liste_titre"), array("box_titre widgetheader", "liste_titre widgetheader"), $return);
    if(is_array($return))
        $return = afficheWidget($return);
    echo $return;
}


function afficheWidget($return){
    $html = '<table width="100%" class="noborder">
                <tbody>
                    <tr class="liste_titre">
                        <th colspan="3">'.$return['title'].'</th>
                    </tr>
                    <tr class="impair">
                        '.$return['content'].'
                    </tr>
...
function execute_op_getWidgets() {
    echo '{
"result" :
  {
  "layout": "layout2",
  "data" : [
    {
      ...
      "open" : true,
      "url" : "/Synopsis_Tools/dashboard2/ajaxData.php?op=get_widget&id=boxAction"
    },
    {
