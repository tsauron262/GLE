			...
			switch ($type) {
				...
				default:
					...
					if ((int) BimpTools::getArrayValueFromPath($this->field_params, 'hashtags', 0)) {
						$html = BimpObject::replaceHastags($html, $this->no_html);
					}

					if ((int) BimpTools::getArrayValueFromPath($this->field_params, 'field_path', 0)) {
						$replace_obj = $this->object;
						if (!empty($this->field_params['field_path_object'])) {
							$replace_obj = $this->object->getChildObject($this->field_params['field_path_object']);
						}

						if (is_a($replace_obj, 'BimpObject')) {
							$html = $replace_obj->replaceFieldsValues($html, $this->no_html);
						}
					}
					break;
					...
					if (!is_null($collection)) {
						switch ($type) {
							...
							case 'ref_nom':
								...
								if (!$this->no_html) {
									if ($this->params['card'] && !(int) BimpCore::getConf('mode_eco')) {
