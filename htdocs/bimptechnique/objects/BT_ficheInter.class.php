	...
	public function sendClientEmailForFarSign()
	{
		...
		if ($this->getData('email_signature')) {
			...
			if (count($up_errors)) {
				...
				$msg .= 'Cet accès n\'est valable que 4 Jours calendaires.<br/><br/>';
				$msg .= 'Cordialement';
			...
			if (count($children) > 0) {
				...
				foreach ($children as $id_child => $child) {
					...
					if ($product->find(array('ref' => $arrayCode[$child->getData('type')]))) {
						...
						$paBase = /* BimpCore::getConf('cout_horaire_technicien', */
							$product->getCurrentPaHt()/* , 'bimptechnique') */
						;
...
//            }
	...
	public function create(&$warnings = array(), $force_create = false)
	{
		...
		if ((int) $this->getData('fk_contrat')) {
			...
			$heuresRestantes = $verifContrat->getHeuresRestantesDelegation('float');
