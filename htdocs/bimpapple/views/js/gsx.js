...
function Cart(prodId, serial, PM) {
    ...
    this.onComptiaLoadingFail = function () {
        ...
        html += '<p class="alert alert-danger" style="font-size: 11px">';
    ...
    this.displayModifMsg = function () {
        ...
        if ($div.length) {
            ...
            if ($p.length)
                return;
            $div.stop().css('opacity', 1).append('<p class="alert alert-warning changesMsg">Votre panier a été modifié. Si vous souhaitez conserver ces modifications, pensez à sauvegarder le panier.</p>').slideDown(250);
        }
    }
...
//            ptr.showTypeFilters();
    ...
    this.addKeywordFilter = function () {
        ...
        if (!kw) {
            this.$prod.find('.curKeywords').append('<p class="alert alert-danger">Veuillez entrer un mot-clé</p>');
            this.$prod.find('.curKeywords').find('p.alert-danger').fadeOut(2000, function () {
                $(this).remove();
            });
            return;
        }
        if (/ +/.test(kw)) {
            this.$prod.find('.curKeywords').append('<p class="alert alert-danger">Veuillez n\'entrer qu\'un seul mot à la fois</p>');
            this.$prod.find('.curKeywords').find('p.alert-danger').fadeOut(5000, function () {
                $(this).remove();
            });
            return;
        }
        if (!/^[a-zA-Z0-9\.,\-]+$/.test(kw)) {
            this.$prod.find('.curKeywords').append('<p class="alert alert-danger">Caractères interdits. Merci de n\'utiliser que des caractères aplha-numériques</p>');
            this.$prod.find('.curKeywords').find('p.alert-danger').fadeOut(5000, function () {
                $(this).remove();
            });
            return;
        ...
        this.$prod.find('.curKeywords').find('p.alert-danger').stop().remove();
    ...
    this.searchPartByNum = function () {
        ...
        if (!n) {
            this.$prod.find('.searchResult').append('<p class="alert alert-danger">Aucun composant compatible ne correspond à ce numéro</p>');
        }
    };
...
function GSX_Product(id, serial, id_resultContainer) {
    this.id = id;
    this.serial = serial;
    this.PM = new PartsManager(id, serial);
    this.cart = new Cart(id, serial, this.PM);

    if (typeof (id_resultContainer) !== 'undefined') {
        this.$resultContainer = $('#' + id_resultContainer);
    } else {
        this.$resultContainer = $('#requestsResponsesContainer');
    }

    this.loadDatas = function () {
        this.$resultContainer.append('<div id="prod_' + this.id + '" class="productDatasContainer"></div>');
        this.PM.newProdContainer();
        this.cart.newProdContainer();
        setRequest('GET', 'loadProduct', this.id, '&prodId=' + this.id + '&serial=' + this.serial);
...
function GSX() {
        ...
        if ($form.length) {
            ...
            if (prodId && $container.length && $template.length) {
                ...
                if ($prod.length) {
                    ...
                    if (!$cart.length) {
                        $form.find('.partsImportResults').html('<p class="alert alert-warning">Veuillez charger la liste des composants pour afficher le panier</p>').slideDown(250);
                        return;
                    }
                    var $partsRows = $cart.find('table.cartProducts').find('tbody').find('tr');
                    if (!$partsRows.length) {
                        $form.find('.partsImportResults').html('<p class="alert alert-warning">Veuillez ajouter des éléments au panier depuis la liste des composants compatibles</p>').slideDown(250);
                        return;
                    }
                    $container.html('');
                    var i = 1;
    ...
    this.loadProduct = function (serial, id_result_container) {
        if (!serial) {
            displayRequestMsg('alert alert-danger', 'Veuillez entrer un numéro de série');
            return;
        }

        if (!/^[0-9A-Z]{11}[0-9A-Z]*$/.test(serial)) {
            displayRequestMsg('alert alert-danger', 'Le format du numéro de série est incorrect');
            return;
        }

        for (id in this.products) {
            if (this.products[id].serial == serial) {
                displayRequestMsg('alert alert-danger', 'Un produit possédant ce numéro de série a déjà été chargé');
                return;
            }
        }
        this.products[this.nextProdId] = new GSX_Product(this.nextProdId, serial, id_result_container);
    ...
    this.loadProductParts = function ($button) {
        ...
        if (this.products[prodId]) {
            ...
            displayRequestMsg('alert alert-danger', 'Erreur : produit non initialisé', $('#prod' + prodId).find('.partsRequestResult'));
        }
    };
    ...
    this.loadRepairForm = function ($button) {
        ...
        if (this.products[prodId]) {
            ...
            displayRequestMsg('alert alert-danger', 'Erreur : produit non initialisé', $prod.find('.partsRequestResult'));
        }
    };
}
...
function markRepairAsReimbursed($span, repairId) {
    ...
    var $container = $('#repair_' + repairId).find('.repairRequestsResults');
    if ($container.length) {
        $span.attr('class', 'button bluehover markAsReimbursed deactivated');
        displayRequestMsg('requestProcess', '', $container);
        setRequest('GET', 'markRepairAsReimbursed', repairId, '&repairRowId=' + repairId);
        return;
    }
    alert('Une erreur est survenue, opération impossible.');
}
...
function reloadProductInfos(prod_id, serial) {
    ...
    $.ajax({
        type: "GET",
        url: DOL_URL_ROOT + '/bimpapple/ajax/requestProcess.php?action=loadProduct&serial=' + serial + '&prodId=' + prod_id + '&chronoId=' + chronoId,
        dataType: 'html',
        success: function (html) {
    ...
    if ($prod.length) {
        ...
        if ($input.length) {
            ...
            if ($resultContainer.length) {
                ...
                if (!number.length) {
                    displayRequestMsg('alert alert-danger', 'Veuillez indiquer un identifiant et sélectionner le type correspondant.', $resultContainer);
                    return;
                } else {
                    if (checkInput($input, 'text')) {
                        ...
                        displayRequestMsg('alert alert-danger', 'L\'identifiant indiqué ne respecte pas le bon format', $resultContainer);
                        return;
                    }
                }
            }
    ...
    switch (requestType) {
        ...
        case 'loadProduct':
            ...
            if ($div.length) {
                ...
                displayRequestMsg('alert alert-danger', 'Erreur : container absent pour cet ID produit, impossible d\'afficher les données');
            }
            break;
        ...
        case 'loadPartsCart':
            if (xhr.responseText == 'noSerial') {
                displayCartRequestResult(prodId, '<p class="alert alert-danger">Erreur: numéro de série absent</p>');
            } else if (xhr.responseText == 'noPart') {
                displayCartRequestResult(prodId, '<p class="alert alert-warning">Aucun composant enregistré pour ce numéro de série</p>');
            } else if (xhr.responseText == 'noDb') {
                displayCartRequestResult(prodId, '<p class="alert alert-danger">Impossible d\'accéder à la base de données.</p>');
            } else {
        ...
        case 'markRepairAsReimbursed':
            ...
            if ($repair.length) {
                var $repairResult = $repair.find('div.repairRequestsResults');
                $span = $repair.find('span.markAsReimbursed');
                if ($repairResult.length) {
                    if (xhr.responseText == 'ok') {
                        displayRequestMsg('confirmation', 'La réparation a bien été marquée comme remboursée', $repairResult);
                        $span.remove();
                        return;
                    } else {
                        $span.attr('class', 'button redHover closeRepair');
...
function setRequest(method, requestType, prodId, requestParams) {
    ...
    switch (method) {
        case 'GET':
            xhr.open(method, DOL_URL_ROOT + '/bimpapple/ajax/requestProcess.php?action=' + requestType + requestParams + extra);
            xhr.send();
            break;

        case 'POST':
            xhr.open(method, DOL_URL_ROOT + '/bimpapple/ajax/requestProcess.php?action=' + requestType + extra);
