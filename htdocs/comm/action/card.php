...
if ($action == 'create') {
	...
	*/
	...
	if (!empty($conf->societe->enabled)) {
		...
		print $form->selectcontacts(GETPOSTISSET('socid') && GETPOST('socid', 'int') > 0? GETPOST('socid', 'int') : -1, $preselectedids, 'socpeopleassigned[]', 1, '', '', 0, 'minwidth300 quatrevingtpercent', false, 0, array(), false, 'multiple', 'contactid');
		print '</td></tr>';
	}
...
if ($id > 0) {
	...
	if ($action == 'edit') {
		...
		*/
        ...
        print '<input type="submit" class="button valignmiddle" name ="addGroupMember" value="Ajouter"/>';
        print '</div>';
		...
		if (!empty($conf->societe->enabled)) {
			...
			print '<div class="maxwidth200onsmartphone">';
			print img_picto('', 'contact', 'class="paddingrightonly"').$form->selectcontacts($object->socid > 0? $object->socid : -1, array_keys($object->socpeopleassigned), 'socpeopleassigned[]', 1, '', '', 1, 'quatrevingtpercent', false, 0, 0, array(), 'multiple', 'contactid');
			print '</div>';
			print '</td>';
			print '</tr>';
                        }
