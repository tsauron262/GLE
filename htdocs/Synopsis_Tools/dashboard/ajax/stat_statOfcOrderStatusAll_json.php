...
foreach ($tabStat as $id => $label) {
    $label = html_entity_decode($langs->trans($label));
    $arr[$id] = 0;
    array_push($arrXLabel, $label);
}
...
$title = new OFC_Elements_Title("Toutes les commandes par statut 6 mois)");

$bar_stack = new OFC_Charts_Bar_3d();
$bar_stack->set_alpha(0.9);

$maxSoc = 0;
for ($i = -1; $i < count($arrXLabel)-1; $i++) {
