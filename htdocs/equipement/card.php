	...
	if ($productid > 0)
	{
		...
		if ($conf->productbatch->enabled && $fk_product_batch == 0)
		{
			...
			else
			{
				...
				$arraydateendequipement=array('eatby'=>$langs->trans("DateEatBy"), 'sellby'=>$langs->trans("DateSellBy"));
...
else if ($id > 0 || ! empty($ref))
{
	...
	if ($action != 'editnumversion' && $object->statut == 0) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editnumversion&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a></td>';
	print '</tr></table></td><td colspan="3">';
	...
	if ($object->quantity >1)
	{
		...
		if ($action != 'editquantity' && $object->statut == 0) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editquantity&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a></td>';
		print '</tr></table></td><td colspan="3">';
	...
	if ($action != 'editunitweight' && $object->statut == 0) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editunitweight&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a></td>';
	print '</tr></table></td><td colspan="3">';
	...
	if ($action != 'editstock' && $object->statut == 0) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editstock&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a></td>';
	print '</tr></table></td><td colspan="3">';
	if ($action == 'editstock')
	{
	...
	if ($action != 'editclient' && $object->statut == 0) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editclient&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a></td>';
	print '</tr></table></td><td colspan="3">';
	...
	if ($user->rights->facture->lire)
	{
		...
		if ($action != 'editfactclient' && $object->statut == 0 )
			if( $object->fk_soc_client > 0 ) 
				...
				print '<td align="right">'.img_info($langs->trans('NoCustomerSelected'),1).'</td>';
	...
	if ($action != 'editetatequip' && $object->statut == 0) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editetatequip&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a></td>';
	print '</tr></table></td><td colspan="3">';
	...
	if ($action != 'editnumimmo' && $object->statut == 0) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editnumimmo&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a></td>';
	print '</tr></table></td><td colspan="3">';
...
$formquestioncutEquipement=array(
	...
	array('type' => 'text', 'name' => 'ref_new','label' => $langs->trans("NewRefForCutEquipment"), 'value' => $object->ref." (1)", 'size'=>24),
	array('type' => 'text', 'name' => 'quantitynew','label' => $langs->trans("QuantitytoCut"), 'value' => '1', 'size'=>5),
	array('type' => 'checkbox', 'name' => 'cloneevent','label' => $langs->trans("CloneContentEquipment"), 'value' => 1),
);
...
if ($action == '' )
{
	...
	print "<br>\n";
	$somethingshown=$formfile->show_documents('equipement',$filename,$filedir,$urlsource,
		$genallowed, $delallowed, $object->model_pdf, 1,0,0,28,0,'','','', $soc->default_lang);
