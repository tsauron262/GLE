    ...
    function query($query,$usesavepoint=0,$type='auto', $donRecur = false)
    {
        ...
        if(defined('BDD_2_HOST') && !defined('OFF_MULTI_SQL') && BDD_2_HOST !=  $this->database_host){
            if(stripos(trim($query), "SELECT") === 0){
                if(stripos(trim($query), "MAXnnnnn") !== false){
                    $testPlusPetitQueDix = rand(2,12);//rand(6,15);
                    if(1){
                        global $dbRead;
                        if(!$dbRead){
                            $servRead = BDD_2_HOST;
        ...
        if (! preg_match("/^COMMIT/i",$query) && ! preg_match("/^ROLLBACK/i",$query))
        {
            ...
            if (! $ret)
            {
                ...
                if(function_exists("synGetDebug"))
                    $debug = synGetDebug();
				if ($conf->global->SYSLOG_LEVEL < LOG_DEBUG) dol_syslog(get_class($this)."::query SQL Error query: ".$query." |".$debug, LOG_ERR);	// Log of request was not yet done previously
