			...
			if ((int) $this->getData('statut') > 0) {
				...
				if (!empty($lines)) {
					foreach ($lines as $line) {
						...
						if ((int) $line->getData('achat_periodicity') > 0) {
							$hasLinesToBuy = true;
							$line_status = (int) $line->getData('statut');

							if ($line_status > 0 && $line_status < self::STATUS_CLOSED) {
								$periods_data = $line->getPeriodsToBuyData();
								if ((int) $periods_data['nb_periods_tobuy_max'] > 0) {
									$nLinesToBuy++;
								}
								if ((int) $periods_data['nb_periods_tobuy_today'] > 0) {
									$nLinesToBuyToday++;
								}
							}
						}
				...
				if (!$hasLinesToBuy) {
					...
					if ($nLinesToBuy > 0) {
						$html .= '<div><span class="warning">Achats en cours</span></div>';

						if ($nLinesToBuyToday > 0) {
							$html .= '<div><span class="important">' . $nLinesToBuyToday . ' achat(s) à effectuer à date</span></div>';
						}
					} else {
						$html .= '<span class="danger">' . BimpRender::renderIcon('fas_times', 'iconLeft') . 'Achats terminés</span>';
					}
				}
			}
		} else {
