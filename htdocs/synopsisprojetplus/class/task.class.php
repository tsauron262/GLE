...
/*mod drsi Copie de la classe original avec
 * "projet_task_time  changé en "synopsis_projet_task_timeP    sauf    getSummaryOfTimeSpent
 * fmod drsi
 */
 ...
 */
class TaskP extends CommonObject
{
...
//        $sql.= ", ".($this->progress!=''?$this->progress:0);
    ...
    function delete($user, $notrigger=0)
    {
        ...
        if (! $error)
        {
	        $sql = "DELETE FROM ".MAIN_DB_PREFIX."synopsis_projet_task_timeP";
    ...
    function addTimeSpent($user, $notrigger=0)
    {
        ...
        $sql = "INSERT INTO ".MAIN_DB_PREFIX."synopsis_projet_task_timeP (";
        $sql.= "fk_task";
        $sql.= ", task_date";
        $sql.= ", task_datehour";
        ...
        if ($resql)
        {
            $tasktime_id = $this->db->last_insert_id(MAIN_DB_PREFIX."synopsis_projet_task_timeP");
            $ret = $tasktime_id;
        ...
        if ($ret >= 0)
        {
            ...
            /*mod drsi ajout si pas existant au contact du projet*/
            $this->verifUser($this->timespent_fk_user);
            /*fmod drsi*/
...
//			if (isset($this->progress)) $sql.= ", progress = " . $this->progress;	// Do not overwrite value if not provided
            ...
            $sql = "UPDATE ".MAIN_DB_PREFIX."synopsis_projet_task_timeP";
    ...
    function updateTimeSpent($user, $notrigger=0)
    {
        ...
        $sql = "UPDATE ".MAIN_DB_PREFIX."synopsis_projet_task_timeP SET";
