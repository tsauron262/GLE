...
if ($sql) {
    ...
    $subject = "[OK Finance] pour le client " . $societe->nom . " commande " . $commande->ref . " du " . date('d/m/Y', $commande->date);
    ...
    if ($commande->finance_ok == 1) {
        ...
        $result = $interface->run_triggers('PREPACOM_OK_FINANCE', $commande, $user, $langs, $conf);
        if ($result < 0) {
            $error++;
        ...
        $statusFin = 'Non';
        $subject = "[Non Finance] pour le client " . $societe->nom . " commande " . $commande->ref . " du " . date('d/m/Y', $commande->date);
        // Appel des triggers
        include_once(DOL_DOCUMENT_ROOT . "/core/class/interfaces.class.php");
        $interface = new Interfaces($db);
        $result = $interface->run_triggers('PREPACOM_KO_FINANCE', $commande, $user, $langs, $conf);
        if ($result < 0) {
            $error++;
            $errors = $interface->errors;
        }
        ...
        include_once(DOL_DOCUMENT_ROOT . "/core/class/interfaces.class.php");
        $interface = new Interfaces($db);
        $result = $interface->run_triggers('PREPACOM_PARTIAL_FINANCE', $commande, $user, $langs, $conf);
        if ($result < 0) {
            $error++;
            $errors = $interface->errors;
        }
    ...
    if (isset($conf->global->BIMP_MAIL_FROM) && isset($conf->global->BIMP_MAIL_GESTFINANCIER) && isset($conf->global->BIMP_MAIL_GESTPROD)) {
        $to = $tmpUser->email;
