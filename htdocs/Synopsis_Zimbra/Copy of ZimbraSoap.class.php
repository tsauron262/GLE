...
class Zimbra
{
    ...
    */
    public function __construct($username)
    {
        ...
        $this->_preAuthKey = '4d557d3b64955c7d0140f44843079b686b68a984aaaa81f966b86895bf522d59';
...
//        $this->_admin=1;
//        $this->_admin_username = 'admin@synopsis-erp.com';
//        $this->_admin_password = 'de6rehy';
...
//print "<pre>".preg_replace('/</','&lt;',$soap)."</pre>";
...
//print $soap;
    ...
    public function createApptBabel($aptArr,$option_string="")
    {
        ...
        //        print "start " . $datestart."<BR>";
        //        $dateend = $datestart + (2 * 60 * 60) - 1;
        //        $dateend = "20090226T170000"; //18H
        //        print "end " . $dateend."<BR>";
        //        print date("M-d-Y", $datestart)."<BR>";
        //        print date("M-d-Y", $dateend)."<BR>";
        $type = 'event'; //useless ?
        //$type = 'appt';
...
//            Var_Dump::Display($array);
        ...
        public function getListWikiFolder($folderId)
    {//TODO
...
//                }
        ...
        //print  FIELD[lastname]:a* OR FIELD[lastname]:b* </query>
...
// Var_Dump::display($val1);
// print str_repeat("###",120)."<BR>\n";
// $this->errArr = $val1;
                            if (is_array($val1['folder']))
                            {
                                $tmpLevel = $level +1;
...
//        } else {
//            print "Error at ".$level;
//            Var_Dump::Display($this->errArr);
...
//                        print $typeId."<BR>";
                    ...
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              array('debut' => $res->date_valid, "fin" => $res->fin_validite),
                                              "Valid de " . "".$res->ref."" . " (".$res->socname.")",
                                              "Validit&eacute; de la proposition commerciale " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."propal",
                                              1, //all day
...
//                            $arrRes = $this->Babel_pushDateArr(
//                                                  $res->datef,
//                                                  "Date facture " . "".$res->ref."" . " (".$res->socname.")",
...
//                                         $id++;
//                    }
//                        print "toto ".$res->date_lim_reglement . " ".$res->date_valid."<BR>";
                    if ($res->date_lim_reglement && $res->datef)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              array('debut' => $res->datef, "fin" => $res->date_lim_reglement),
                                              "Facture " . "".$res->ref."" . " (".$res->socname.")",
                                              "Réglement de la facture " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."facture",
                                              1, //all day
                    ...
                    }else if ($res->datef)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->datef,
                                              "Valid de " . "".$res->ref."" . " (".$res->socname.")",
                                              "Validit&eacute; de la facture " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."facture",
                                              1, //all day
                ...
                if ($resql)
                {
                    while($res=$db->fetch_object($resql))
                    {
                        ...
                        if ($resqlLocZim=$this->db->query($requeteLocZim))
                        {
                            ...
                            $arrRes = $this->Babel_pushDateArr(
                                                          $objPai->datep,
                                                          "Regl de " . "".$res->ref."" . " (".$res->socname.")",
                                                          "R&egrave;glement de la facture " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                                          $res->ref,
                                                          $id,
                    ...
                    while($res=$db->fetch_object($resql))
                    {
                        ...
                        if ($resqlLocZim=$this->db->query($requeteLocZim))
                        {
                            ...
                            $arrRes = $this->Babel_pushDateArr(
                                                          $objPai->datep,
                                                          "Regl de " . "".$res->ref."" . " (".$res->socname.")",
                                                          "R&egrave;glement de la facture fournisseur " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                                          $res->ref,
                                                          $id,
                                                          "".MAIN_DB_PREFIX."paiement",
                                                          1, //all day
...
//                        print "toto ".$res->date_lim_reglement . " ".$res->date_valid."<BR>";
                    if ($res->date_lim_reglement && $res->datef)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              array('debut' => $res->datef, "fin" => $res->date_lim_reglement),
                                              "Facture " . "".$res->ref."" . " (".$res->socname.")",
                                              "Réglement de la facture fournisseur" . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."facture_fourn",
                                              1, //all day
                    ...
                    }else if ($res->datef)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->datef,
                                              "Valid de " . "".$res->ref."" . " (".$res->socname.")",
                                              "Validit&eacute; de la facture fournisseur" . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."facture_fourn",
                                              1, //all day
...
//                        print $typeId."<BR>";
                    ...
                    if ($res->date_valid)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_valid,
                                              "Validation intervention " . "".$res->rowid."" . " (".$res->socname.")",
                                              "Validation de l'intervention " . $res->rowid . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->rowid,
                                              $id,
                                              "".MAIN_DB_PREFIX."fichinter",
                                              1, //all day
...
//                        print $typeId."<BR>";
                        ...
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_creation,
                                              "Prise en compte de l'expedition " . "".$res->ref."" . " (".$res->socname.")",
                                              "Prise en compte de l'expedition " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                    ...
                    if ($res->date_expedition)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_expedition,
                                              "Expedition " . "".$res->ref."" . " (".$res->socname.")",
                                              "Expedition de " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."expedition",
                                              1, //all day
                    ...
                    if ($res->date_valid)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_valid,
                                              "Validation de l'expedition " . "".$res->rowid."" . " (".$res->socname.")",
...
//     print $requeteLocZim."<BR>";
                        ...
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_creation,
                                              "Prise en compte de la livraison " . "".$res->ref."" . " (".$res->socname.")",
                                              "Prise en compte de la livraison " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                    ...
                    if ($res->date_livraison)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_livraison,
                                              "livraison " . "".$res->ref."" . " (".$res->socname.")",
                                              "livraison de " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."livraison",
                                              1, //all day
                    ...
                    if ($res->date_valid)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_valid,
                                              "Validation de la livraison " . "".$res->rowid."" . " (".$res->socname.")",
...
//print "<BR>".$res->socname . " <BR>". $requeteLocZim . "<BR><BR>";
                    if ($resqlLocZim = $db->query($requeteLocZim))
                    {
                        $zimRes =  $db->fetch_object($resqlLocZim);
                        $zimLoc =$zimRes->fid;
                        $typeId = $zimRes->ftid;
//                        print $requeteLocZim . " ".$res->socname . " ".$res->folder_uid."<br>";
//si durationp => debut + fin =  si datep2 != datep => debut + fin
                        ...
                        if ($res->datep)
                        {
                            ...
                            $arrRes = $this->Babel_pushDateArr(
                                                  $date,
                                                  $res->libelle . " (".$res->socname.")",
                                                  "Action commerciale " .  $res->libelle ."<HR><P>".$res->label . "<BR>Ref :" . $res->ref . "<HR><P>" . $res->note. "<BR><P>" ,
                                                  $res->ref,
                                                  $id,
                                                  "".MAIN_DB_PREFIX."actioncomm",
                                                  $allDay, //all day
                                                  "",
                                                  1,//loc géo
                                                  $zimLoc,//loc zimbra
...
//                        print $typeId."<BR>";
                        ...
                        $zimLoc =$zimRes->fid;
                        $typeId = $zimRes->ftid;
//                        print $res->socname ." ".$zimLoc ."<br>";

                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_creation,
                                              "Créat. de " . "".$res->ref."" . " (".$res->socname.")",
                                              "Cr&eacute;ation de la commande commerciale " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                    ...
                    if ($res->date_commande)
                    {
                            $arrRes = $this->Babel_pushDateArr(
                                                  $res->datec,
                                                  "Date commande " . "".$res->ref."" . " (".$res->socname.")",
                    ...
                    if ($res->date_valid && $res->date_cloture."x"=="x")
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_valid,
                                              "Valid de " . "".$res->ref."" . " (".$res->socname.")",
                                              "Validit&eacute; de la commande " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."commande",
                                              1, //all day
                    ...
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              array('debut'=>$res->date_valid, 'fin'=>$res->date_cloture),
                                              "Clot de " . "".$res->ref."" . " (".$res->socname.")",
                                              "Cloture de la commande " . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."commande",
                                              1, //all day
...
//                        print $res->socname ." ".$zimLoc ."<br>";
                        ...
                        $arrRes = $this->Babel_pushDateArr(
                                              ...
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."commande_fournisseur",
                                              1, //all day
                                              "",
                                              1,//loc géo
                                              $zimLoc,//loc zimbra
                    ...
                    if ($res->date_commande)
                    {
                            $arrRes = $this->Babel_pushDateArr(
                                                  $res->datec,
                                                  "Date commande fournisseur" . "".$res->ref."" . " (".$res->socname.")",
                    ...
                    if ($res->date_valid && $res->date_cloture."x"=="x")
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              $res->date_valid,
                                              "Valid de " . "".$res->ref."" . " (".$res->socname.")",
                                              "Validit&eacute; de la commande fournisseur" . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."commande_fournisseur",
                                              1, //all day
                    ...
                    }else if ($res->date_cloture)
                    {
                        $arrRes = $this->Babel_pushDateArr(
                                              array('debut'=>$res->date_valid, 'fin'=>$res->date_cloture),
                                              "Clot de " . "".$res->ref."" . " (".$res->socname.")",
                                              "Cloture de la commande fournisseur" . $res->ref . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                              $res->ref,
                                              $id,
                                              "".MAIN_DB_PREFIX."commande_fournisseur",
                                              1, //all day
    ...
    public function Synopsis_Zimbra_GetSoc()
    {
        ...
        if ($resqlsoc = $db->query($requete))
        {
            while ($soc = $db->fetch_object($resqlsoc))
            {
                ...
                //print "<BR>toto".$soc->nom." => ".$firstLetterA."<BR>";
...
//                                                      print $requete."<br>";
            ...
            print $count ."soci&eacute;t&eacute;s synchronis&eacute;es<BR>";
        ...
        if ($resqlsoc = $db->query($requete))
        {
            while ($res = $db->fetch_object($resqlsoc))
            {
              ...
              if($resql=$db->query($requete))
              {
                    ...
                    $confPrefCat = "work";
                    $confJabberPrefix = "other://";
                    if ("x".$cont->nom != "x")              { $contArr['contactDet']["lastName"] = iconv("ISO-8859-1","UTF-8",$cont->nom); }
                    if ("x".$cont->prenom != "x")           { $contArr['contactDet']["firstName"] = iconv("ISO-8859-1","UTF-8", $cont->prenom); }
                    if ("x".$cont->address != "x")          { $contArr['contactDet'][$confPrefCat."Street"] = iconv("ISO-8859-1","UTF-8",$cont->address);}
                    if ("x".$cont->cp != "x")               { $contArr['contactDet'][$confPrefCat."PostalCode"] = iconv("ISO-8859-1","UTF-8",$cont->cp); }
                    if ("x".$cont->ville != "x")            { $contArr['contactDet'][$confPrefCat."City"] = iconv("ISO-8859-1","UTF-8",$cont->ville); }
                    if ("x".$cont->pays != "x" && $cont->fk_pays > 0)             { $contArr['contactDet'][$confPrefCat."Country"] = iconv("ISO-8859-1","UTF-8",$cont->pays); }
                    if ("x".$cont->poste != "x")            { $contArr['contactDet']["jobTitle"] = iconv("ISO-8859-1","UTF-8",$cont->poste); }
                    if ("x".$cont->email != "x")            { $contArr['contactDet']["email"] = iconv("ISO-8859-1","UTF-8",$cont->email); }
                    if ("x".$cont->birthday_mysql != "x")   { $contArr['contactDet']["birthday"] = iconv("ISO-8859-1","UTF-8",$cont->birthday_mysql); }
                    if ("x".$cont->jabberid != "x")         { $contArr['contactDet']["imAddress1"] = iconv("ISO-8859-1","UTF-8",$confJabberPrefix . $cont->jabberid); }
                    if ("x".$cont->note != "x")             { $contArr['contactDet']["notes"] = iconv("ISO-8859-1","UTF-8",$cont->note); }
                    if ("x".$cont->phone_pro != "x")        { $contArr['contactDet']["workPhone"] = iconv("ISO-8859-1","UTF-8",$cont->phone_pro); }
                    if ("x".$cont->fax != "x")              { $contArr['contactDet'][$confPrefCat."Fax"] = iconv("ISO-8859-1","UTF-8",$cont->fax); }
                    if ("x".$cont->phone_perso != "x")      { $contArr['contactDet']["homePhone"] = iconv("ISO-8859-1","UTF-8",$cont->phone_perso); }
                    if ("x".$cont->phone_mobile != "x")     { $contArr['contactDet']["mobilePhone"] = iconv("ISO-8859-1","UTF-8",$cont->phone_mobile); }
                    if ("x".$cont->socname != "x")          { $contArr['contactDet']["company"] = iconv("ISO-8859-1","UTF-8",$cont->socname); }
