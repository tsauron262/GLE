...
class InvoiceStatementPDF extends BimpDocumentPDF
{
    ...
    public function renderContent() {
        ...
        $html .= '<td style="width:30%" >Libellé</td>';
        $html .= '<th style="width:10%">Date</th>';
        $html .= '<th style="width:10%">Echeance</th>';
        $html .= '<th style="width:25%">Contact de livraison</th>';
        $html .= '<th style="width:13%">Montant TTC</th>';
        $html .= '</tr>';
        $list = $this->bimpDb->getRows('facture', 'fk_soc = ' . $this->object->id . ' AND datef BETWEEN "'.$this->date_debut->format('Y-m-d').'" and "'.$this->date_fin->format('Y-m-d').'"');
        $total_facture = 0;
        foreach($list as $facture) {
            ...
            $html .= '<td>'.round($instance->getData('total_ttc'), 2).'€</td>';
            $html .= '</tr>';
        }
        
        $html .= '<tr><td></td><td></td><td></td><td></td><td><b>TOTAL TTC des factures:</b></td><td><b>'.$total_facture.'€</b></td></tr>';
