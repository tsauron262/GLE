...
// Suppression de la propale
    ...
    if ($result > 0) {
        ...
        $langs->load("errors");
        if ($object->error == 'ErrorFailToDeleteDir')
            $mesg = '<div class="error">' . $langs->trans('ErrorFailedToDeleteJoinedFiles') . '</div>';
        else
    ...
    if ($result >= 0 && isset($_POST['qty']) && (($_POST['np_price'] != '' && ($_POST['np_desc'] || $_POST['dp_desc'])) || $_POST['idprod'])) {
        ...
        if ($price_min && (price2num($pu_ht) * (1 - price2num($_POST['remise_percent']) / 100) < price2num($price_min))) {
            ...
            $result = $object->addline(
                    $_POST["id"], $desc, $pu_ht, $_POST['qty'], $tva_tx, $localtax1_tx, $localtax2_tx, $_POST['idprod'], $_POST['remise_percent'], $price_base_type, $pu_ttc, $info_bits, $type, -1, 0, $_POST['fk_parent_line']
            );
...
// Mise a jour d'une ligne dans la propale
    ...
    if ($productid && $price_min && (price2num($up_ht) * (1 - price2num($_POST['remise_percent']) / 100) < price2num($price_min))) {
        ...
        $result = $object->updateline(
                $_POST['lineid'], $up_ht, $_POST['qty'], $_POST['remise_percent'], $vat_rate, $localtax1_rate, $localtax2_rate, $_POST['desc'], 'HT', $info_bits, $special_code, $_POST['fk_parent_line']
        );
...
if ($id > 0 || !empty($ref)) {
    ...
    /* Modif drsi */
    if ($action == 'reviser') {
        $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('ReOpen'), $langs->trans('Êtes-vous sûr de vouloir réviser cette proposition commerciale ?', $object->ref), 'confirm_reviser', '', 0, 1);
    }
    /* /MOdif drsi */
    ...
    if (!empty($object->lines))
        $object->printObjectLines($action, $mysoc, $soc, $lineid, 0, $hookmanager);
    ...
    if ($action != 'presend') {
        ...
        if ($action != 'statut' && $action <> 'editline') {
            ...
            // ReOpen
            if (($object->statut == 3) && $user->rights->propale->cloturer) {
            ...
            /* Modif drsi */
            if (($object->statut == 2) && $user->rights->SynopsisRevProp->SynopsisRevProp->Push) {
                print '<a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&amp;action=reviser' . (empty($conf->global->MAIN_JUMP_TAG) ? '' : '#reviser') . '"';
                print '>' . $langs->trans('Réviser') . '</a>';
            }
            /* Modif drsi */
    ...
    if ($sall)
        $sql.= " AND (s.nom LIKE '%" . $db->escape($sall) . "%' OR p.note LIKE '%" . $db->escape($sall) . "%' OR pd.description LIKE '%" . $db->escape($sall) . "%')";
    if ($socid)
