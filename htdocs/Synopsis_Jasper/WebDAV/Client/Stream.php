...
require_once DOL_DOCUMENT_ROOT . '/Synopsis_Jasper/WebDAV/Tools/_parse_propfind_response.php';
require_once DOL_DOCUMENT_ROOT . '/Synopsis_Jasper/WebDAV/Tools/_parse_lock_response.php';
...
');
    ...
    function stream_upload_file($buffer)
    {
        // do some math
        $start = $this->position;
        $end   = $this->position + strlen($buffer);

        $req =& new HTTP_Request($this->url,array('timeout' => 20,
            'readTimeout' => array(20,0)) );
        if (is_string($this->user)) {
            $req->setBasicAuth($this->user, @$this->pass);
        }
        $req->setMethod(HTTP_REQUEST_METHOD_PUT);
    ...
    function stream_write($buffer)
    {
        ...
        case 204:
            $this->position += strlen($buffer);
            print "toto<BR>".$this->position."<BR>";
            return $end - $start;
