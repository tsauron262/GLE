 ...
 * $Source: /cvsroot/dolibarr/dolibarr/htdocs/Licence/index.php,v $
 *
...
//var_dump($user->rights->Licence->lire);
if (!$user->rights->Licence->lire) { accessforbidden(); }
 ...
 */
    ...
    $js .= ' <script src="'.$jspath.'/jquery.validate.js" type="text/javascript"></script>';
    $js .= "<script>jQuery(document).ready(function(){ jQuery('#createLicence').validate({
    ...
    jQuery('#modDialog').dialog({
        ...
        minWidth: 560,
        modal: true,
        title: 'Modification de la licence',
        buttons:{
            'OK':function() {
                if (jQuery('#modLicence').validate({
                        rules: { nom:{ required: true},
                                 datec: { FRDate: true , required: true },
                                 datef: { FRDateNotRequired: true  },
                               },
                    ...
                    var data = jQuery('#modLicence').serialize();
                    jQuery.ajax({
                        url: 'index.php',
                        data: 'action=mod&id='+Licenceid+'&data='+data,
                        datetype: 'xml',
                        type: 'post',
        ...
        open: function(){
            jQuery('#modLicence')[0].reset();
            jQuery('#ui-datepicker-div').addClass('promoteZ');
            jQuery.ajax({
                url: 'index.php',
                data: 'action=getData&id='+Licenceid,
                type: 'POST',
                datatype: 'xml',
                success: function(msg){
                    jQuery('#modLicence').find('#nom').val(jQuery(msg).find('label').text());
                    jQuery('#modLicence').find('#datec').val(jQuery(msg).find('datec').text());
                    jQuery('#modLicence').find('#datef').val(jQuery(msg).find('datef').text());
                    jQuery('#modLicence').find('#note').val(jQuery(msg).find('note').text());
                    jQuery('#modLicence').find('#code').val(jQuery(msg).find('code').text());
                    var active = jQuery(msg).find('active').text();
                    if (active == 1)
                    {
                        jQuery('#modLicence').find('#actif').attr('checked','checked');
                    } else {
                        jQuery('#modLicence').find('#actif').attr('checked','checked');
                    }
                }
    ...
    jQuery('#list2').jqGrid({
        ...
        colNames:['rowid','".$langs->trans("Description")."','".$langs->trans("DateCreate")."', '".$langs->trans("DateFin")."', '".$langs->trans("Code")."','".$langs->trans("Note")."', '".$langs->trans("Actif")."', '".$langs->trans("Action")."'],
        ...
        viewrecords: true,
        sortorder: 'desc',
        caption:'".$langs->trans('LicenceNames')."' });
...
function deleteLicence(pId)
{
    Licenceid=pId;
    jQuery('#delDialog').dialog('open');
}
function modifyLicence(pId)
{
    Licenceid=pId;
...
";
if ($_REQUEST['action'] == "mod" && $user->rights->Licence->creer)
{
  ...
  $Licenceid = $_REQUEST['id'];
  $requete = "UPDATE Babel_Licence
                 SET label='".$nom."',
                     datec=".$datec.",
                     datef=".$datef.",
                     active=".$actif.",
...
if ($_REQUEST['action'] == "getData" && $user->rights->Licence->lire)
{
    $LicenceId = $_REQUEST['id'];
    if ($LicenceId > 0)
    {
        $requete = "SELECT label, note, active, code,
                           date_format(datec,'%d/%m/%Y') as datec,
                           date_format(datef,'%d/%m/%Y') as datef
                      FROM Babel_Licence WHERE rowid = ".$LicenceId;
        $sql = $db->query($requete);
        ...
        if ($sql)
        {
            ...
            $xml .= "<code><![CDATA[".utf8_encode($res->code)."]]></code>";
        } else {
            $xml .= "<KO>KO</KO>";
        }
        if ( stristr($_SERVER["HTTP_ACCEPT"],"application/xhtml+xml") ) {
...
if ($_REQUEST['action'] == 'list'&& $user->rights->Licence->lire)
{
    $page = $_GET['page'];
    $limit = $_GET['rows'];
    $sidx = $_GET['sidx'];
    $sord = $_GET['sord'];
    if(!$sidx) $sidx =1;
    $result = $db->query("SELECT COUNT(*) AS count FROM Babel_Licence");
    ...
    $SQL = "SELECT * FROM Babel_Licence  ORDER BY $sidx $sord LIMIT $start , $limit";
    ...
    while($row = $db->fetch_object($result,MYSQL_ASSOC))
    {
        ...
        $responce->rows[$i]['cell']=array($row->rowid,
                                           ...
                                           " <center>   <table style='margin-top: -20px;' cellpadding=0><tr><t><td style='background:none repeat scroll 0 0 transparent;border:0 none transparent;max-width:16px;width:16px; height: 16px;max-height:16px;' valign=center><span title='Modifier' style='width: 16px;' class='ui-icon ui-icon-arrowrefresh-1-e' onclick='modifyLicence(".$row->rowid.")'></span><td style='background:none repeat scroll 0 0 transparent;border:0 none transparent;max-width:16px;width:16px; height: 16px;max-height:16px;' valign=center><span title='Supprimer' style='width: 16px;' class='ui-icon ui-icon-trash' onclick='deleteLicence(".$row->rowid.")'></span></table></center>");
        $i++;
    }
...
if ($_REQUEST['action'] == 'del' && $user->rights->Licence->effacer)
{
    $xml = "<ajax-response>";
    $id = $_REQUEST['id'];
    $requete = "DELETE FROM Babel_Licence WHERE rowid = ".$id;
    $sql = $db->query($requete);
    if ($sql)
    {
        $xml .= "<OK>OK</OK>";
...
exit;
    ...
    llxHeader($js,'Licencees',"",1);
    if (($_REQUEST['action']=='add') && $_REQUEST['cancel']!= 1&& $user->rights->Licence->creer)
    {
        $datec = parseDate($_REQUEST['datec']);
        $datef = parseDate($_REQUEST['datef']);
        $requete = "INSERT INTO `Babel_Licence`
                                (`datec`,`datef`,`label`,`code`,`note`,`active`)
                        VALUES
                                ( ".$datec.",".$datef."  , '".preg_replace('/\'/','\\\'',$_REQUEST['nom'])."', '".preg_replace('/\'/','\\\'',$_REQUEST['code'])."','".preg_replace('/\'/','\\\'',$_REQUEST['note'])."', ".($_REQUEST['actif']."x"!='x'?1:0).");";
        $sql = $db->query($requete);
    }
    if ($_REQUEST['action']=="create" || $_REQUEST['addNew']== 1 && $user->rights->Licence->creer)
    {
          print "<br/><div class='titre'>".$langs->trans("NewLicence")."</div><br/>";
          print "<form method='POST' action='index.php' id='createLicence'>";
          ...
          print "<tr><th class='ui-widget-header ui-state-default'>Code</th>";
          print "    <td class='ui-widget-content'><textarea name='code' id='code'></textarea></td>";
...
//
        print "<br>";
        print "<div class='titre'>".$langs->trans("LicenceNames")."</div>";
        print "<br>";
    ...
    print '<table id="list2"></table> <div id="pager2"></div>';
        ...
        print "<br/>";
        if ($user->rights->Licence->creer)
        {
            print "<hr/>";
            print "<div id='tabBar' style='float: right;'>";
            print "<form action='index.php?action=create' method='POST'>
                        <button style='padding: 5px 10px' class='ui-widget-header ui-state-default ui-corner-all butAction' >".$langs->trans('NewLicence')."</button></form>";
            print "</div>";
        }
...
print "<div id='delDialog'>&Ecirc;tes vous sur de vouloir supprimer cette licence ?</div>";

print "<div id='modDialog'>";
      print "<form method='POST' action='index.php' id='modLicence'>";
