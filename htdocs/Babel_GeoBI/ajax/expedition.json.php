...
$requete = "SELECT count(*) as cnt, lat,lng, ".MAIN_DB_PREFIX."societe.nom,".MAIN_DB_PREFIX."societe.rowid FROM ".MAIN_DB_PREFIX."expedition, ".MAIN_DB_PREFIX."societe, Babel_GeoBI
WHERE ".MAIN_DB_PREFIX."expedition.fk_soc=".MAIN_DB_PREFIX."societe.rowid AND ".MAIN_DB_PREFIX."societe.rowid = Babel_GeoBI.`socid` AND lat is not null AND year(date_expedition)  > ". $dateStart ."
...
while ($res = $db->fetch_object($sql))
{
    ...
    $dateStart = date('Y') - 3 ;
    $requete = "SELECT count(*) as t, quarter(date_expedition) as d, year(date_expedition) as y
                  FROM ".MAIN_DB_PREFIX."expedition
                 WHERE fk_soc = ". $res->rowid." AND year(date_expedition)  > ". $dateStart ."
              GROUP BY d, y";
              //print $requete."\n";
    $arrPropal = array();
    ...
    while ($res1 = $db->fetch_object($sql1))
    {
        ...
        $arrPropal[$res1->y]['count']+= $res1->t;
    }
    $dateStart = date('Y') - 2;


$table = '<table class"canvasStats" style="display: none;" id="expedTable'.$res->rowid.'">
...
';
    ...
    $arr[]=array('socid'=>$res->rowid, 'cnt' => $res->cnt, 'lat' => $res->lat, 'lng' => $res->lng, 'nom' => utf8_encode($res->nom),'stat' => $table,
                    "y0" => $dateStart,
                    "y1" => $dateStart1,
                    "y2" => $dateStart2,
