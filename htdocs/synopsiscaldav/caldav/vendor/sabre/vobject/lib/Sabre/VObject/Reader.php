        ...
        if (!$result) {
            if ($options & self::OPTION_IGNORE_INVALID_LINES) {
                ...
                if(!is_file("/data/synchro/test2.txt"))
                    file_put_contents("/data/synchro/test2.txt", $dataOrig2);
                dol_syslog("Probléme de parsage caldav ".print_r($lines,1)." ligne original ". $dataOrig." apres modifs ". $dataOrig2." apres modifs ". $dataOrig3, 3, 0, "_caldav");
