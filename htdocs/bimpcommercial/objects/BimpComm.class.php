...
//                return BimpTools::ucfirst($this->getLabel()) . ' #' . $this->id;
	...
	public function getClientFinal(){
		...
		if ($id_contact > 0) {
			...
			if ($id_client) {
				$client = BimpCache::getBimpObjectInstance('bimpcommercial', 'Bimp_Societe', $id_client);
				return $client;
			}
		}
		$client = $this->getChildObject('client');
	...
	{
		$html = $this->displayCountNotes(true);
	...
	public function duplicate($new_data = array(), &$warnings = array(), $force_create = false)
	{
		...
		if (count($copy_errors)) {
			...
			if ($this->isProductsPricesUpdateAllowed() && (int) BimpTools::getArrayValueFromPath($new_data, 'update_products_prices', 0)) {
				$params['update_products_prices'] = true;
			}
	...
	public function createMajLn($dataFiltre, $dataParamsDirect, $data = array(), &$newLn = null)
	{
		...
		if (!count($dataFiltre)) {
			...
			if (is_null($newLn)) {
				$newLn = $newLnTmp;
			} //BimpObject::getInstance($newLn->module, $newLn->object_name);
	...
	public function createLinesFromOrigin($origin, $params = array(), &$warnings = array())
	{
		...
		foreach ($lines as $line) {
			...
			if ((int) $params['update_products_prices']) {
				$product = $line->getProduct();
				if (BimpObject::objectLoaded($product)) {
					$pu_ht = (float) $product->getData('price');
			...
			$new_line->qty = $qty;
			$new_line->pu_ht = $pu_ht;
			$new_line->tva_tx = $tva_tx;
			$new_line->pa_ht = $pa_ht;
...
//            }
	...
	public function updateProductPrices(&$warnings = array())
	{
		$errors = array();

		if (!$this->areLinesEditable()) {
			$errors[] = 'Les lignes ne sont pas éditables';
			return $errors;
		}
		...
		foreach ($lines as $line) {
			...
			if (BimpObject::objectLoaded($product) && $product->hasFixePu()) {
				$line->pu_ht = (float) $product->getData('price');
				$line->tva_tx = (float) $product->getData('tva_tx');
				$line->pa_ht = $product->getCurrentPaHt();

				$line_errors = $line->update($warnings, true);

				if (count($line_errors)) {
					$errors[] = BimpTools::getMsgFromArray($line_errors, 'Echec mise à jour de la ligne n° ' . $line->getData('position'));
				}
			}
		}
	...
	public function actionUpdateProductsPrices($data, &$success = '')
	{
		$success = 'Mise à jour des prix effectuée';

		$warnings = array();
		$errors = $this->updateProductPrices($warnings);
