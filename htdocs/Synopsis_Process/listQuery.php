...
jQuery(document).ready(function() {
    var grid = jQuery("#gridListQuery").jqGrid({
            datatype: "json",
            url: "ajax/listQuery_json.php?",
            colNames:['id', "Label",'Description'],
EOF;
    $jqgridJs .= <<<EOF
...
$jqgridJs .= <<<EOF
            ...
            rowList:[30,50,100],
            imgpath: gridimgpath,
            pager: jQuery('#gridListQueryPager'),
            sortname: 'label',
            beforeRequest: function(){
                jQuery('#gview_gridListQuery').find('.ui-jqgrid-titlebar').addClass('ui-state-default');
            },
...
$jqgridJs .= <<<EOF
            ...
            sortorder: "asc",
            //multiselect: true,
            caption: "<span style='padding:4px; font-size: 16px;'>Requ&ecirc;tes</span>",
            viewsortcols: true,
            ondblClickRow: function(e,u){
                location.href='queryBuilder.php?id='+e;
            },
EOF;

    $jqgridJs.= '    }).navGrid("#gridListQueryPager",'."\n";
...
llxHeader($js,'Liste requetes','',1);

print "<br/>";
print "<br/>";
print "<br/>";

  print '<table id="gridListQuery" class="scroll ui-widget " cellpadding="0" cellspacing="0"></table>';
  print '<div id="gridListQueryPager" class="scroll" style="text-align:center;"></div>';
