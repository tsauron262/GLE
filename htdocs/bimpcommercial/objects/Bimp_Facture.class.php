...
//            $html .= '<span class="btn btn-default" onclick="' . $this->getJsActionOnclick('generatePDFDuplicata') . '">';
    ...
    public function renderHeaderExtraLeft()
    {
        ...
        if (1) {
            $result = BimpObject::getBimpObjectObjects($this->module, $this->object_name, array('fk_soc' => $this->getData('fk_soc'), 'type' => $this->getData('type'), 'datec' => array('custom' => 'datec < DATE_ADD("' . $this->getData('datec') . '", INTERVAL 1 MINUTE) AND datec > DATE_ADD("' . $this->getData('datec') . '", INTERVAL -1 MINUTE)')));
