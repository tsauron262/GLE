...
$_SESSION['ex4customer']="select CustomerID,CompanyName,ContactName,Address,City,Region,PostalCode,Country,Phone,Fax from customers order by CustomerID";
$_SESSION['ex4order']="select CustomerID,OrderID,ShipName,ShipCity,ShipCountry,OrderDate,ShippedDate from orders order by OrderID";
$_SESSION['ex4detail']="select OrderID,p.ProductName,QuantityPerUnit,od.UnitPrice,Quantity,od.UnitPrice*Quantity as Total,Discount,od.UnitPrice*Quantity*(1.0-Discount) as NetPrice from order_details od left join products p on od.ProductID=p.ProductID order by od.ProductID";
?>
...
function newGrid(TableName) {
  ...
  ex4_buffer=null;
  ex4_grid=null;
  ex4_buffer=new Rico.Buffer.AjaxSQL('ricoXMLquery.php', { requestParameters: ['id=ex4'+TableName] }); // override the id sent in the ajax request
