    ...
    */
    function update($id)
    {
        ...
        */
        ...
        $sql .= ", natureInter = '".$this->natureInter."'";
        $sql .= " WHERE rowid = ".$id;
  ...
  /*  function verifyNumRef($soc)
        ...
        if ($user->rights->synopsisficheinter->creer)
        {
            ...
            if ($sqlres)
            {
                ...
                if ($numprojet > 0)
                {
                    ...
                    $sql = 'UPDATE '.MAIN_DB_PREFIX.'Synopsis_fichinter SET fk_projet = '.$project_id;
                    $sql .= ' WHERE rowid = '.$this->id.' AND fk_statut = 0 ;';
                    $this->db->query($sql);
                }
            }
    ...
    */
    ...
    function getNomUrl($withpicto=0,$option=0)
    {
        ...
        if ($withpicto && $option==6) $result.=($lien.img_object($label,$picto,16,16,false,true).$lienfin);
...
//Compute total tva / total_TTC
        ...
        if ($this->typeIntervProd > 0) $sql .= ", fk_depProduct ";
        $sql.= ')';
        ...
        if ($this->typeIntervProd > 0) $sql .= ", ".$this->typeIntervProd;
        $sql.= ')';
...
//print "toto".$this->total_ht;
        ...
        if ($this->fk_contratdet == 0) $sql .= ",fk_contratdet =  0";
        if ($this->typeIntervProd > 0 && $isDep) $sql .= ",fk_depProduct = ".$this->typeIntervProd;
