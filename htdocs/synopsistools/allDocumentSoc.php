...
if ($object->id) {
    ...
    dol_fiche_head($head, 'allDoc', $langs->trans("ThirdParty"), 0, 'company');
    ...
    print "<br><br>";
    foreach ($tabElement as $typeElemOrTab) {
        if (!is_array($typeElemOrTab))
            $typeElemOrTab = array($typeElemOrTab, $typeElemOrTab);

        $typeElem = $typeElemOrTab[0];
        $nomTab = $typeElemOrTab[1];
        $nomDossier = $typeElemOrTab[0];
        $nomPublic = $typeElemOrTab[0];
        if (isset($typeElemOrTab[2]))
