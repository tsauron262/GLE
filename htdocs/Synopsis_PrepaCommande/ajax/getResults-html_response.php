...
$requete = "SELECT SUM(total_ht) as tht
             ...
             WHERE p.rowid = cdet.fk_product
               ...
               AND c.rowid IN (SELECT catId FROM llx_Synopsis_PrepaCom_c_cat_total)
...
print "</table>";print "<td valign=top>";
...
while($resContent = $db->fetch_object($sqlContent))
{
    ...
    foreach($com->lines as $key=>$val)
    {
        if ($val->fk_product > 0){
            $replacePosition++;
            $requete = "SELECT ct.fk_categorie, c.label, c.rowid, '".$replacePosition."' as position
...
//    }
//}
//if (count($arrLabelSort) >0){
//    array_multisort($arrLabelSort,SORT_ASC,SORT_STRING);
//    foreach($arrLabelSort as $key=>$val){
//        print "<tr><th colspan=2 class='ui-wdiget-header ui-state-default'>".traiteStr($val).'<td class="ui-widget-content" colspan=2 align=right>'.price($arrCat[$val]).'&euro;';
//    }
//}
//print "</table>";
...
//array_multisort($arrLabelSort,SORT_ASC,SORT_STRING);
//foreach($arrLabelSort as $key=>$val){
//    print "<tr><th colspan=2 class='ui-wdiget-header ui-state-default'>".traiteStr($val).'<td class="ui-widget-content" align=right colspan=2 >'.price($arrCat[$val]).'&euro;';
//}
//print "</table>";
