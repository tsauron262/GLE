 ...
 * $Source: /cvsroot/dolibarr/dolibarr/htdocs/domain/index.php,v $
 *
...
if (!$user->rights->domain->lire) { accessforbidden(); }
 ...
 */
    ...
    $js .= ' <script src="'.$jspath.'/jqGrid-3.5/src/grid.tbltogrid.js" type="text/javascript"></script>';
    ...
    $js .= "<script>jQuery(document).ready(function(){ jQuery('#createDomain').validate({
    ...
    jQuery('#modDialog').dialog({
        ...
        buttons:{
            'OK':function() {
                if (jQuery('#modDomain').validate({
                    ...
                    var data = jQuery('#modDomain').serialize();
                    jQuery.ajax({
                        url: 'index.php',
                        data: 'action=mod&id='+domainid+'&data='+data,
                        datetype: 'xml',
                        type: 'post',
        ...
        open: function(){
            jQuery('#modDomain')[0].reset();
            jQuery('#ui-datepicker-div').addClass('promoteZ');
            jQuery.ajax({
                url: 'index.php',
                data: 'action=getData&id='+domainid,
                type: 'POST',
                datatype: 'xml',
                success: function(msg){
                    jQuery('#modDomain').find('#nom').val(jQuery(msg).find('label').text());
                    jQuery('#modDomain').find('#datec').val(jQuery(msg).find('datec').text());
                    jQuery('#modDomain').find('#datef').val(jQuery(msg).find('datef').text());
                    jQuery('#modDomain').find('#dater').val(jQuery(msg).find('dater').text());
                    jQuery('#modDomain').find('#note').val(jQuery(msg).find('note').text());
                    var active = jQuery(msg).find('active').text();
                    if (active == 1)
                    {
                        jQuery('#modDomain').find('#actif').attr('checked','checked');
                    } else {
                        jQuery('#modDomain').find('#actif').attr('checked',false);
                    }
                }
            });
        }
    ...
    jQuery('#delDialog').dialog({
        ...
        minWidth: 560,
        modal: true,
        title: 'Suppression du domaine',
        buttons:{
            'OK':function() {
                jQuery.ajax({
                    url: 'index.php',
                    data: 'action=del&id='+domainid,
                    datetype: 'xml',
                    type: 'post',
                    success: function(msg){
    ...
    jQuery('#list2').jqGrid({
        ...
        viewrecords: true,
        sortorder: 'desc',
        caption:'".$langs->trans('DomainNames')."' });
...
function deleteDomain(pId)
{
    domainid=pId;
    jQuery('#delDialog').dialog('open');
}
function modifyDomain(pId)
{
    domainid=pId;
...
";
if ($_REQUEST['action'] == "mod" && $user->rights->domain->creer)
{
...
if ($_REQUEST['action'] == "getData" && $user->rights->domain->lire)
{
    $domainId = $_REQUEST['id'];
    if ($domainId > 0)
    {
        $requete = "SELECT label, note, active,
                      ...
                      FROM Babel_domain WHERE rowid = ".$domainId;
        $sql = $db->query($requete);
...
if ($_REQUEST['action'] == 'list'&& $user->rights->domain->lire)
{
    $page = $_GET['page'];
    $limit = $_GET['rows'];
    $sidx = $_GET['sidx'];
    $sord = $_GET['sord'];
    if(!$sidx) $sidx =1;
    $result = $db->query("SELECT COUNT(*) AS count FROM Babel_domain");
    ...
    $SQL = "SELECT * FROM Babel_domain  ORDER BY $sidx $sord LIMIT $start , $limit";
    ...
    while($row = $db->fetch_object($result,MYSQL_ASSOC))
    {
        ...
        $responce->rows[$i]['cell']=array($row->rowid,
                                           ...
                                           " <center>   <table style='margin-top: -20px;' cellpadding=0><tr><t><td style='background:none repeat scroll 0 0 transparent;border:0 none transparent;max-width:16px;width:16px; height: 16px;max-height:16px;' valign=center><span title='Modifier' style='width: 16px;' class='ui-icon ui-icon-arrowrefresh-1-e' onclick='modifyDomain(".$row->rowid.")'></span><td style='background:none repeat scroll 0 0 transparent;border:0 none transparent;max-width:16px;width:16px; height: 16px;max-height:16px;' valign=center><span title='Supprimer' style='width: 16px;' class='ui-icon ui-icon-trash' onclick='deleteDomain(".$row->rowid.")'></span></table></center>");
        $i++;
    }
...
if ($_REQUEST['action'] == 'del'&& $user->rights->domain->effacer)
{
    $xml = "<ajax-response>";
    $id = $_REQUEST['id'];
    $requete = "DELETE FROM Babel_domain WHERE rowid = ".$id;
    $sql = $db->query($requete);
    if ($sql)
    {
        $xml .= "<OK>OK</OK>";
    ...
    llxHeader($js,'Domaines',"",1);
    if (($_REQUEST['action']=='add') && $_REQUEST['cancel']!= 1 && $user->rights->domain->creer)
    {
        $datec = parseDate($_REQUEST['datec']);
        $datef = parseDate($_REQUEST['datef']);
        $dater = parseDate($_REQUEST['dater']);
        $requete = "INSERT INTO `Babel_domain`
    ...
    if (($_REQUEST['action']=="create" || $_REQUEST['addNew']== 1) && $user->rights->domain->creer)
    {
          print "<br/><div class='titre'>".$langs->trans("NewDomain")."</div><br/>";
          print "<form method='POST' action='index.php' id='createDomain'>";
          ...
          print "<tr><td align=center colspan=2 class='ui-widget-content'><button class='ui-button ui-state-default ui-widget-header ui-corner-all' name='addStop' value='1' style='padding: 5px 10px;'>Ajouter</button>
                                                                          <button class='ui-button ui-state-default ui-widget-header ui-corner-all' name='addNew' value='1' style='padding: 5px 10px;'>Ajouter et Creer nouveau</button>
                                                                          <button class='ui-button ui-state-default ui-widget-header ui-corner-all' name='cancel' value='1' style='padding: 5px 10px;'>Annuler</button>";
          print "</thead></table>";
          print "</form>";
          exit;
    }
...
//
        print "<br>";
        print "<div class='titre'>".$langs->trans("DomainNames")."</div>";
        print "<br>";
    ...
    print '<table id="list2"></table> <div id="pager2"></div>';
        ...
        print "<br/>";
        if ($user->rights->domain->creer)
        {
            print "<hr/>";
            print "<div id='tabBar' style='float: right;'>";
            print "<form action='index.php?action=create' method='POST'>
                        <button style='padding: 5px 10px' class='ui-widget-header ui-state-default ui-corner-all ui-button' >".$langs->trans('NewDomain')."</button></form>";
            print "</div>";
        }
...
print "<div id='delDialog'>&Ecirc;tes vous sur de vouloir supprimer ce domaine ?</div>";

print "<div id='modDialog'>";
      print "<form method='POST' action='index.php' id='modDomain'>";
