...
class Societe extends CommonObject
{
	...
	public function id_prof_url($idprof, $thirdparty)
	{
		...
		if (empty($reshook)) {
			...
			if ($idprof == 2 && $thirdparty->country_code == 'FR') {
				$url='http://www.societe.com/cgi-bin/search?champs='.$strippedIdProf2;    // See also http://avis-situation-sirene.insee.fr/
			}
		...
		if (!$error) {
			$this->db->begin();
                        ...
                        while($ln = $this->db->fetch_object($sql)){
                            $this->db->query('UPDATE `'.MAIN_DB_PREFIX.'product_fournisseur_price` SET ref_fourn = CONCAT(ref_fourn, "-B") WHERE fk_product = '.$ln->fk_product.' AND ref_fourn = "'.$ln->ref_fourn.'" AND fk_soc = '.$soc_origin_id.' ');
                        }
                        /* fmoddrsi*/
