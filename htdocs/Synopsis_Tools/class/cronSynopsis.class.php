...
//global $oldPref, $nbIframeMax, $nbIframe, $nbErreur;
    ...
    function majSAv($mailTech = false) {
        ...
        $this->verifOBj("Propal", "Propal sans Sav", "SELECT * from llx_propal where rowid not in (select propalid from llx_synopsischrono_view_105) AND extraparams is null AND ref LIKE 'PR%'", 'ligneSav');
        $this->verifOBj("Facture", "Facture sans propal", "select * from llx_facture where rowid not in (SELECT f.rowid from llx_facture f, llx_element_element ee, llx_propal p WHERE p.rowid = ee.fk_source AND f.rowid = ee.fk_target AND ee.sourcetype = 'propal' AND ee.targettype = 'facture') AND close_code is null ");
