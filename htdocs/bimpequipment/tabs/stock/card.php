...
else
{
	...
	if ($id)
	{
		...
		if ($action <> 'edit' && $action <> 're-edit')
		{
			...
			dol_fiche_head($head, 'bimpstock', $langs->trans("Warehouse"), 0, 'stock');
...
//			if ($user->rights->stock->creer)            print_liste_field_titre('');
			...
			$sql.= " FROM ".MAIN_DB_PREFIX."product as p";
                        $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."product_stock as ps ON ps.fk_product = p.rowid AND ps.fk_entrepot = ".$object->id;
			$sql.= " WHERE 1";
