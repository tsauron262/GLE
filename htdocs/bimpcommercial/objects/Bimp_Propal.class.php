...
//                }
	...
	public function isDemandeFinAllowed(&$errors = array())
	{
		...
		if ((int) BimpCore::getConf('allow_df_from_propal_dev_only', null, 'bimpcommercial') && !BimpCore::isUserDev()) {
			$errors[] = 'Demandes de location à partir des devis actives seulement pour les devs';
			return 0;
		}
...
//                            $onclick = 'window.location = \'' . $url . '\'';
					...
					if ($status > 0) {
						...
						if ($this->isActionAllowed('review', $errors)) {
							if ($this->canSetAction('review')) {
								$params = array();
								if ($this->isProductsPricesUpdateAllowed()) {
									$params['form_name'] = 'review_propal';
								} else {
									$params['confirm_msg'] = 'Veuillez confirmer la mise en révision ' . $this->getLabel('of_this');
								}
								$buttons[] = array(
									'label'   => 'Réviser',
									'icon'    => 'fas_undo',
									'onclick' => $this->getJsActionOnclick('review', array(), $params)
								);
							} else {
		...
		if ($this->isActionAllowed('getPropositionLocation') && $this->canSetAction('getPropositionLocation')) {
			$df_buttons[] = array(
		...
		if (!empty($signature_buttons || !empty($df_buttons))) {
			...
			if (BimpObject::objectLoaded($line)) {
				if ($by_linked_contrat) {
					...
					if ($id_contrat_line) {
						...
						if (!$id_contrat) {
							$line_errors[] = 'Ajout à un abonnement en cours : le contrat n\'existe plus. Veuillez corriger';
						} else {
							if (!isset($lines[$id_contrat])) {
								$lines[$id_contrat] = array();
							}

							$lines[$id_contrat][$id_line] = $line;
							continue;
						}
					}
...
//                        ), "a");
	...
	public function renderContratAboInput()
	{
		...
		if (empty($contrats)) {
			...
			$has_linked_lines = false;
			$id_def_contrat = 0;

			foreach ($contrats as $id_contrat => $lines) {
				if (!$id_contrat || empty($lines)) {
					continue;
				}

				$has_linked_lines = true;
				if ($id_def_contrat < $id_contrat) {
					$id_def_contrat = $id_contrat;
				}
			}
		}
		...
		if (count($errors)) {
			...
			if (isset($contrats[0]) && count($contrats[0])) {
				if (!$id_def_contrat) {
					$id_def_contrat = (int) $this->getDefaultContratAbo();
				}

				$html .= '<tr>';
				$html .= '<td colspan="2"><b>Lignes non assignées à un abonnement en cours</b></td>';
				$html .= '</tr>';

				$html .= '<tr>';
				$html .= '<td><ul>';
				foreach ($contrats[0] as $line) {
					$html .= '<li>' . $line->displayLineData('desc_light') . '</li>';
				}

				$html .= '</ul></td>';
				$html .= '<td>';

				$client_contrats = $this->getClientContratsAboArray();
				$html .= BimpInput::renderInput('select', 'id_contrat', $id_def_contrat, array(
					'options' => $client_contrats
				));
	...
	public function review($check = true, &$errors = array(), &$warnings = array(), $is_refus = false, $update_products_prices = false)
	{
		global $user, $langs;
		...
		$errors = BimpTools::merge_array($errors, $new_propal->createLinesFromOrigin($this, array(
			...
			'update_products_prices'    => (int) $update_products_prices
		)));
...
//                $contrat_line->delete($w, true);
	...
	public function actionReview($data, &$success)
	{
		...
		$update_products_prices = BimpTools::getArrayValueFromPath($data, 'update_products_prices', 0);
