...
if (isset($_REQUEST['appli']) && $_REQUEST['appli'] == "Oui") {
    ...
    while ($ligneModel = $db->fetch_object($resultModel)) {
        ...
        $nbVue = 1;
//    $view = "" . MAIN_DB_PREFIX . "synopsischrono_view_" . traiteCarac($ligneModel->id, "") . "";
...
//                continue;
//            $createView3 .= ' LEFT JOIN " . MAIN_DB_PREFIX . "synopsischrono_value tab' . $i . ' on tab' . $i . '.chrono_refid = chrono.id AND tab' . $i . '.key_id = ' . $resPre->id;
//            if ($resPre->type_valeur == 8) {//Liste
//                $createView3 .= ' LEFT JOIN " . MAIN_DB_PREFIX . "Synopsis_Process_form_list_members tab' . $i . 'list ON tab' . $i . 'list.list_refid = ' . $resPre->type_subvaleur . ' AND tab' . $i . 'list.valeur = tab' . $i . '.value';
//                $createView2 .= ',tab' . $i . 'list.label as ' . $resPre->nom . ', tab' . $i . 'list.valeur as ' . $resPre->nom . "Val";
//            } else {
//                $createView2 .= ',tab' . $i . '.value as ' . $resPre->nom;
//            }
//        }
//        $requeteView = "Create view " . $view . " as (SELECT chrono.* " . $createView2 . " FROM " . MAIN_DB_PREFIX . "synopsischrono chrono" . $createView3 . " WHERE model_refid = " . $ligneModel->id . ");";
...
//        $res2 = $db->query("DESCRIBE " . MAIN_DB_PREFIX . "synopsischrono");
