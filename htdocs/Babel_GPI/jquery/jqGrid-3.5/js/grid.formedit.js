...
//                    //Babel Modif
                    var dc;
                    if (this.hidedlg==true || this.hidedlg=="undefined")
                    {
                        dc = hc ? "style='display:none'" : "";
                    }
//                     else {
//                        dc = hc ? "style='display:inline'" : "";
//                    }
                ...
                $('table:first tr#'+rowid+' td',obj.grid.bDiv).each( function(i) {
                    ...
                    if ( nm !== 'cb' && nm !== 'subgrid' && obj.p.colModel[i].editable===true) {
                        ...
                        switch (obj.p.colModel[i].edittype) {
                            ...
                            case "select":
                                tmp = $.jgrid.htmlDecode(tmp);
                                var tmp1 = tmp.replace(/(<([^>]+)>)/ig,"");
                                    tmp1 = tmp1.replace(/^[\s]*/ig,"");
                                    tmp1 = tmp1.replace(/[\s]*$/ig,"");
                                $("#"+nm+" option","#"+fmid).each(function(j){
                                    if (!obj.p.colModel[i].editoptions.multiple && (tmp == $(this).text() || tmp1 == $(this).text() )){
                                        this.selected= true;
