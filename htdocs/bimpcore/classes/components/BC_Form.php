...
class BC_Form extends BC_Panel
{
	...
	public static $custom_row_params = array(
		...
		'reload_now'      => array('data_type' => 'bool', 'default' => 1),
...
//        $this->params_def['sub_objects'] = array('type' => 'keys');
	...
	public function renderHtmlContent($form_tag = true)
	{
		...
		if (is_null($this->config_path) || count($this->params['rows']) == 0 || $this->params['all_fields']) {
...
//        }
	...
	public function renderCustomRow($row, $params = array(), $label_cols = 3)
	{
		...
		if (!is_null($params['depends_on'])) {
			...
			$html .= BC_Field::renderDependsOnScriptStatic($this->object, $this->identifier, $params['input_name'], $params['depends_on'], $this->fields_prefix, $keep_new_value/*TODO bug pas de reload a l'initialiation du form ex : deplacer paiement bimpcommercial/index.php?fc=factures&tab=paiements, (int) $this->params['reload_now']*/);
		}
	...
	public static function renderReloadInputBtn($input_name, $keep_new_value = true)
	{
		$onclick = 'reloadObjectInput($(this).findParentByClass(\'object_form\').attr(\'id\'), \'' . $input_name . '\', {}, ' . ($keep_new_value ? 'true' : 'false') . ')';
		return '<div style="text-align: right; margin-top: 10px">' . BimpRender::renderButton(array(
				'label'   => 'Actualiser',
				'icon'    => 'fas_redo',
				'onclick' => $onclick
			)) . '</div>';
