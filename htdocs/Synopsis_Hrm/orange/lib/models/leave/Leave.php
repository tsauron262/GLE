...
class Leave {
    ...
    */
    ...
    public function fetchAllEmployeeLeaveSummary( $year, $leaveTypeId = 0, $searchBy="employee", $sortField=null, $sortOrder=null, $hideDeleted=false, $pageNO=0, $itemPerPage=0, $leaveCount = FALSE) {
        ...
        $joinConditions[2] = "d.`employee_id` = a.`emp_number` AND c.`leave_type_id` = d.`leave_type_id` AND (d.`leave_status` = " . self::LEAVE_STATUS_LEAVE_TAKEN . " OR d.`leave_status` = " . self::LEAVE_STATUS_LEAVE_APPROVED . ") AND d.`leave_date` BETWEEN DATE('".$year."-01-01') AND DATE('".$year."-12-31')";

        $groupBy = "emp_number, employee_name, leave_type_id, leave_type_name, no_of_days_allotted, available_flag";

        $selectConditions = null;

        if ( $searchBy == "employee" && !empty($employeeId) && ($employeeId != 0)) {
            $selectConditions[] = "a.`emp_number` = {$employeeId}";
        }
        if ( $searchBy == "leaveType" && !empty($leaveTypeId) && ($leaveTypeId != 0)) {
            $selectConditions[] = "b.`leave_type_id` = '{$leaveTypeId}'";
        }
        if ( $searchBy == "both" && !empty($leaveTypeId) && ($leaveTypeId != 0)) {
