...
class BimpCommDemandeFin extends BimpObject
{
		...
		if (is_null($this->origine)) {
			if (!$this->isLoaded()) {
				...
				if ($type_origine && $id_origine) {
					$this->origine = $this->getOrigineFromType($type_origine, $id_origine);
				}
			} else {
				$this->origine = $this->getParentInstance();
			}
		}
	...
	public function getIdClient()
	{
		...
		if (BimpObject::objectLoaded($origine)) {
			return (int) $origine->getData('fk_soc');
		}
	...
	public function getClient()
	{
		if (is_null($this->client)) {
			$id_client = $this->getIdClient();
			if ($id_client) {
				$this->client = BimpCache::getBimpObjectInstance('bimpcore', 'Bimp_Client', $id_client);
			}
		}
	...
	public function getInputValue($input_name)
	{
		...
		switch ($input_name) {
			...
			case 'fonction_signataire':
				$id_contact = (int) BimpTools::getPostFieldValue('id_contact_signature', 0, 'int');
				if ($id_contact) {
					return $this->db->getValue('socpeople', 'poste', 'rowid = ' . $id_contact);
				}
				return '';
			...
			case 'forme_juridique':
				...
				if (BimpObject::objectLoaded($client)) {
					return $client->displayDataDefault('fk_forme_juridique');
				}
				return '';
	...
	public function actionCreateDemandeFinancement($data, &$success)
	{
		...
		if (!count($errors)) {
			...
			if (!count($errors)) {
				...
				$phone_signataire = BimpTools::getPostFieldValue('phone_signataire', (BimpObject::objectLoaded($contact_signature) ? $contact_signature->getPhoneMobile(true) : ''), 'alphanohtml');
				...
				if (!count($errors)) {
					$forme_juridique = BimpTools::getArrayValueFromPath($data, 'forme_juridique', $client->displayDataDefault('fk_forme_juridique', 1, 1));
					$capital = BimpTools::getArrayValueFromPath($data, 'capital', $client->getData('fk_forme_juridique'));
					$rcs = BimpTools::getArrayValueFromPath($data, 'rcs', '');
					...
					if ($client->getData('address') && $client->getData('zip') && $client->getData('town')) {
						...
						if (empty($demande_data['client']['address']['address'])) {
							$errors[] = 'Adresse du client absente - Veuillez complèter la fiche du client';
						}
						if (empty($demande_data['client']['address']['zip'])) {
							$errors[] = 'Code postal du client absent - Veuillez complèter la fiche du client';
						}

						if (empty($demande_data['client']['address']['town'])) {
							$errors[] = 'Ville du client absent - Veuillez complèter la fiche du client';
						}
						...
						if (empty($demande_data['client']['address']['address'])) {
							$errors[] = 'Adresse du contact de suivi absente - Veuillez complèter la fiche du contact';
						}
						if (empty($demande_data['client']['address']['zip'])) {
							$errors[] = 'Code postal du contact de suivi absent - Veuillez complèter la fiche du contact';
						}

						if (empty($demande_data['client']['address']['town'])) {
							$errors[] = 'Ville du contact de suivi absent - Veuillez complèter la fiche du contact';
						}
					}
