...
class GSX_v2 extends GSX_Const
{
    ...
    public function authenticate()
    {
        ...
        if (isset($result['authToken'])) {
            ...
            global $user, $langs;
            mailSyn2('auth GSX', 'tommy@bimp.fr', null, $user->getFullName($langs).' '.$this->appleId.' auth OK' . date('l jS \of F Y h:i:s A'));
            $this->logged = true;
            return 1;
        }
        ...
        $this->initError('Echec authentification (token ' . $this->acti_token . ')');
            ...
            global $user, $langs;
            mailSyn2('auth GSX bad', 'tommy@bimp.fr', null, $user->getFullName($langs).' '.$this->appleId.' auth bad'. date('l jS \of F Y h:i:s A'));
        $this->logged = false;
        $this->saveToken('acti', '');
