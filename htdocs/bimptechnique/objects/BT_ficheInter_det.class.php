...
class BT_ficheInter_det extends BimpDolObject
{
	...
	const TYPE_DEPLACEMENT_INTERNE = 8;
	...
	public static $types = [
		...
		self::TYPE_DEPLACEMENT_INTERNE  => ['label' => "Déplacement en interne", 'icon' => 'fas_car', 'classes' => ['success']],
...
//        }
	...
	public function getInputTypesArray()
	{
		...
		if (BimpObject::objectLoaded($fi)) {
			...
			if (in_array((int) $fi->getData('fk_soc'), $array_serv_interne)) {
				return array(
					...
					(int) self::TYPE_DEPLACEMENT_INTERNE => self::$types[self::TYPE_DEPLACEMENT_INTERNE]
				);
			}
		}
		...
		unset($types[self::TYPE_DEPLACEMENT_INTERNE]);
...
//print_r($product);
	...
	private function getTypePlanningCode(): int
	{
		...
		switch ($this->getData('type')) {
			...
			case self::TYPE_DEPLACEMENT_INTERNE:
				$code = "REU_INT";
		}
...
//                return '14:00';
	...
	public function display_service_ref($with_details_commande_line = true)
	{
		...
		if ($fk_product > 0) {
			...
			$parent = $this->getParentInstance();
			if (in_array($this->getData('type'), array(self::TYPE_INTERNE, self::TYPE_DEPLACEMENT_INTERNE))) {
		...
		if (!BimpObject::objectLoaded($fi)) {
			...
			if ($contrat->isContratDelegation()) {
				...
				if ($heuresFaites > $heuresRestantes) {
					...
					BimpUserMsg::envoiMsg($code, $sujet, $message, $contrat);
//					mailSyn2($sujet, $commercial->getData('email') . ', contrat@bimp.fr', null, $message); // $contrat->getCommercialClient(true);
				}
			}
		}
