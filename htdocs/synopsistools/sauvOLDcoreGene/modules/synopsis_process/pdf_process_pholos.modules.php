...
class pdf_process_pholos extends ModeleNumRefProcess
{
            ...
            if (file_exists($dir))
            {
                ...
                for ($i = 0 ; $i < $nblignes ; $i++)
                {
                    ...
                    if ($com->lines[$i]->date_start && $com->lines[$i]->date_end)
                    {
                        ...
                        $libelleproduitservice.="<br>".dol_htmlentitiesbr("(".$outputlangs->transnoentities("From")." ".dol_print_date(strtotime($com->lines[$i]->date_start))." ".$outputlangs->transnoentities("to")." ".dol_print_date(strtotime($com->lines[$i]->date_end)).")",1);
                    }
    ...
    */
    function _pagehead(&$pdf, $object, $showadress=1, $outputlangs)
    {
        ...
        $pdf->SetXY(100,$posy);
        $pdf->SetTextColor(0,0,60);
        $pdf->MultiCell(100, 3, $outputlangs->transnoentities("OrderDate")." : " . dol_print_date(strtotime($object->date),"%d %b %Y"), '', 'R');
