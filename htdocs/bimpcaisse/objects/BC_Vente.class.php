...
//                        'VIR' => 'Virement bancaire'
    ...
    public function findArticleToAdd($search, &$errors = array())
    {
        ...
        $sql .= ' WHERE (p.barcode = "' . $search . '" OR p.ref LIKE "%' . $search . '%" OR p.label LIKE "%' . $search . '%") AND tosell = 1';
            ...
            if (count($returns)) {
                ...
                foreach ($returns as $return) {
                    ...
                    if (BimpObject::objectLoaded($equipment)) {
                        ...
                        $stock_errors = array();
                        $label = ((int) $return->getData('type') === BC_VenteReturn::TYPE_RACHAT ? 'Rachat' : 'Retour') . ' produit Vente #' . $this->id;
...
//            $line->desc = $product->getName() . ' - Réf. ' . $product->getRef() . ($serial ? ' - N° de série: ' . $serial : '');
        ...
        if (!$is_avoir) {
            ...
            if ($total_paid > $total_facture_ttc) {
                ...
                $facture->updateField('paiement_status', 2);
            } else {
                ...
                if ($diff < 0.01 && $diff > -0.01) {
                    ...
                    $facture->updateField('paiement_status', 1);
                }
            }
        } else {
