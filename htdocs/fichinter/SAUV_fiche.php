...
if ($_REQUEST["action"] == 'create')
{
    ...
    */
    ...
    $fichinter = new Fichinter($db);
    $fichinter->date = time();
    if ($fichinterid) $result=$fichinter->fetch($fichinterid);
    ...
    if ($_REQUEST["socid"])
    {
        ...
        if ($conf->projet->enabled)
        {
            ...
            if ($_REQUEST["socid"]) $numprojet = $societe->has_projects();
...
EOF;
        ...
        if ($conf->projet->enabled)
        {
            ...
            if ($socid) $numprojet = $societe->has_projects();
...
//        }
    ...
    if ($conf->projet->enabled)
    {
        ...
        if (! $numprojet)
        {
            ...
            {
                if ($_REQUEST['action'] != 'classer' && $fichinter->brouillon) print '<a href="'.$_SERVER['PHP_SELF'].'?action=classer&amp;id='.$fichinter->id.'">'.img_edit($langs->trans('SetProject'),1).'</a>';
                print '</th><td colspan="3" class="ui-widget-content">';
  ...
  /*  $colspan=3;
    ...
    if ($_REQUEST['action'] != 'editdescription' && $fichinter->brouillon) print '<a href="'.$_SERVER["PHP_SELF"].'?action=editdescription&amp;id='.$fichinter->id.'">'.img_edit($langs->trans('SetDescription'),1).'</a>';
    print '</th><td colspan="3" class="ui-widget-content">';
