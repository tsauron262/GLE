...
$sql = $db->query("SELECT c.id, c.ref FROM `" . MAIN_DB_PREFIX . "synopsischrono` c, " . MAIN_DB_PREFIX . "synopsischrono_chrono_105 c105, " . MAIN_DB_PREFIX . "element_element e1, " . MAIN_DB_PREFIX . "propal p, " . MAIN_DB_PREFIX . "facture f"
        . " WHERE c105.Suivie = '' AND c105.id = c.id AND fk_source = p.rowid AND sourcetype = 'propal' AND targettype = 'facture' AND fk_source = f.rowid AND c.propalid = p.rowid AND `facnumber` LIKE 'FA%' "
        . "AND DATEDIFF(f.`date_valid`, now()) < -2 AND DATEDIFF(f.`date_valid`, now()) > -7");
