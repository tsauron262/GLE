...
//        $sql2 = $db->query("SELECT * FROM " . MAIN_DB_PREFIX . "synopsistools_notificationAjax WHERE vue = 0 AND idPagePrinc =".$_REQUEST['idPagePrinc']);
...
//            $return .= "<echo>" . $result->idElem . "hihi</echo>";
                ...
                $db->query("UPDATE " . MAIN_DB_PREFIX . "synopsistools_notificationAjax SET vue = 1 WHERE id =" . $result->id);
                $changement = true;
        }*/
...
if (isset($_REQUEST['idVue']) && $_REQUEST['idVue'] > 0) {
//boucle();
    echo "<ok>ok</ok>";
    die();
}
...
if (isset($_REQUEST['callBack'])) {
    $db->query("UPDATE " . MAIN_DB_PREFIX . "synopsistools_notificationAjax SET callBack = 1 WHERE id =" . $_REQUEST['callBack']);
}
if (isset($_REQUEST['ouvreId']) && isset($_REQUEST['ouvreType'])) {
    $db->query("INSERT INTO " . MAIN_DB_PREFIX . "synopsistools_notificationAjax (idPagePrinc, typeElem, idElem) VALUES ('".$_REQUEST['idPagePrinc']."', '" . $_REQUEST['ouvreType'] . "', '" . $_REQUEST['ouvreId'] . "')");
    boucleCallBack($db->last_insert_id(MAIN_DB_PREFIX . "synopsistools_notificationAjax"));
}

function boucleCallBack($id) {
//    global $db;
//    sleep(1);
//    $sql2 = $db->query("SELECT * FROM " . MAIN_DB_PREFIX . "synopsistools_notificationAjax WHERE callBack = 1 AND id =" . $id);
//    if ($db->num_rows($sql) > 0)
