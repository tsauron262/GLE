...
$datecons 		= dol_mktime(GETPOST('deohour','int'), GETPOST('deomin','int'), 0, GETPOST('deomonth','int'), GETPOST('deoday','int'), GETPOST('deoyear','int'));
...
if ($action == "addline" && $user->rights->equipement->creer)
{
	...
	if (! $error)
	{
		...
		if ($result >= 0)
		{
			// on vérifie si le produit est un équipements dans l'entrepot (saisie supplémentaire possible alors)
...
if ($id > 0 || ! empty($ref))
{
	...
	{
		$ret=$form->form_confirm($_SERVER["PHP_SELF"].'?id='.$object->id.'&line_id='.GETPOST('line_id','int'), $langs->trans('DeleteConsumptionLine'), $langs->trans('ConfirmDeleteconsumptionLine'), 'confirm_deleteline','',0,1);
	...
	if ($resql)
	{
		...
		while ($i < $num)
		{
			...
			if ($action == 'editline' && $user->rights->equipement->creer && GETPOST('line_id','int') == $objp->rowid)
			{
				...
				$doleditor=new DolEditor('np_desc',($objp->description ? $objp->description :GETPOST('np_desc')),'',100,'dolibarr_details','',false,true,$conf->global->FCKEDITOR_ENABLE_DETAILS,ROWS_6,60);
				$doleditor->Create();
				print '</td>';
...
dol_fiche_end();
		...
		if ($action <> 'editline' && $user->rights->equipement->creer )
		{
			...
			// editeur wysiwyg
			require_once(DOL_DOCUMENT_ROOT."/core/class/doleditor.class.php");
			$doleditor=new DolEditor('np_desc',($predefdescription ? $predefdescription :''),'',100,'dolibarr_details','',false,true,$conf->global->FCKEDITOR_ENABLE_DETAILS,ROWS_6,60);
			$doleditor->Create();
			print '</td>';
