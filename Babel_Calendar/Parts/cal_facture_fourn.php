            ...
            while($res=$db->fetch_object($resql))
            {
                ...
                if ($res->datec && $showFactureFournCreate)
                {
                   $arrRes = $BCalc->pushDateArr(
                                          ...
                                          "Cr&eacute;ation de la facture fournisseur" . $res->facnumber . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                          $res->ref,
                                          $id,
                                          "FactureFourn",
                                          1,
                ...
                if ($res->datef && $showFactureFournDate)
                {
                    $arrRes = $BCalc->pushDateArr(
                                          $res->datef,
                                          "Date fact fourn " . "".$res->facnumber."" . " (".$soc->nom.")",
                                          "Facture fournisseur " . $res->facnumber . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                          $res->ref,
                                          $id,
                                          "FactureFourn",
                                          1,
                ...
                if ($res->date_valid && $showFactureFournValid)
                {
                    $arrRes = $BCalc->pushDateArr(
                                          ...
                                          "Validation de la Facture Fournisseur " . $res->facnumber . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                          $res->ref,
                                          $id,
                                          "FactureFourn",
                                          1,
                ...
                if ($res->date_lim_reglement && $showFactureFournDateLim)
                {
                    $arrRes = $BCalc->pushDateArr(
                                          ...
                                          "Date limite du réeacute;glement de la Facture fournisseur " . $res->facnumber . "<BR><P>" . $res->note. "<BR><P>" . $res->note_public,
                                          $res->ref,
                                          $id,
                                          "FactureFourn",
                                          1,
