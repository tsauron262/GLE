...
if (isset($_REQUEST['saveForm'])) {
    if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0)
        $req = "UPDATE " . MAIN_DB_PREFIX . "synopsistools_notificationUser SET fk_type_contact = " . $_REQUEST['typeContact'] . ", fk_trigger = " . $_REQUEST['fk_trigger'] . ", sujet = '" . addslashes($_REQUEST['sujet']) . "', message = '" . addslashes($_REQUEST['text']) . "', mailTo = '" . addslashes($_REQUEST['mailTo']) . "' WHERE rowid = " . $_REQUEST['id'];
    else
        $req = "INSERT INTO " . MAIN_DB_PREFIX . "synopsistools_notificationUser (fk_type_contact, fk_trigger, sujet, message, mailTo, active) VALUES (" . $_REQUEST['typeContact'] . "," . $_REQUEST['fk_trigger'] . ",'" . addslashes($_REQUEST['sujet']) . "','" . addslashes($_REQUEST['text']) . "','" . addslashes($_REQUEST['mailTo'])."',1)";
    if (!$db->query($req))
        die("Erreur : " . $req);
...
if (isset($_REQUEST['delElem'])) {
    if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0)
        $req = "DELETE  FROM " . MAIN_DB_PREFIX . "synopsistools_notificationUser WHERE rowid = " . $_REQUEST['id'];
    if (!$db->query($req))
        die("Erreur : " . $req);
    header("Location: list.php");
}
...
if($_REQUEST['id'] > 0){
    $sql = $db->query("SELECT *, nu.rowid as idNU
        FROM  `" . MAIN_DB_PREFIX . "synopsistools_notificationUser` nu
...
echo "<a href='" . DOL_URL_ROOT . "/synopsistools/notificationUser/list.php'><input type='button' class='butAction' name='cancel' value='Annuler'/></a>";
