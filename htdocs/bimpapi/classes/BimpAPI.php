	...
	public static function getApiInstance($api_name, $api_idx = 0)
	{
		if (!isset(self::$instances[$api_name][$api_idx])) {
			...
			if (BimpCore::getExtendsVersion()) {
				if (file_exists(DOL_DOCUMENT_ROOT . '/bimpapi/extends/version/' . BimpCore::getExtendsVersion() . '/classes/apis/' . $api_class . '.php')) {
					$final_class = $api_class . '_ExtVersion';
					require_once DOL_DOCUMENT_ROOT . '/bimpapi/extends/version/' . BimpCore::getExtendsVersion() . '/classes/apis/' . $api_class . '.php';
				}
			}
...
//            echo BimpTools::displayBacktrace();
				...
				if (!$ch) {
					...
					switch ($params['type']) {
						...
						case 'PUT':
							...
							break;
						case 'POST':
							if(!is_array($curl_options[CURLOPT_POSTFIELDS])){
								$curl_options[CURLOPT_HTTPHEADER][] = 'Content-Length: ' . strlen($curl_options[CURLOPT_POSTFIELDS]);
								$curl_options_str['CURLOPT_HTTPHEADER'][] = 'Content-Length: ' . strlen($curl_options[CURLOPT_POSTFIELDS]);
							}
							break;
		...
		if ($this->options['log_requests']) {
			BimpCore::addlog('Requête API ' . $this->options['public_name'] . ': "' . $request_name . '"', Bimp_Log::BIMP_LOG_NOTIF, 'api', $this->apiObject, array(
				'Requête' => str_replace('<br/>', "\n", str_replace('Array', "", $infos))
			));
			dol_syslog($infos);
		}
