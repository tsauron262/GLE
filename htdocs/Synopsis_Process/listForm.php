...
jQuery(document).ready(function() {
    var grid = jQuery("#gridListForm").jqGrid({
            datatype: "json",
            url: "ajax/listForm_json.php?",
            colNames:['id', "Label",'Description','&Eacute;tat'],
EOF;
    $jqgridJs .= <<<EOF
...
$jqgridJs .= <<<EOF
            ...
            rowList:[30,50,100],
            imgpath: gridimgpath,
            pager: jQuery('#gridListFormPager'),
            sortname: 'label',
            beforeRequest: function(){
                jQuery('#gview_gridListForm').find('.ui-jqgrid-titlebar').addClass('ui-state-default');
            },
...
$jqgridJs .= <<<EOF
            ...
            sortorder: "asc",
            //multiselect: true,
            caption: "<span style='padding:4px; font-size: 16px;'>Formulaires</span>",
            viewsortcols: true,
            ondblClickRow: function(e,u){
                location.href='formBuilder.php?id='+e;
            },
EOF;

    $jqgridJs.= '    }).navGrid("#gridListFormPager",'."\n";
...
llxHeader($js,'Liste formulaires','',1);


  print '<table id="gridListForm" class="scroll ui-widget " cellpadding="0" cellspacing="0"></table>';
  print '<div id="gridListFormPager" class="scroll" style="text-align:center;"></div>';
