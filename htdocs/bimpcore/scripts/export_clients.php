...
$sql .= ' AND (';
$sql .= '(SELECT COUNT(p.rowid) FROM llx_propal p WHERE p.fk_soc = s.rowid AND p.datec > \'2016-06-18 00:00:00\') > 0';
$sql .= ' OR (SELECT COUNT(c.rowid) FROM llx_commande c WHERE c.fk_soc = s.rowid AND c.date_creation > \'2016-06-18 00:00:00\') > 0';
$sql .= ' OR (SELECT COUNT(f.rowid) FROM llx_facture f WHERE f.fk_soc = s.rowid AND f.datec > \'2016-06-18 00:00:00\') > 0';
$sql .= ' OR (SELECT COUNT(ct.rowid) FROM llx_contrat ct WHERE ct.fk_soc = s.rowid AND ct.datec > \'2016-06-18 00:00:00\') > 0';
$sql .= ' OR (SELECT COUNT(sav.id) FROM llx_bs_sav sav WHERE sav.id_client = s.rowid AND sav.date_create > \'2016-06-18 00:00:00\') > 0';
$sql .= ' OR (SELECT COUNT(t.id) FROM llx_bs_ticket t WHERE t.id_client = s.rowid AND t.date_create > \'2016-06-18 00:00:00\') > 0';
$sql .= ')';
...
if (is_null($rows)) {
    ...
    foreach ($rows as $r) {
        $str .= '"' . $r['code_client'] . '";"' . $r['code_comta'] . '";"' . $r['nom'] . '";"' . "\n";
    }
