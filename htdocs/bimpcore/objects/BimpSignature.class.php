		...
		if (!count($signataires)) {
			...
			foreach ($signataires as $signataire) {
				...
				if (!empty($signature_params_errors)) {
					...
					if (isset($signature_params['extra_signatures'])) {
						$extra_signature_idx = 1;
						foreach ($signature_params['extra_signatures'] as $extra_signature) {
							$extra_signature_idx++;
							$extra_signature_params = array(
								'name'          => 'Signature ' . $extra_signature_idx,
								'anchorString'  => $extra_signature['anch'],
								'anchorXOffset' => (isset($extra_signature['x']) ? $extra_signature['x'] : 0),
								'anchorYOffset' => (isset($extra_signature['y']) ? $extra_signature['y'] : 0)
							);

							if (isset($extra_signature['p'])) {
								$extra_signature_params['pageNumber'] = $extra_signature['p'];
							}

							$params['tabs']['signHereTabs'][] = $extra_signature_params;

							if (isset($extra_signature['date'])) {
								if (!isset($params['tabs']['dateSignedTabs'])) {
									$params['tabs']['dateSignedTabs'] = array();
								}

								$params['tabs']['dateSignedTabs'][] = array(
									'name'          => "Date signature " . $extra_signature_idx,
									'anchorString'  => $extra_signature['anch'],
									'anchorXOffset' => BimpTools::getArrayValueFromPath($extra_signature, 'date/x', 0),
									'anchorYOffset' => BimpTools::getArrayValueFromPath($extra_signature, 'date/y', 0),
									'fontSize'      => BimpTools::getArrayValueFromPath($extra_signature, 'date/fs', $font_size)
								);
							}
						}
					}

					if (isset($signature_params['paraphe'])) {
						$params['tabs']['initialHereTabs'] = array(
							array(
								'name'          => "Paraphe",
								'anchorString'  => (isset($signature_params['paraphe']['anch']) ? $signature_params['paraphe']['anch'] : 'ds_paraphe'),
								'anchorXOffset' => (isset($signature_params['paraphe']['x']) ? $signature_params['paraphe']['x'] : 0),
								'anchorYOffset' => (isset($signature_params['paraphe']['y']) ? $signature_params['paraphe']['y'] : 0)
							)
						);
					}
					...
					if (isset($signature_params['texts'])) {
						...
						foreach ($signature_params['texts'] as $field_name => $text_params) {
							$textTab = array(
								'tabLabel'      => $field_name,
								'name'          => (isset($text_params['label']) ? $text_params['label'] : ucfirst($field_name)),
								'anchorString'  => (isset($text_params['anch']) ? $text_params['anch'] : $anchor),
								'anchorXOffset' => (isset($text_params['x']) ? $text_params['x'] : 0),
								'anchorYOffset' => (isset($text_params['y']) ? $text_params['y'] : 0),
								'fontSize'      => (isset($text_params['fs']) ? $text_params['fs'] : $font_size),
								'required'      => (isset($text_params['req']) ? (bool) $text_params['req'] : true),
								'height'        => (isset($text_params['h']) ? $text_params['h'] : 14),
								'width'         => (isset($text_params['w']) ? $text_params['w'] : 60)
							);

							if (isset($text_params['p'])) {
								$textTab['pageNumber'] = $text_params['p'];
							}

							if (isset($text_params['regex'])) {
								$textTab['validationPattern'] = $text_params['regex']['pattern'];
								$textTab['validationMessage'] = $text_params['regex']['msg'];
							}
					...
					if (isset($signature_params['files'])) {
						...
						foreach ($signature_params['files'] as $file_key => $file) {
							...
							$params['tabs']['signerAttachmentTabs'][] = array(
								...
								'width'         => (isset($file['w']) ? $file['w'] : 20),
								'height'        => (isset($file['h']) ? $file['h'] : 30),
					...
					if ((int) $signataire->getData('need_sms_code')) {
						...
						switch ((int) $signataire->getData('type')) {
							case BimpSignataire::TYPE_CLIENT:
							...
							case BimpSignataire::TYPE_CUSTOM:
								$phone = $signataire->getData('phone');
								if ($phone) {
									$phones[] = BimpTools::cleanPhoneNumberStr($phone, true, true);
								}
								break;
...
//                            );
					...
					$signers[] = $params;
					$signataire->updateField('signature_idx', $i);
					$i++;
				} else {
...
//        exit;
	...
	public function renderHeaderExtraRight($no_div = false)
	{
		...
		$ds_certif_url = $this->getCertifDocusignUrl();
	...
	public function checkStatus(&$errors = array(), &$warnings = array(), $trigger_callbacks = true)
	{
		...
		if (count($signataires)) {
			...
			if ($new_status !== $cur_status) {
				...
				if ($trigger_callbacks && !count($errors)) {
					switch ($new_status) {
...
//		if ((int) $this->getData('status') === self::STATUS_SIGNED) {
//			$success = 'Document déjà entièrement signé';
//			return $errors;
//		}
		...
		if (!count($errors)) {
			...
			$recipients = $api->getEnvelopeRecipients($id_envelope, $errors, $warnings);

			if (isset($recipients['signers'])) {
				foreach ($recipients['signers'] as $recipient) {
					if ($recipient['status'] == 'completed') {
						/** @var BimpSignataire $signataire */
						$signataire = BimpCache::findBimpObjectInstance('bimpcore', 'BimpSignataire', array(
							'id_signature'  => $this->id,
							'signature_idx' => $recipient['recipientId']
						), true);

						if (BimpObject::objectLoaded($signataire) && $signataire->getData('status') !== BimpSignataire::STATUS_SIGNED) {
							$has_new_completed = true;
							$dt_signed = new DateTime($recipient['signedDateTime']);
							$dt_signed->setTimezone(new DateTimeZone('Europe/Paris'));
							$signataire_errors = $signataire->setSignedDocuSign($dt_signed->format('Y-m-d H:i:s'), false);
							if (count($signataire_errors)) {
								$warnings[] = BimpTools::getMsgFromArray($signataire_errors, 'Echec de la mise à jour du signataire ' . $signataire->getName());
							} else {
								$success .= ($success ? '<br/>' : 'Signature du signataire ' . $signataire->getName() . ' effectuée le ') . $dt_signed->format('d / m / Y à H:i');
							}
						}
					}
				}
			...
			if (!count($errors) && (int) $this->getData('status') !== self::STATUS_SIGNED) {
				...
				if (isset($envelope['completedDateTime'])) {
					...
					$success .= "Document entièrement signé le " . $dt_signed->format('d / m / Y à H:i') . '<br/>';
					...
					$signataires = $this->getChildrenObjects('signataires', array(
						'status' => array(
							'operator' => '!=',
							'value'    => BimpSignataire::STATUS_SIGNED
						)
					));

					if (!empty($signataires)) {
						foreach ($signataires as $signataire) {
							$signataire_errors = $signataire->setSignedDocuSign($date_signed, false);
							if (count($signataire_errors)) {
								$warnings[] = BimpTools::getMsgFromArray($signataire_errors, 'Echec de la mise à jour du signataire ' . $signataire->getName());
							}
						}
					...
					if (!count($errors)) {
						if ((int) $this->getData('status') !== self::STATUS_SIGNED) {
							$this->updateField('status', self::STATUS_SIGNED);
	...
	public function downloadDocusignExtraFiles(&$warnings = array(), &$success = '')
	{
		$errors = array();
		if (!$this->isLoaded($errors)) {
			return $errors;
		...
		if (!count($errors)) {
			// Recherche de fichiers supplémentaires :
			$docs = $api->getEnveloppeDocumentsList($id_envelope, $errors, $warnings);
			...
			if (isset($docs['envelopeDocuments'])) {
				...
				foreach ($docs['envelopeDocuments'] as $doc) {
					...
					$request_errors = array();
					$result = $api->getEnvelopeFile($id_envelope, $doc['documentId'], $request_errors);
					if (count($request_errors)) {
						...
						if (!is_dir($files_dir)) {
							...
							if ($dir_err) {
								$errors[] = 'Impossible d\'enregistrer le fichier supplémentaire #' . $doc['documentId'] . ' - Echec de la création du dossier - ' . $dir_err;
							}
						}
						...
						if (!count($errors)) {
							if (!file_put_contents($files_dir . $file_name, $file_content)) {
								...
								$success .= '<br/>Fichier supplémentaire #' . $doc['documentId'] . ' téléchargé avec succès';
							}
						}
					}
				}
	...
	public function rebuildSignedDoc()
	{
		...
		foreach ($signataires as $signataire) {
			...
			if (count($err)) {
				$errors[] = BimpTools::getMsgFromArray($err, 'Echec incrustation de la sigature du signataire "' . $signataire->getName() . '"');
			}
		}
