...
//		''  => "",
	...
	public function getClientContactsArray($with_default = true, $include_empty = false)
	{
		...
		if ($id_client) {
			$contacts = self::getSocieteContactsArray($id_client, $include_empty);

			if ($with_default) {
				$soc = BimpCache::getBimpObjectInstance('bimpcore', 'Bimp_Societe', $id_client);

				if (BimpObject::objectLoaded($soc)) {
					$id_contact_default = (int) $soc->getData('contact_default');

					// If there is a default contact
					if ($id_contact_default) {
						$label_default = $contacts[$id_contact_default];
						unset($contacts[$id_contact_default]);
						$contacts = array($id_contact_default => $label_default . ' (Contact facturation email par défaut)') + $contacts;
					}
				}
			}
		}
				...
				if (isset($data[$type]) && is_array($data[$type])) {
					foreach ($data[$type] as $mail_to) {
						...
						if ($emails) {
							switch ($type) {
								case 'mail_to':
									$to .= ($to ? ', ' : '') . BimpTools::cleanEmailsStr($emails, $name, true);
									break;

								case 'copy_to':
									$cc .= ($cc ? ', ' : '') . BimpTools::cleanEmailsStr($emails, $name, true);
									break;
							}
						}
					}
...
//                if (mailSyn2($mail_object, $to, $from, $data['msg_html'], $filename_list, $mimetype_list, $mimefilename_list, $cc, '', $deliveryreceipt)) {
	...
	public function actionAddContact($data, &$success)
	{
		...
		if (method_exists($this, 'onContactsListUpdate')) {
			$this->onContactsListUpdate();
		}
