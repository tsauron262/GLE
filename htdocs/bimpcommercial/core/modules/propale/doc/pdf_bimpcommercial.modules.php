 ...
 */
class pdf_bimpcommercial extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "bimpcommercial";
		$this->description = $langs->trans('DocModelBimpCommercialDescription');
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->propal->dir_output)
		{
			...
			else
			{
				...
				$file = $dir . "/" ."proposition_" . $objectref. ".pdf";
			}
			...
			if (file_exists($dir))
			{
                ...
                {
                    $pagecount = $pdf->setSourceFile($conf->bimpcommercial->dir_output.'/'.$conf->global->MAIN_ADD_PDF_BACKGROUND);
				...
				$pdf->SetSubject($outputlangs->transnoentities("proposition loyer"));
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					{
						$pdf->MultiCell($this->posxunit-$this->posxqty-0.8, 4, $qty . " mois" , 0, 'R');
					}
					else
					{
						$pdf->MultiCell($this->posxdiscount-$this->posxqty-0.8, 4, $qty . " mois", 0, 'R');
					}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxqty-$this->posxup-1,2, ("Prix (mois)"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			if($conf->global->PRODUCT_USE_UNITS)
			{
				$pdf->MultiCell($this->posxunit-$this->posxqty-1,2, ("Engagement"),'','C');
			}
			else
			{
				$pdf->MultiCell($this->posxdiscount-$this->posxqty-1,2, ("Engagement"),'','C');
			}
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// Logo
		$logo=$conf->bimpcommercial->dir_output.'/logos/'.$this->emetteur->logo;
