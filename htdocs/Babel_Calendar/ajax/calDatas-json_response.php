...
if ($type == 'Affaire' && $id > 0)
{
    $requete = "SELECT UNIX_TIMESTAMP(date_format(str_to_date(value_affaire, '%d/%m/%Y %H:%i'), '%Y-%m-%d %H:%i')) as dateAff,
                 ...
                 WHERE type_affaire = 'date'
                   ...
                   AND Babel_Affaire.id = babel_affaire_template_value_view.affaire_id";
    if($id > 0) $requete .= " AND Babel_Affaire.id = ".$id;
...
//print $requete;
    ...
    while($res = $db->fetch_object($sql))
    {
        $arr[]=array('id'=>$res->id,
                   ...
                   'editable' => false,
                   'color'=> $arrColor[0],
                   'url' => DOL_URL_ROOT.'/Babel_Affaire/fiche.php?id='.$res->affaire_id
                   );
        ...
        $requete1 = "SELECT * FROM Babel_Affaire_Element WHERE affaire_refid = ".$id;
