...
// Authentication mode
if (empty($dolibarr_main_authentication)) $dolibarr_main_authentication='ldap,http,dolibarr';
// Authentication mode: forceuser
if ($dolibarr_main_authentication == 'forceuser' && empty($dolibarr_auto_user)) $dolibarr_auto_user='auto';

// Set authmode
$authmode=preg_split('/,/',$dolibarr_main_authentication);
...
if (! isset($_SESSION["dol_login"]))
{
    ...
    if ($conf->webcal->enabled && $user->webcal_login != "")
    {
        ...
        //if (preg_match('/^(https:[\\\/]+[^\\\/]+)/i',$conf->global->PHPWEBCALENDAR_URL,$reg)) $domain=$reg[1];
...
if (! preg_match('/^rodolphe/i',$conf->left_menu) && ! preg_match('/^auguria/i',$conf->left_menu)) $conf->global->PRODUCT_CANVAS_ABILITY=0;
...
function dol_loginfunction($langs,$conf,$mysoc)
{
    ...
    {
        // TODO A virer Cas qui ne devrait pas arriver (pour compatibilit�)
    ...
    if (function_exists("imagecreatefrompng") && ! $disabled)
    {
        ...
        print '<td><img src="'.DOL_URL_ROOT.'/core/lib/antispamimage.php" border="0" width="128" height="36"></td>';
    ...
    {
        print '<center><table width="60%"><tr><td align="center" class="small"><div class="error ui-state-error">';
...
function top_menu($head, $title="", $target="")
{
    ...
    // Affiche le menu
    $menutop->showmenu();
    //Babel : Ajouter la calculette Javascript ici
    if ($conf->global->MAIN_MODULE_BABELsCALC && $user->rights->sCalBabel->sCalBabel->Affiche)
    {
        print "<span onclick=scal_showCacl('".DOL_URL_ROOT."/Babel_sCal/');> " . img_picto('','calc')."</span>";
    }
