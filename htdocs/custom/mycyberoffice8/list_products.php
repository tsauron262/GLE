...
//cyberoffice
$searchEshop = GETPOST("search_eshop", 'int');
...
if ($resql) {
	...
	if ($search_finished) {
        ...
        //cyberoffice
        $param = "&search_eshop=".$searchEshop;
	...
	$moreforfilter = '';
        ...
        $mysql = "SELECT";
        $mysql.= " rowid";
        $mysql.= ", ".$db->decrypt('name')." as name";
        $mysql.= ", ".$db->decrypt('value')." as value";
        $mysql.= ", type";
        $mysql.= ", note";
        $mysql.= ", entity";
        $mysql.= " FROM ".MAIN_DB_PREFIX."const";
        $mysql.= " WHERE name like 'CYBEROFFICE_SHOP%'";
        $mysql.= " ORDER BY entity, name ASC";

        $myresult = $db->query($mysql);
        if ($myresult)
        {
            $const = array();
                $mynum = $db->num_rows($myresult);
                $i = 0;
                while ($i < $mynum)
                {
                    $obj = $db->fetch_object($myresult);
                    $const[$obj->rowid] = 'eshop '.substr($obj->name,-2). ' : '.$obj->note;
                    $i++;
                }
        }
        $moreforfilter .=  "<div class='divsearchfield' style='float: revert;'>";
	$moreforfilter .= img_picto('prestashop', 'object_cyberoffice@cyberoffice','class="pictomodule"');
        $moreforfilter .= Form::selectarray('search_eshop', $const, $searchEshop, 1, 0);
	$moreforfilter .=  "</div>";
	...
	if (!empty($arrayfields['p.ref']['checked'])) {
        ...
        //cyberoffice
        print_liste_field_titre('import_key');
	...
	while ($i < min($num, $limit)) {
		$obj = $db->fetch_object($resql);
                //cyberoffice
                $c = new Categorie($db);
		$cats = $c->containing($obj->rowid, Categorie::TYPE_PRODUCT);
                if (count($cats) == 0) {
                    $i++;
                    continue;
                }
                ...
                $parent = $productcombination->fetchByFkProductChild($obj->rowid);
                $refparent = 0;
                if ($parent != 0) {
                    $produitparent = new Product($db);
                    $produitparent->fetch($parent);
                    $refparent = ($parent==0?'0':$produitparent->ref);
                }
                $myparam = array('ref'=>$obj->ref, 'parent'=>$refparent);
                $myshop = explode(' : ',$const[$searchEshop]);
                $myglobalshop = "MYCYBEROFFICE_shop" . substr($myshop[0],-2);
                $mygloballang = "MYCYBEROFFICE_lang" . substr($myshop[0],-2);
                $myglobalkey = "MYCYBEROFFICE_key" . substr($myshop[0],-2);
                $ns = 'http://www.lvsinformatique.com/ns/';
                $options = array(   'location'  => $myshop[1].'modules/mycyberoffice/server_product_soap.php',
                                    'uri'       => $myshop[1]);
                ...
                if (htmlentities($conf->global->$myglobalkey, ENT_COMPAT, 'UTF-8')) {
                    try {
                        $result0 = $soapclient->read($authentication, $myparam, $ns, '');
                    }
                    catch(SoapFault $fault) {
                        if($fault->faultstring != 'Could not connect to host') {
                            print '<span style="color:red">'.$fault->faultstring.'</span>';
                            //print_r($fault);
                            throw $fault;
                        }
                    }
		...
		if (!empty($arrayfields['p.ref']['checked'])) {
			...
			print $obj->import_key;
                        print ' ('.count($cats).')';
                        if ($parent!=0) print ' V'.$parent.'-'.$result0['result']['combination'];
                        print "</td>\n";
		// Ref supplier
		if (!empty($arrayfields['pfp.ref_fourn']['checked'])) {
			print '<td class="tdoverflowmax200">';
			...
			if ($obj->tosell) {
				if ($obj->price_base_type == 'TTC') {
					...
					print '<span class="amount">'.price($obj->price).' '.$langs->trans("HT").'</span>';
                                ...
                                //cyberoffice
                                if ($result0['result']['statut']=='ok')
                                    print '<span style="color:red">'. price($result0['result']['price']).'</span>';
			}
			print '</td>';
			if (!$i) {
				$totalarray['nbfield']++;
		...
		if (!empty($arrayfields['p.stock']['checked'])) {
			...
			if ($obj->fk_product_type != 1) {
				...
				print price(price2num($product_static->stock_reel, 'MS'));
                                //cyberoffice
                                $entrepot_id= $conf->global->{"MYCYBEROFFICE_warehouse".substr($myshop[0],-2)};
                                if ($result0['result']['statut']=='ok') {
                                    print '<span style="color:red">'. $result0['result']['stock'].'</span>';
                    if ($entrepot_id > 0) {
                        $myentrepot = explode(',', $entrepot_id);
                        foreach($arrentrepot as $myentrepot) {
                            print '/'. $myentrepot.':'.$product_static->stock_warehouse[$myentrepot]->real;
                        }
                    }
                                }
			}
