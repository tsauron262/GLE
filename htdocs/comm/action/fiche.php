    ...
    if (!GETPOST('label')) {
        if (GETPOST('actioncode') == 'AC_RDV' && $contact->getFullName($langs)) {
            ...
            if ($langs->trans("Action" . $actioncomm->type_code) != "Action" . $actioncomm->type_code) {
                ...
                $actioncomm->label = $cactioncomm->libelle;
        }
    }
    ...
    if (!$error) {
        ...
        if ($idaction > 0) {
            ...
            if (!empty($actioncomm->error))
                setEventMessage($langs->trans($actioncomm->error), 'errors');
            if (count($actioncomm->errors))
...
if ($id > 0) {
    ...
    if ($act->author->id > 0) {
        $tmpuser = new User($db);
        $res = $tmpuser->fetch($act->author->id);
        $act->author = $tmpuser;
    }
    if ($act->usermod->id > 0) {
        $tmpuser = new User($db);
        $res = $tmpuser->fetch($act->usermod->id);
        $act->usermod = $tmpuser;
    }
    if ($act->usertodo->id > 0) {
        $tmpuser = new User($db);
        $res = $tmpuser->fetch($act->usertodo->id);
        $act->usertodo = $tmpuser;
    }
    if ($act->userdone->id > 0) {
        $tmpuser = new User($db);
        $res = $tmpuser->fetch($act->userdone->id);
        $act->userdone = $tmpuser;
    }
    ...
    if ($action == 'edit') {
        ...
        if (GETPOST("afaire") == 1)
            $form->select_date($act->datep, 'ap', 1, 1, 0, "action", 1, 1, 0, 0, 'fulldaystart');
        else if (GETPOST("afaire") == 2)
            $form->select_date($act->datep, 'ap', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldaystart');
        else
            $form->select_date($act->datep, 'ap', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldaystart');
        print '</td></tr>';
        // Date end
        ...
        if (GETPOST("afaire") == 1)
            $form->select_date($act->datef, 'p2', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldayend');
        else if (GETPOST("afaire") == 2)
            $form->select_date($act->datef, 'p2', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldayend');
        else
            $form->select_date($act->datef, 'p2', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldayend');
        ...
        print '<tr><td width="30%" class="nowrap">' . $langs->trans("ActionAffectedTo") . '</td><td colspan="3">';
        if ($act->usertodo->id > 0)
            print $act->usertodo->getNomUrl(1);
        print '</td></tr>';

        // Busy
        print '<tr><td class="nowrap">' . $langs->trans("Busy") . '</td><td colspan="3">';
        if ($act->usertodo->id > 0)
            print yn(($act->transparency > 0) ? 1 : 0); // We show nothing if event is assigned to nobody
        print '</td></tr>';
        ...
        if ($conf->global->AGENDA_ENABLE_DONEBY) {
            print '<tr><td class="nowrap">' . $langs->trans("ActionDoneBy") . '</td><td colspan="3">';
            if ($act->userdone->id > 0)
                print $act->userdone->getNomUrl(1);
            print '</td></tr>';
        }
    ...
    $object->fetch($idObj);
    $idActionLier = array();
    if (isset($object->ref_ext) && $object->ref_ext > 0 && $object->ref_ext != $idObj) {
    ...
    if (isset($_REQUEST['addUser'])) {
        ...
        if ($idMaitre == $_REQUEST['removeUser']) {
            $sql = $db->query("UPDATE " . MAIN_DB_PREFIX . "actioncomm SET ref_ext = '" . $object->id . "' WHERE ref_ext = '" . $idMaitre . "'");
            $sql = $db->query("UPDATE " . MAIN_DB_PREFIX . "actioncomm SET ref_ext = 'null' WHERE rowid = '" . $object->id . "'");
            $idActionLier = array();
            $idMaitre = $object->id;
        }
        $objectT->delete(1);
