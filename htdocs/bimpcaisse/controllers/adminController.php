...
class adminController extends BimpController
{
    ...
    function ajaxProcessLoadFormFondCaisse()
    {
        ...
        foreach ($tabEntCaisse as $entrepot => $tabCaisses) {
            ...
            foreach ($tabCaisses as $idCaisse => $nameCaisse) {
                ...
                $htmlEnt .= "<h4>Caisse : " . $nameCaisse . " </h4>Au " . date('d / m / Y', strtotime($date)) . " | " . $montant . " €";
                $tot += $montant;
            }
    ...
    function renderFondsFermeture()
    {
        ...
        $html = BimpForm::renderFreeForm(array(array('label' => 'Date', 'input' => BimpInput::renderInput('date', 'dateRef'))), array('<button class="btn btn-primary" onclick="' . htmlentities($onclick) . '">Ok</button>'), 'Fonds de caisse');
