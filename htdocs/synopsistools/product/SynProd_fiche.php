...
if (empty($reshook))
{
    ...
    if ($action == 'confirm_delete' && $confirm == 'yes')
    {
        ...
        if ( ($object->type == 0 && $user->rights->produit->supprimer)	|| ($object->type != 0 && $user->rights->service->supprimer) )
        {
...
else
{
    ...
    if ($action == 'create' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        ...
        if ($type != 0) $title=$langs->trans("NewService");
        ...
        // Stock min level
        if ($type == 0 && ! empty($conf->stock->enabled))
        {
        ...
        if ($type == 0)	// Le poids et le volume ne concerne que les produits et pas les services
        {
            // Weight
    ...
    else if ($id || $ref)
    {
        ...
        if ($action == 'edit' && ($user->rights->produit->creer || $user->rights->service->creer))
        {
            ...
            if ($object->type!=0) $type = $langs->trans('Service');
            ...
            if ($object->type==0 && ! empty($conf->stock->enabled))
            {
        ...
        else
        {
            ...
            $picto=($object->type != 0?'service':'product');
