...
EOF;
    ...
    $requete = "SELECT * FROM llx_Synopsis_Affaire_Element WHERE type='projet' AND affaire_refid=".$affaireid;
    ...
    $requete = " SELECT *
                  ...
                  WHERE rowid in (SELECT element_id FROM llx_Synopsis_Affaire_Element WHERE type='propale' AND affaire_refid = ".$affaireid.")
               ORDER BY datep DESC";
    $sql = $db->query($requete);
...
//TODO liaison fcture - commande
    ...
    $requete = "SELECT rowid, fk_soc, unix_timestamp(date_commande) as date_commandeU
                  ...
                  WHERE rowid in (SELECT element_id FROM llx_Synopsis_Affaire_Element WHERE type='commande' AND affaire_refid = ".$affaireid.")
    ...
    $requete = "SELECT rowid, fk_soc, unix_timestamp(datef) as datefU
                  ...
                  WHERE rowid in (SELECT element_id FROM llx_Synopsis_Affaire_Element WHERE type='facture' AND affaire_refid = ".$affaireid.")
               ORDER BY datef DESC";
    $sql= $db->query($requete);
    if ($db->num_rows($sql) > 0)
    {
    ...
    $requete = " SELECT *
                  ...
                  WHERE ".MAIN_DB_PREFIX."facture_fourn.rowid in (SELECT element_id
                    ...
                    AND ".MAIN_DB_PREFIX."facture_fourn.rowid not in (SELECT fk_facture
                                                         ...
                                                         WHERE Babel_li_fourn_co_fa.fk_commande not in( SELECT rowid FROM llx_Synopsis_Affaire_Element WHERE affaire_refid = ".$affaireid." AND type='commande fournisseur'))";
