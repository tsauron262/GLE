			...
			switch ($this->getData('type')) {
				...
				case self::LINE_FREE:
					...
					if ($this->date_from || $this->date_to) {
						...
						if (!preg_match('/^\d{4}\-\d{2}\-\d{2}( \d{2}:\d{2}:\d{2})?$/', (string) $this->date_from)) {
						...
						if ((string) $this->date_to && !preg_match('/^\d{4}\-\d{2}\-\d{2}( \d{2}:\d{2}:\d{2})?$/', (string) $this->date_to)) {
							$errors[] = 'Date de fin invalide';
							$date_check = false;
						}

						if ($date_check && $this->date_from && $this->date_to) {
							if ($this->date_from > $this->date_to) {
