...
';
    ...
    foreach ($prestations as $prestation) {
        ...
        /*     $requete = "SELECT * FROM ".MAIN_DB_PREFIX."Synopsis_fichinter_c_typeInterv WHERE active = 1 ORDER by rang";
          ...
          $selectHtml = "<SELECT name='fk_typeinterv'>";
          $selectHtml .= "<OPTION value='-1'>Selectionner-></OPTION>";
          ...
          {
          $selectHtml .= "<OPTGROUP label='".$res1->label."'>";
          while($res2 = $db->fetch_object($sql2))
          {
          if ($res2->rowid == $objp->fk_depProduct){
          $selectHtml .= "<OPTION SELECTED value='dep-".$res2->rowid."'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$res2->ref." ".$res2->label."</OPTION>";
          } else {
          $selectHtml .= "<OPTION value='dep-".$res2->rowid."'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$res2->ref." ".$res2->label."</OPTION>";
          }
          }
          $selectHtml .= "</OPTGROUP>";
          ...
          {
          $selectHtml .= "<OPTION SELECTED value='".$res1->id."'>".$res1->label."</OPTION>";
          } else {
          $selectHtml .= "<OPTION value='".$res1->id."'>".$res1->label."</OPTION>";
          }
          }
          } else {
          if ($res1->id == $objp->fk_typeinterv)
    ...
    echo '<tr id="ajPrestaZone"><td colspan="4"><input type="button" id="ajPresta" value="Ajouter ligne"/></td></tr>';
    $checked = (extra(17) == 1) ? ' checked="checked"' : '';

    echo '<tr>
            <th width="25%" class="ui-widget-header ui-state-default">Pr&eacute;conisation du technicien</th>
...
/*
  echo '<tr>
  <th width="25%" class="ui-widget-header ui-state-default">Réf.</th>
  <td class="ui-widget-content" colspan="1">Val</td>
  <th width="25%" class="ui-widget-header ui-state-default">Réf.</th>
  <td class="ui-widget-content" colspan="1">Val</td>
  </tr>
  '; */
...
$htmlStr = '<tr>
        ...
        <tr>
            ...
            <td class="ui-widget-content" colspan="1"><input type="text" name="\'+prefId+\'duree" class="heures" value="0"/></td>
        </tr>';
$htmlStr .= '<tr>
            <th width="25%" class="ui-widget-header ui-state-default">Rapport</th>
...
//    $oldData = mysql_fetch_object($req);
    $req = "UPDATE  `gle1main`.`".MAIN_DB_PREFIX."Synopsis_fichinter` SET  `datei` =  '" . convertirDate($_POST['date'], false) . "',`description` =  '" . $_POST['desc'] . "', natureInter = '" . $_POST['natureInter'] . "' WHERE  `".MAIN_DB_PREFIX."Synopsis_fichinter`.`rowid` =" . $fichinter->id;
    $sql = $db->query($req);
    ...
    for ($i = 1; isset($_POST['presta' . $i . "_rowid"]); $i++) {
        ...
        if (!isset($_POST['presta' . $i . "_rowid"]) || $_POST['presta' . $i . "_rowid"] == 0)
            $fichinter->addline(
                    $fichinter->id,
                    $_POST['presta' . $i . "_descPrest"],
                    date("Y-m-d"),
                    $_POST['presta' . $i . "_duree"],
                    $_POST['presta' . $i . "_fk_typeinterv"],
                    1,
                    $pu_ht,
                    $_POST['typeInter'],
                    $_REQUEST['comLigneId'],
                    0,
                    $_POST['presta' . $i . "_fk_prod"]
            );
        else {
...
function extra($cle, $val = null) {
    ...
    if ($val == null) {//On fait un select
        ...
        if (is_null(extra($cle))) {
            $req = "INSERT INTO `gle1main`.`".MAIN_DB_PREFIX."Synopsis_fichinter_extra_value` (`extra_value`, extra_key_refid, `interv_refid`, typeI) VALUES('" . $val . "', " . $cle . "," . $fichinter->id . ", 'FI')";
            $sql = $db->query($req);
        } else {
            $req = "UPDATE  `gle1main`.`".MAIN_DB_PREFIX."Synopsis_fichinter_extra_value` SET  `extra_value` =  '" . $val . "' WHERE extra_key_refid = " . $cle . " AND `babel_interv_extra_value`.`interv_refid` =" . $fichinter->id;
            $sql = $db->query($req);
        }/*
          if (!$sql) {
          echo mysql_error();
...
//        $req = "SELECT * FROM `".MAIN_DB_PREFIX."Synopsis_fichinterdet` WHERE rowid = (SELECT min(rowid) FROM `".MAIN_DB_PREFIX."Synopsis_fichinterdet` WHERE fk_fichinter= " . $fichinter->id . ")";
        ...
        $sql = $db->query($req);
//        return mysqlToArray($sql);
    }
}

function mysqlToArray($data) {
    $tab = array();
    if ($data) {
