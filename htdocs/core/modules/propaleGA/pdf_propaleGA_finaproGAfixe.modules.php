...
/**
 \file       htdocs/includes/modules/propale/pdf_propaleGA_finaproGAfixe.modules.php
 \ingroup    propale
 \brief      Fichier de la classe permettant de generer les propales au modele Azur
 \author        Laurent Destailleur
 \version    $Id: pdf_propaleGA_azur.modules.php,v 1.121 2008/08/07 07:47:38 eldy Exp $
 */
...
class pdf_propaleGA_finaproGAfixe extends ModelePDFPropalesGA
{
    ...
    */
    function pdf_propaleGA_finaproGAfixe($db)
    {
        ...
        $this->db = $db;
        $this->name = "finaproGAfixe";
        $this->libelle = "Finapro Fixe";
    ...
    function write_file($propale,$outputlangs='')
    {
        ...
        if ($conf->PROPALEGA->dir_output)
        {
            ...
            if (file_exists($dir))
            {
                ...
                $html = "<br>".chr(149)." Loyers Mensuels, Constants.<br>".chr(149)." Règlement Terme A Echoir.<br>".chr(149)." Assurance Vol/Bris Multirisques <bi>non-incluse</bi>.<br>".chr(149)." Services de Gestion de Parc FINAPRO inclus.";
                $pdf->lasth = $pdf->lasth * 0.9;
                ...
                $html ='<br>Description des matériels loués <br><br>Voir devis  N°: '. utf8_decode($propale->ref_client).' du '.date("d/m/Y",$propale->date_devis_fourn). " de la société ".$tmpSocFourn->nom;
                $this->writeHTML($html,2,0,$pdf);
//TODO rajouté nom fournisseur
                $pdf->SetFont('',"B");
                $pdf->SetY($pdf->getY()+5);
                $pdf->SetX($larg1);
                ...
                $nexY=$y+$repos + 10;
                $pdf->SetY($nexY);
                $pdf->MultiCell($larg1, 6, $outputlangs->transnoentities(utf8_decode("Durées proposées")), 0, 'C', 1);
                $pdf->SetXY($larg1+$this->marge_gauche+1,$nexY);
                $pdf->MultiCell($larg1, 6, $outputlangs->transnoentities(utf8_decode("loyer 24 mois")), 0, 'C', 1);
                $pdf->SetXY($larg1+$larg1+$this->marge_gauche+2,$nexY);
                $pdf->MultiCell($larg1, 6, $outputlangs->transnoentities(utf8_decode("loyer 36 mois")), 0, 'C', 1);
                $pdf->SetXY($larg1+$larg1+$larg1+$this->marge_gauche+3,$nexY);
                $pdf->MultiCell($larg1, 6, $outputlangs->transnoentities(utf8_decode("loyer 48 mois")), 0, 'C', 1);

                $bottomlasttab = $pdf->getY();

                $pdf->SetXY($this->marge_gauche , $bottomlasttab);
                $pdf->MultiCell($larg1, 6,"Montant mensuel", 0, 'C', 1);

                $pdf->SetFont('Vera','B',10);


//TODO recalculer les loyers en fonction de la durée relative en terme à échoir => marge = marge de la propal
                $loyer24 = $propale->loyer24;
                $loyer36 = $propale->loyer36;
                $loyer48 = $propale->loyer48;


                $pdf->SetXY($larg1+$this->marge_gauche+1,$bottomlasttab);
                $pdf->MultiCell($larg1, 6, price(round($loyer24*100)/100 ) ." ". chr(128), 0, 'C', 0);
                $pdf->SetXY($larg1+$larg1+$this->marge_gauche+2,$bottomlasttab);
                $pdf->MultiCell($larg1, 6, price(round($loyer36*100)/100 ) ." ". chr(128), 0, 'C', 0);
                $pdf->SetXY($larg1+$larg1+$larg1+$this->marge_gauche+3,$bottomlasttab);
                $pdf->MultiCell($larg1, 6, price(round($loyer48*100)/100 ) ." ". chr(128), 0, 'C', 0);
