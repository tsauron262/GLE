...
//$tabTypeLigne = array_merge($tabProductType, $tabTypeLigne);
    ...
    public function afterUserOk(){
        global $user, $db, $conf;
         if(isset($conf->global->DOUBLE_AUTH_SMS) && $user->id > 0){//l'adresse n'est pas wite liste et le module est activé 
...
//                        die($crypt);
                ...
                if(stripos($to, "+336") === false && stripos($to, "+337") === false)
                    mailSyn2("ATTENTION Ip Inconnue phone KO ATTENTION", "tommy@bimp.fr", "admin@bimp.fr", "Ip inconnue : ".$_SERVER['REMOTE_ADDR']." user ".$this->user->login. " phone : ".$to);
                else
