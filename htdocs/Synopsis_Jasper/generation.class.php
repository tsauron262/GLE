...
//
//$jasperSoap = new jasperSoap("http://192.168.1.73:8080/jasperserver/services/", 'jasperadmin', 'jasperadmin');
//$jasperSoap->traiteReport('/Test/Employees', 'HTML');

class jasperSoap {

    function traiteReport($uri, $format) {
        $attachments = array();
        $this->ws_runReport($uri, array(), array(RUN_OUTPUT_FORMAT => $format), $attachments);

        $return = $attachments["cid:report"];

        if ($return == '')
            die("Aucune données recu");
        else {
            ob_clean();
...
// 		}
    ...
    function jasperSoap($jasperserver_url, $username, $pass) {
        $SCHEDULING_WS_URI = $jasperserver_url . "ReportScheduler?wsdl";
        $this->webservices_uri = $jasperserver_url . "repository";
    ...
    function ws_list($uri, $args = array()) {
        $connection_params = array("user" => $this->username, "pass" => $this->pass);
        $info = new SOAP_client($this->webservices_uri, false, false, $connection_params);
    ...
    function ws_runReport($uri, $report_params, $output_params, &$attachments) {
        ...
        $connection_params = array("user" => $this->username, "pass" => $this->pass, "timeout" => $max_execution_time);
        $info = new SOAP_client($this->webservices_uri, false, false, $connection_params);
