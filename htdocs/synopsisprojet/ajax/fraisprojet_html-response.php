...
//jqgrid
 //Affiche les ressources du projet
    ...
    while ($res = $db->fetch_object($sql))
    {
        $jsEditopts["factFourn"].= $res->rowid . ":" . preg_replace("/'/","\\'",(($res->ref . " ".date('d/m/Y',$res->Fdatef) . " ".price($res->total_ht)."euro")))  . ";";
    }
    ...
    while ($res = $db->fetch_object($sql))
    {
        $jsEditopts["cmdFourn"].= $res->rowid . ":" . preg_replace("/'/","\\'",(($res->ref . " ".date('d/m/Y',$res->Fdatef) . " ".price($res->total_ht)." euro")))  . ";";
    }
