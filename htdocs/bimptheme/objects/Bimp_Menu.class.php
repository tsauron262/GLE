...
class Bimp_Menu extends BimpObject
{
	...
	public static function getMenuItems($active_only = true, $enabled = true, $check_perms = true)
	{
		...
		), null, null, 'position', 'ASC', 'array');

		if (is_array($rows)) {
			return self::getMenuSubItems($rows, 0, $active_only, $enabled, $check_perms);
		}

		return array();
	}

	public static function getMenuSubItems($all_items, $id_parent, $active_only = true, $enabled = true, $check_perms = true)
	{
		$sub_items = array();

		foreach ($all_items as $item) {
			if ((int) $item['fk_menu'] !== $id_parent) {
				continue;
			}
			...
			$sub_items[(int) $item['rowid']] = $item;
			$sub_items[(int) $item['rowid']]['sub_items'] = self::getMenuSubItems($all_items, (int) $item['rowid'], $active_only, $enabled, $check_perms);
		}
	...
	public static function getFullMenu($handler = null, $active_only = true, $enabled = true, $check_perms = true)
	{
		...
		if (empty($items)) {
			$items = self::getMenuItems(false, false, false);
	...
	public function getInsertSql(&$errors = array())
	{
		...
		if ($this->isLoaded()) {
			...
			if (!count($errors)) {
				$sql = 'INSERT INTO `llx_menu` (`menu_handler`, `entity`, `fk_menu`, `position`, `url`, `titre`, `perms`, `enabled`, `code_path`, `active`, `bimp_module`, `bimp_icon`, `bimp_object`, `synchronised`)';
				$sql .= ' VALUES (';
		...
		if (!count($errors)) {
			...
			if (is_dir($dir)) {
				...
				if (!file_put_contents($dir . '/' . 'fullmenu_' . $version . '.sql', $sql)) {
					...
					$html .= '<br/><br/>Création manuelle du fichier "fullmenu_' . $version . '.sql" : <br/><br/>';
		...
		if ($version) {
			...
			if (!file_exists($file)) {
				...
				if (!$sql) {
					...
					if (!count($errors)) {
						// Màj des id_parents des lignes non synchronisées:
						$items = BimpCache::getBimpObjectObjects('bimptheme', 'Bimp_Menu', array(
							'menu_handler' => 'bimptheme',
							'synchronised' => 0,
							'fk_menu'      => array(
								'operator' => '>',
						...
						foreach ($items as $item) {
							$code_path = $item->getData('code_path');

							if (preg_match('/^(.*)\/([^\/]+)$/', $code_path, $matches)) {
								$parent_code_path = $matches[1];
								$id_parent = (int) $bdb->getValue('menu', 'rowid', 'code_path = \'' . $parent_code_path . '\' AND menu_handler = \'bimptheme\'');
