 ...
 */
 ...
 if ($_REQUEST['action'] == 'setRevisionModel')
 {
    ...
    $id = $_REQUEST['id'];
    $requete = "UPDATE ".MAIN_DB_PREFIX."synopsischrono_conf SET revision_model_refid = ".$revModel . " WHERE id = ".$id;
    $sql = $db->query($requete);
    if ($sql) header('Location: synopsischrono_advMode.php?panelRevision=1&id='.$id);
    else $msg="Erreur : ".$db->error;

 }
 if ($_REQUEST['action']=='del')
 {
    $requete = "DELETE FROM ".MAIN_DB_PREFIX."synopsischrono_key WHERE id = " .$_REQUEST['delid'];
    $sql = $db->query($requete);
 }
 if ($_REQUEST['action']=='addKey')
 {
    ...
    $requete = "SELECT * FROM ".MAIN_DB_PREFIX."synopsischrono_key_type_valeur WHERE nom='".$type_valeur."'";
    $sql = $db->query($requete);
    $res = $db->fetch_object($sql);
    if ($res->id > 0)
        $type_valeur = $res->id;
    ...
    $requete = "INSERT INTO ".MAIN_DB_PREFIX."synopsischrono_key (nom, description,model_refid, type_valeur,type_subvaleur,extraCss,inDetList)
...
if ($_REQUEST['action'] == 'setDroits')
{
    ...
    $requete = "DELETE FROM ".MAIN_DB_PREFIX."synopsischrono_rights WHERE chrono_refid = ".$id;
    $sql = $db->query($requete);
    $requete = "DELETE FROM ".MAIN_DB_PREFIX."synopsischrono_group_rights WHERE chrono_refid = ".$id;
    $sql1 = $db->query($requete);
    ...
    foreach($_REQUEST as $key=>$val)
    {
        if (preg_match('/([u|g]{1})([0-9]+)-([0-9]+)/',$key,$arr))
        {
            ...
            $userid= $arr[2];
            $rightId= $arr[3];
            $requete = "INSERT INTO ".MAIN_DB_PREFIX."synopsischrono_rights (chrono_refid,user_refid,right_refid,valeur)
                             VALUES (".$id.",".$userid.",".$rightId.",1)";
            if ($type == 'g')
            {
                $requete = "INSERT INTO ".MAIN_DB_PREFIX."synopsischrono_group_rights (chrono_refid,group_refid,right_refid,valeur)
                                 VALUES (".$id.",".$userid.",".$rightId.",1)";
            }
            $sql = $db->query($requete);
            $commit = ($sql&& $commit?true:false);
    ...
    if ($commit) header('Location: synopsischrono_advMode.php?panelDroit=1&id='.$id);
    else $msg="Erreur : ".$db->error;
}
...
$js .= <<<EOF
    ...
    jQuery('#delDialog').dialog({
        buttons:{
            "OK": function(){
                location.href="synopsischrono_advMode.php?delid="+dialogId+"&action=del&id="+chronoId
            },
            "Annuler": function(){
...
//    $requete = "SELECT * FROM ".MAIN_DB_PREFIX."synopsischrono_conf WHERE id = ".$_REQUEST['id'];
//    $sql = $db->query($requete);
//    $res = $db->fetch_object($sql);
    ...
    $head[$h][0] = DOL_URL_ROOT.'//synopsischrono/admin/synopsischrono_advMode.php?id='.$_REQUEST['id'];
    $head[$h][1] = $res->titre;
    $head[$h][2] = 'Chrono';
    $h++;
    ...
    if ($hasRevision)
    {
        ...
        while($res = $db->fetch_object($sql))
        {
            ...
            if ($res->id == $revision_model)
                ...
                print "       <a href='synopsischrono_advMode.php?action=setRevisionModel&id=".$_REQUEST['id']."&revision_model=".$res->id."'>Activer</a>";
        }
        print "</table>";
        print "</div>";
    }
    ...
    print "    <td class='ui-widget-content'><button onclick=\"location.href='synopsischrono_advMode.php?action=genWidget&id=".$id."';\" class='butAction'>OK</button>";
    print "</table>";
    print "</div>";

    print "<div id='droits'>";

    print "<form action='synopsischrono_advMode.php?id=".$_REQUEST['id']."' method='POST'>";
    ...
    $requete = "SELECT * FROM ".MAIN_DB_PREFIX."synopsischrono_rights_def WHERE active=1 ORDER BY rang ";
    $sql = $db->query($requete);
    while ($res = $db->fetch_object($sql))
    {
    ...
    while ($res = $db->fetch_object($sql))
    {
        ...
        $requete1 = "SELECT * FROM ".MAIN_DB_PREFIX."synopsischrono_rights_def WHERE active=1 ORDER BY rang ";
...
//            print ($tmpgrp->rights->chrono_group->$chrono_right_ref && $tmpgrp->rights->chrono_group->$chrono_right_ref->$type?img_tick("Oui"):"");
    ...
    $requete = "SELECT k.nom,
                       ...
                       k.id,
                       k.rang
                  FROM ".MAIN_DB_PREFIX."synopsischrono_key as k ,
                       ".MAIN_DB_PREFIX."synopsischrono_key_type_valeur  as v
    ...
    print '<form name="addForm" id="addForm" action="synopsischrono_advMode.php?id='.$_REQUEST['id'].'" method="POST" onSubmit="if(!jQuery(\'#addForm\').validate().form()) return(false);">';
    ...
    $requete = "SELECT * FROM ".MAIN_DB_PREFIX."synopsischrono_key_type_valeur ORDER BY hasSubValeur, nom";
    $sql = $db->query($requete);
