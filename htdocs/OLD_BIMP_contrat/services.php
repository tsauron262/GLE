...
/**
        ...
        \version    $Id: services.php,v 1.22 2008/08/10 16:16:33 eldy Exp $
*/
...
p.rowid as pid, p.label as label, 

MIN(unix_timestamp(cd.date_ouverture_prevue)) as date_ouverture_prevue,
MIN(unix_timestamp(cd.date_ouverture)) as date_ouverture, 
MIN(unix_timestamp(cd.date_fin_validite)) as date_fin_validite, 
MIN(unix_timestamp(cd.date_cloture)) as date_cloture 

FROM llx_contrat as c, llx_societe as s, llx_contratdet as cd LEFT JOIN babel_product as p ON cd.fk_product = p.rowid WHERE c.rowid = cd.fk_contrat AND c.fk_soc = s.rowid AND cd.statut = 4 AND date_fin_validite < (sysdate() + INTERVAL '1' MONTH) GROUP BY c.rowid";
}


if ($search_nom)      $sql.= " AND s.nom like '%".addslashes($search_nom)."%'";
if ($search_contract) $sql.= " AND c.ref like '%".addslashes($search_contract)."%'";
...
if ($resql)
{
    ...
    print_liste_field_titre($langs->trans("Contract"),"services.php", "c.cref","$param","","",$sortfield,$sortorder);
    if ($mode != "6")
    ...
    // Date fin
    if ($mode != "4") print_liste_field_titre($langs->trans("DateEndPlannedShort"),"services.php", "cd.date_fin_validite","$param",'',' align="center"',$sortfield,$sortorder);
    ...
    print '<input type="text" class="flat" size="18" name="search_contract" value="'.stripslashes($search_contract).'">';
    print '</td>';
    print '<td class="liste_titre">';
...
llxFooter('$Date: 2008/08/10 16:16:33 $ - $Revision: 1.22 $');
?>
