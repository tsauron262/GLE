...
//    var $saut = "<br/>";        
    ...
    public function exportFactureSav() {
        ...
        $result = $this->db->query("SELECT fact.rowid as id, idtech8sens as id8Sens, Centre "
                ...
                . "WHERE fe.fk_object = fact.rowid AND fe.`type` = 'S' AND el.targettype = 'facture' AND el.sourcetype = 'propal' AND fk_target = fact.rowid AND prop.rowid = el.fk_source AND prop.fk_statut != 3 AND prop.rowid = chrono.propalid AND chronoT.id = chrono.id AND `fk_object` = IF(chronoT.Technicien > 0, chronoT.Technicien, fact.fk_user_author) AND fact.fk_soc = soc.rowid "
                . $this->where);
