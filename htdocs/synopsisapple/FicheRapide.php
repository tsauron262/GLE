...
if($("input:text#NoMachine").val() != undefined){
    $("input:text#NoMachine").each(function(){
        getInfoApple($(this));
    });
}
...
$pro = (isset($_POST['Pro']) && ($_POST['Pro'] == 1 || $_POST['Pro'] == 'on' || $_POST['Pro'] == 'checked') ? 'checked' : "");
...
if ($socid != "" && $socid > 0 && $NoMachine) {
    if ($NoMachine) {
        $chronoProdid = existProd($NoMachine, $socid);
        if ($chronoProdid > 0) {
            $chronoProd = new Chrono($db);
            $chronoProd->fetch($chronoProdid);
            $chronoProd->getValues();
            $NoMachine = $chronoProd->values['N__Serie'];
            $pass = $chronoProd->values['Mdp_Admin'];
            $loginA = $chronoProd->values['Login_Admin'];
            $DateAchat = $chronoProd->values['Date_Achat'];
            $garantie = $chronoProd->values['Date_fin_SAV'];
            $typeGarantie = $chronoProd->values['Type_garantie'];
            $systeme = $chronoProd->values['Systeme'];
            $machine = $chronoProd->description;
        }
    }
    echo "<div id='reponse' >";
    echo "</div>";
...
//    foreach ($centres as $val => $centre) {
  ...
  /*  echo "</tr>";
    ...
    echo " <textarea class='pGrand' type='text' name='Descr' id='Descr' >$descr</textarea>";
    echo "</td>";
   /* echo "</tr>";
    echo "</p>";
    ...
    echo "<td class='ui-widget-content' colspan='1'>";
    echo "<textarea class='pGrand choixAccess ' name='Chrono-1041' id='Chrono-1041'>$accessoire</textarea>";
    echo "</td>";
    echo "</tr>";
    echo "</p>";
  ...
  /*  echo "</tr>";
   ...
   /* echo "</tr>";
    ...
    echo "<td class='ui-widget-content' colspan='3'>";
