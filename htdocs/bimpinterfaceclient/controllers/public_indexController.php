...
require_once DOL_DOCUMENT_ROOT . '/bimpinterfaceclient/controllers/InterfaceClientController.php';

class public_indexController extends InterfaceClientController
{
    ...
    public function renderHtml()
    {
        ...
        $valide_contrat = $userClient->getContratsVisibles(true);

        // $valide_contrat = Array(); // Simuler que la société n'est pas sous contrat
        $all_contrat = $userClient->getContratsVisibles();
        $html = "";
        $soc = $userClient->getDolObjectInstance($userClient->getData('id_client'), 'societe');
        ...
        if ($valide_contrat) {
            ...
            $userTickets = BimpObject::getInstance('bimpinterfaceclient', 'BIC_UserClientTicket');
            ...
            $html .= '<button id="createTicketFromAll" class="btn btn-info" socid="' . $userClient->getData('id_client') . '" >Créer le ticket</button>';

            $html .= '</div>';

            $html .= $this->displayListCard($valide_contrat, true);
        }
        if ($all_contrat) {
        ...
        if ($contracts) {
            ...
            $html .= $this->displayListCard($contracts);
            $html .= '</div></div>';
        }
    ...
    public function displayListCard($objects)
    {
        $return = "";
