...
if (!$modify) {
    ...
    foreach ($comGrp->commandes as $key => $val) {
        ...
        $societe->fetch($val->socid);
        print "<tr " . $bc[$bool] . ">";
        print "   <td>" . $val->getNomUrl(1);
        print '<td>' . $societe->getNomUrl(1);
    ...
    $selected = $val->commande[0]->socid;
    print "<tr class='ui-widget-content'><td>" . $html->select_company($selected, 'socid', '', $showempty = 1, false, "");
    ...
    foreach ($comGrp->commandes as $key => $val) {
        $val->fetch_lines();
        foreach ($val->lines as $key1 => $val1) {
    ...
    foreach ($arrProd as $key => $val) {
        ...
        if ($tmpProd->fetch($key) && $tmpProd->ref . "x" != "x")
            print "<tr " . $bc[$bool] . "><td>" . $tmpProd->getNomUrl(1) . '<td align="center">' . price($tmpProd->price) . '&euro;' . "<td align='center'>" . $val;
        $bool = !$bool;
    }
    print "</table>";
}
