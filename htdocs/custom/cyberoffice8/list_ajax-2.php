...
$sql = "SELECT s.rowid, s.nom as name, s.name_alias, s.barcode, s.address, s.town, s.zip, s.datec, s.code_client, s.code_fournisseur, s.logo, s.entity"
    ...
    . ",s.import_key, s.fk_pays, s.fk_departement"
    ...
    . " AND (";
if ($object->name) $sql.= " s.nom LIKE '%".safe($object->name)."%'";
if ($object->name_alias) $sql.= " OR s.name_alias LIKE '%".safe($object->name_alias)."%'";
/*if ($object->zip) $sql.= " OR s.zip LIKE '%".safe($object->zip)."%'";*/
/*if ($object->town) $sql.= " OR s.town LIKE '%".safe($object->town)."%'";*/
if ($object->email) $sql.= " OR s.email LIKE '%".safe($object->email)."%'";
if ($object->phone) $sql.= " OR s.phone LIKE '%".safe($object->phone)."%'";
if ($object->idprof1) $sql.= " OR s.siren LIKE '%".safe($object->idprof1)."%'";
$sql.=")";
if ($l==1) {
