...
if ($conf->contrat->enabled) require_once(DOL_DOCUMENT_ROOT."/Babel_GMAO/contratMixte.class.php");
 ...
 switch($action)
 {
    ...
    //                $date_end = strtotime($date_end_update);
                ...
                $result = $contrat->addline(
                        htmlentities(utf8_decode($description)),
                        $pu_ht,
                        $qte,
                        $tva_tx,
                        ($prodid?$prodid:""),
...
//print $requete;
                    ...
                    {
                        $requete = "INSERT INTO ".MAIN_DB_PREFIX."product_serial_cont (element_id, serial_number, date_creation, date_fin_SAV, fk_user_author,element_type)
...
//print $requete;
                    $req1 = "SELECT * FROM Babel_GMAO_contratdet_prop WHERE contratdet_refid = ".$_REQUEST['lineid'];
                    $sql1 = $db->query($req1);
                    if (!$db->num_rows($sql1) > 0){
                        $requeteIns = "INSERT INTO Babel_GMAO_contratdet_prop (contratdet_refid) VALUES (".$_REQUEST['lineid'].")";
                        $db->query($requeteIns);
                    }
                    $result1 = $db->query($requete);
                    $totalSAV = $basicSAV + $durVal;
                    ...
                    {
                        $requete = "SELECT *
                                      FROM ".MAIN_DB_PREFIX."product_serial_cont
...
//print $requete."\n";
                    ...
                    if('x'.$serial == "x")
                    {
                        $requete = "DELETE FROM ".MAIN_DB_PREFIX."product_serial_cont
                                          WHERE element_id = ".$_REQUEST['lineid']."
                                            AND element_type LIKE 'contrat%'";
                        $result1 = $db->query($requete);
                    }
...
//                $mesg=$contrat->error;
    ...
    case 'getLineDet':
    {
        ...
        $requete = "SELECT DISTINCT c.statut,
                 ...
                 LEFT JOIN ".MAIN_DB_PREFIX."product_serial_cont as sc ON sc.element_id = c.rowid AND sc.element_type LIKE 'contrat%'
        ...
        {
            //  Description
            $xml .= "<description><![CDATA[".utf8_encode($res->description)."]]></description>";
            //  date debut
            $xml .= "<dateDeb><![CDATA[".date('d/m/Y',$res->GMAO_ddeb)."]]></dateDeb>";
            //  SLA
            $xml .= "<SLA><![CDATA[".utf8_encode($res->GMAO_sla)."]]></SLA>";
            //  reconduction auto
            ...
            //  Produit contrat
            $xml .= "<fk_contrat_prod><![CDATA[".$res->GMAO_fk_contrat_prod."]]></fk_contrat_prod>";

            if ($res->GMAO_fk_contrat_prod > 0){
                $tmpProd = new Product($db);
                $tmpProd->fetch($res->GMAO_fk_contrat_prod);
                $xml .= "<contratClause><![CDATA[".utf8_encode($tmpProd->clause)."]]></contratClause>";
            } else {
            ...
            //  prix HT
            $xml .= "<totalht><![CDATA[".round($res->price_ht*100)/100 ."]]></totalht>";
            //  TVA
            ...
            if ($res->fk_product > 0){
                ...
                $xml .= "<productClause><![CDATA[".utf8_encode($tmpProd->clause)."]]></productClause>";
            } else {
            ...
            //  clause
            $xml .= "<clause><![CDATA[".utf8_encode($res->GMAO_clause)."]]></clause>";
