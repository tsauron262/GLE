...
class BimpConfig
{
	...
	public function getParamsFromFile($module, $module_dir, $file_name, &$errors = array(), $check_extends = true)
	{
		...
		if (!isset(self::$files_cache[$cache_key])) {
			...
			$version_file = '';
			if ($ext_version) {
				$version_module_dir = 'extends/versions/' . $ext_version . ($module_dir ? '/' . $module_dir : '');
			...
			$entity_file = '';
			if ($ext_entity) {
				$entity_module_dir = 'extends/entities/' . $ext_entity . ($module_dir ? '/' . $module_dir : '');
				$entity_file = DOL_DOCUMENT_ROOT . '/' . $module . '/' . $entity_module_dir . '/' . $file_name . '.yml';
			}

			if (!file_exists($file)) {
				if (!$version_file || !file_exists($version_file)) {
					if (!$entity_file || !file_exists($entity_file)) {
						$errors[] = 'Fichier de configuration "' . $file . '" absent';
						return array();
					}
				}
			}
	...
	protected function getDictionnaryArray($dict, $path)
	{
		...
		if (is_string($dict)) {
			...
			$empty_value = $this->get($path . '/empty_value', $empty_value, false);
