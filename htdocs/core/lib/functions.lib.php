...
if (! function_exists('dol_getprefix'))
{
	function dol_getprefix($mode='')
	{
		...
		{
			return dol_hash($_SERVER["SERVER_NAME"].$_SERVER["DOCUMENT_ROOT"].DOL_DOCUMENT_ROOT.DOL_URL_ROOT, "md5");
...
function dol_syslog($message, $level = LOG_INFO, $ident = 0, $suffixinfilename='', $restricttologhandler='')
{
	...
	if (empty($conf->syslog->enabled)) return;
        ...
        if(! empty($message)){
            ...
            if(stripos($message, "Forbidden") !== false || stripos($message, "Duplicate") !== false){
                if(function_exists("synGetDebug"))
                    $message .= synGetDebug();
...
function dol_get_fiche_head($links=array(), $active='', $title='', $notab=0, $picto='', $pictoisfullpath=0, $morehtmlright='', $morecss='')
{
	...
	if (! empty($conf->dol_optimize_smallscreen)) $conf->global->MAIN_MAXTABS_IN_CARD=5;
