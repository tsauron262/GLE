...
function widget_listChronoModele[MARK1]() {
...
//    $table .= print_r($user->rights,1);
    if ($conf->global->MAIN_MODULE_SYNOPSISCHRONO && ($user->rights->synopsischrono->read || $user->rights->chrono_user->chrono[MARK1]->voir))
    {
        ...
        $sql = " SELECT * FROM ".MAIN_DB_PREFIX."synopsischrono as p
                  WHERE 1=1";
        if ($res->value."x" != "x")
...
//$table .= $sql;
        ...
        if ($resql)
        {
            ...
            if ($num > 0)
            {
                ...
                while ($res =$db->fetch_object($resql))
                {
                    ...
                    $table2 .=  '<td nowrap="nowrap"><a href="'.DOL_URL_ROOT.'/soc.php?socid='.$res->socid.'">'.img_object('','company')." ".dol_trunc($res->nom,18).'</a></td>';
        ...
        $table2 .="</p>";
      return array(
        'title' => 'Chrono - Derniers chronos ([MARK2])',
...
function widget_listChronoModele[MARK1]_settings() {
