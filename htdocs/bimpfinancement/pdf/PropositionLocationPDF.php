...
class PropositionLocationPDF extends BimpDocumentPDF
{
	...
	public function __construct($data)
	{
		...
		$this->ref_devis = BimpTools::getArrayValueFromPath($data, 'ref_devis', '');
	...
	public function initHeader()
	{
		...
		$header_right = '<span style="font-size: 9px">Limonest, le ' . date('d / m / Y') . ' </span>';
...
//		$html .= '</table>';
		...
		if (empty($this->lines)) {
			...
			if ($this->ref_devis) {
				$html .= ' selon le devis ' . $this->ref_devis;
			}
	...
	public function renderLines()
	{
		...
		if (count($table->rows)) {
			$this->writeContent('<div style="font-weight: bold; font-size: 9px;">Description des équipements et services' . ($this->ref_devis ? ' selon le devis ' . $this->ref_devis : '') . ' :</div>');
