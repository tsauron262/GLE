...
$requete = "SELECT count(*) as cnt, lat,lng, ".MAIN_DB_PREFIX."societe.nom,".MAIN_DB_PREFIX."societe.rowid FROM ".MAIN_DB_PREFIX."commande, ".MAIN_DB_PREFIX."societe, Babel_GeoBI
WHERE ".MAIN_DB_PREFIX."commande.fk_soc=".MAIN_DB_PREFIX."societe.rowid AND ".MAIN_DB_PREFIX."societe.rowid = Babel_GeoBI.`socid` AND lat is not null AND year(date_commande)  > ". $dateStart ."
...
while ($res = $db->fetch_object($sql))
{
        ...
        $dateStart = date('Y') - 3 ;
    $requete = "SELECT SUM(total_ht) as t, quarter(date_commande) as d, year(date_commande) as y,count(total_ht) as c
                  FROM ".MAIN_DB_PREFIX."commande
                 WHERE fk_soc = ". $res->rowid." AND year(date_commande)  > ". $dateStart ."
              GROUP BY d, y";
              //print $requete."\n";
    $arrPropal = array();
...
$table = '<table class"canvasStats" style="display: none;" id="commandeTable'.$res->rowid.'">
