...
if ($action == 'joindre' && $user->rights->equipement->creer) {
	...
	if ($ListEquip !="" ) {
		foreach ($ListEquip as $EquipID) {
			...
			$desc=GETPOST('np_desc', 'alpha');
			$dateo = dol_mktime(GETPOST('deohour', 'int'), GETPOST('deomin', 'int'), 0, 
							GETPOST('deomonth', 'int'), GETPOST('deoday', 'int'), GETPOST('deoyear', 'int')
			);
			$datee = dol_mktime(GETPOST('deehour', 'int'), GETPOST('deemin', 'int'), 0, 
							GETPOST('deemonth', 'int'), GETPOST('deeday', 'int'), GETPOST('deeyear', 'int')
			);
			$fulldayevent =GETPOST('fulldayevent');
			$fk_equipementevt_type =GETPOST('fk_equipementevt_type');
...
//	print_liste_field_titre($langs->trans("EquipementUnitaire")."<input size=10 type=text id='filterchk' name='filterchk' >", $_SERVER["PHP_SELF"],""," align=left ", $urlparam,'', $sortfield, $sortorder);
	...
	print_liste_field_titre($langs->trans("EquipementUnitaire"), $_SERVER["PHP_SELF"], "", " align=left ", $urlparam, '', $sortfield, $sortorder);
