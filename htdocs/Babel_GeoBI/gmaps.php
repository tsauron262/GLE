...
//    jQuery.getJSON('ajax/test.json.php', function(pData) {
    jQuery.getJSON('ajax/global.json.php', function(pData) {
      data = pData;
...
//                 'ps' => ($arrPropal['somme']> 0?price($arrPropal['somme']):0),
//                 'cs' => ($arrCommande['somme']> 0?price($arrCommande['somme']):0),
//                 'fs' => ($arrFacture['somme']> 0?price($arrFacture['somme']):0),

             infowindow[i] = new google.maps.InfoWindow({  content: '<div id="content">\
                        <H1>'+data.results[i].nom+'</H1>\
                        <p><div style="height: 340px; width: 450px;"><table border=0 width=400>\
                            <tr>\
                                <td colspan=3 style="padding-left: 25px">'+data.results[i].stat+'\
                            <tr><td colspan=3><br/>\
                            <tr class="ui-widget-header ui-state-default"><td>Propositions commerciale\
                                <td><span>'+data.results[i].pc+'</span>\
                                <td><span>'+data.results[i].ps+' &euro;</span>\
                            <tr class="ui-widget-header ui-state-default"><td>Commandes clients\
                                <td><span>'+data.results[i].cc+'</span>\
                                <td><span>'+data.results[i].cs+' &euro;</span>\
                            <tr class="ui-widget-header ui-state-default"><td>Factures clients\
                                <td><span>'+data.results[i].fc+'</span>\
                                <td><span>'+data.results[i].fs+' &euro;</span>\
                        </table></div></p>\
                    </div>',
                });
...
//             infowindow[i] = new google.maps.InfoWindow({  content: '<div id="content">\
//                        <H1>'+data.results[i].nom+'</H1>\
//                        <p><div style="height: 280px; width: 450px;"><table border=0>\
//                            <tr>\
//                                <td colspan=2 style="padding-left: 25px">'+data.results[i].stat+'\
//                            <tr><td colspan=2><br/>\
//                            <tr class="ui-widget-header ui-state-default"><td>globals clients\
...
//                });
      ...
      google.maps.event.addListener(marker[i], 'click', function() {
        for (var j=0;j<data.count;j++)
        {
         ...
         jQuery('#globalTable'+data.results[index].socid).visualize({
            width:380,
            height:200,
            appendTitle: true,
            title: "globals",
         });
         jQuery('#globalTable'+data.results[index].socid).parent().parent().parent().parent().parent().parent().parent().css('overflow','hidden');
      });
