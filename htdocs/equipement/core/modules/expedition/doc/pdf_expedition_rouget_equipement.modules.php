			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					if ($strTemp)
					{
						...
						$pdf->writeHTMLCell(140, 3, $this->posxdesc+10, $curY + 4 + $Ytoadd, $outputlangs->convToOutputCharset($strTemp), 0, 1);
						$curY = $curY+$Ytoadd;
					}
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						{
							$Ytoadd=$posyafterprint - $posybefore;
							$pdf->writeHTMLCell(140, 3, $this->posxdesc+10, $curY+$Ytoadd, $outputlangs->convToOutputCharset($strTemp), 0, 1);
							$curY = $curY+$Ytoadd;
						}
...
if ($resql)
{
	...
	function _getEquipementSerialDesc($object, $outputlangs, $i) 
	{
		...
		dol_syslog("PDF_Expedition_Rouget_Equipement::_getEquipementSerialDesc - Object line = ".$i." fk_product = ".$object->lines[$i]->fk_product."  idProd = '".$idprod."'");
		$retStr = '';
