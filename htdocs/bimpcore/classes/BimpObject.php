...
//            BimpCore::addlog('ID ' . $this->getLabel('of_the') . ' absent (201)',3);//trop trop lourd, arrive tous le temps
		}
		...
		if (!array_key_exists($new_status, static::$status_list)) {
			...
			if (!$status_prop) {
				...
				if ($current_status === $new_status) {
					$warnings[] = $object_label . ' a déjà le statut "' . $status_label . '"';
					return array();
				}
...
//        return $join_alias.'.nom_champ_db';
	...
	public function addNote($content, $visibility = null, $viewed = 0, $auto = 1, $email = '', $type_author = 1, $type_dest = 0, $fk_group_dest = 0, $fk_user_dest = 0, $delete_on_view = 0, $id_societe = 0, $email_cc)
	{
		$errors = array();
					...
					if (count($linkedObjects) > 0) {
						...
						foreach ($linkedObjects as $data_linked => $inut) {
							...
							if (!in_array($data_linked['object_name'], array('Equipment', 'BS_SAV', 'BS_Ticket', 'BR_Reservation', 'Bimp_PropalLine', 'Bimp_CommandeLine', 'Bimp_FactureLine', 'BimpRevalorisation', 'FactureDolLine', 'Bimp_Vente', 'Bimp_FactureFournLine', 'Bimp_CommandeFournLine', 'Bimp_Achat'))) {
								$filterLinked['linked']['or'][$data_linked["object_name"] . $data_linked['id_object']] = array(
									'and_fields' => array(
		...
		if ((int) BimpTools::getValue("redirectForce_oldVersion", 0, 'int')) {
			...
			if ($newVersion) {
				...
				if (BimpTools::isSubmit("search_status")) {
					$url .= "&fk_statut=" . BimpTools::getValue("search_status", 0, 'aZ09comma'); //potentiellement plusieurs 1,2
				}
