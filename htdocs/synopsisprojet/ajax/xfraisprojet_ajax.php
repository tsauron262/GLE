...
switch($action)
{
	...
	/*$requete = 'INSERT INTO ".MAIN_DB_PREFIX."Synopsis_global_ressources_resa ( datedeb, fk_user_author, tms, fk_projet, fk_projet_task,fk_user_imputation,fk_ressource)
                                            VALUES ("'.$dateAchat.'", "'.$acheteur_id.'",now(), "'.$fk_projet.'", "'.$fk_task.'", '.$fk_commande_fourn.','.$fk_facture_fourn.')';*/

       $requete = 'INSERT INTO ".MAIN_DB_PREFIX."Synopsis_global_ressources_resa2 ( montantHT, dateAchat, user_id, acheteur_id, fk_task, fk_projet, designation, fk_facture_fourn, fk_commande_fourn)
    ...
    case "edit":
    {
        ...
        $requete = 'UPDATE ".MAIN_DB_PREFIX."Synopsis_global_ressources_resa2
                       SET
                           montantHT="'.$montantHT.'",
                           dateAchat="'.$dateAchat.'",
                           user_id="'.$user_id.'",
    ...
    case 'del':
    {
        $requete = "DELETE FROM ".MAIN_DB_PREFIX."Synopsis_global_ressources_resa2 WHERE id=".$fraisId;
        $sql = $db->query($requete);
        if ($sql)
        {
            $xml .= "<ok>ok</ok>";
