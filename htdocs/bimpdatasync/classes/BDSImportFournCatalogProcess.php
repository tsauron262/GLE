...
class BDSImportFournCatalogProcess extends BDSImportProcess
{
    ...
    public function getFtpParams($params){
        return BimpTools::getArrayValueFromPath($this->params, $params, '');
    }
...
//        $this->DebugData($data, 'Données fichier "'.$fileName.'"');
    ...
    public function processFournPrices($lines, &$errors = array(), $processPV = false)
    {
        if (is_array($lines) && !empty($lines)) {
            $this->fetchProducts();
            
            global $db, $user;
            $dol_prod = new Product($db);
...
//                    $prod = BimpCache::getBimpObjectInstance('bimpcore', 'Bimp_Product', $id_product);
                    ...
                    if($processPV && $line['pa_ht'] < $line['pu_ht'] && ((float)$prod_data['pu_ht'] != (float)$line['pu_ht'] || (float)$prod_data['tva_tx'] != (float)$tva)){
                        ...
                        if($dol_prod->updatePrice($line['pu_ht'], 'HT', $user, $tva) < 0)
                            ...
                            $this->Success('Prix vente MAJ ancien : '.$prod_data['pu_ht'].' € nouveau : '.$line['pu_ht']. ' € '.$line['ref_fourn']);
                    }
    ...
    public function downloadFtpFile($fileName, &$errors = array(), $mode = FTP_BINARY)
    {
        ...
        if (!count($errors)) {
            ...
            if ($ftp !== false && !count($errors)) {
                ...
                if (!$check) {
                    $errors[] = 'Echec du téléchargement du fichier "' . $this->ftp_dir . $fileName . '" dans "' . $this->local_dir . $fileName . '"';
                }
            }
        }
