...
function dragDropLi()
{
    jQuery('#Dispo').find('li').click(function(){
        jQuery(this).clone().appendTo('#Ajoute');
        jQuery(this).remove();
        dragDropLi();
    });
    jQuery('#Ajoute').find('li').click(function(){
...
$(function() {
    ...
    jQuery('#addWidgetDialog').dialog({
        ...
        buttons: { "Ok": function() {
                                ...
                                var data = "ajoute="+serialize(arr1)+"&type="+dashtype+"&userid=-1";
                                jQuery.ajax({
                                    url: DOL_URL_ROOT+"/synopsistools/dashboard/ajax/addWidget-xml_response.php",
                                    data: data,
                                    type: "POST",
                                    cache: false,
                                    dataTypeString: "xml",
        ...
        open: function(){
            ...
            jQuery.ajax({
                url: DOL_URL_ROOT+"/synopsistools/dashboard/ajax/listWidget-xml_response.php",
                cache: false,
                data: 'userid=-1&type='+dashtype,
                dataTypeString: "xml",
                type: 'POST',
                success: function(msg){
                    jQuery(msg).find('disabled').each(function(){
                        jQuery('#Dispo').append('<li class="ui-state-hover ui-widget-header" id="'+jQuery(this).attr('id')+'"><span class="ui-icon ui-icon-cart" style="float: left; margin: -2px 3px 0 0"></span>'+jQuery(this).text()+'</li>');
                    });
//                    jQuery(msg).find('active').each(function(){
//                        jQuery('#activeWidget').append('<li id="'+jQuery(this).attr('id')+'">'+jQuery(this).text()+'</li>');
//                    });
                    dragDropLi();
                }
            })
  ...
  var options = {
    ...
    ajaxCallbacks: {
      ...
      getWidgetsByColumn: {
        url: DOL_URL_ROOT+'/synopsistools/dashboard/ajaxData-config.php?type='+dashtype,
        data: {
