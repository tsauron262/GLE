    ...
    if ($maskraz > 0) {   // A reset is required
        ...
        if ($maskraz > 1) { // if reset is not first month, we need month and year into mask
            ...
            if (preg_match('/^(.*)\{(m+)\}\{(y+)\}/i', $maskwithonlyymcode, $reg)) {
                $posy = 3;
                $posm = 2;
            } else if (preg_match('/^(.*)\{(y+)\}\{(m+)\}/i', $maskwithonlyymcode, $reg)) {
                $posy = 2;
                $posm = 3;
            } else if (preg_match('/^(.*)\{(y+)\}/i', $maskwithonlyymcode, $reg)) {
                $posy = 2;
                $posm = 0;
            } else
        ...
        else if (date("m", $date) < $maskraz && empty($resetEveryMonth)) {
            $yearoffset = -1;
...
function check_value($mask, $value) {
    ...
    $maskLike = str_replace(dol_string_nospecial('{' . $masktri . '}'), str_pad("", dol_strlen($maskcounter), "_"), $maskLike);
    if ($maskrefclient)
        $maskLike = str_replace(dol_string_nospecial('{' . $maskrefclient . '}'), str_pad("", strlen($maskrefclient), "_"), $maskLike);
